...
#define RandomInteger(N)                                                       \
  (assert(N > 0), basic_i_local = random(),                                    \
...
#define Length2(u) (((u).x * (u).x) + ((u).y * (u).y) + ((u).z * (u).z))

#define Length_l1(u) (Abs((u).x) + Abs((u).y) + Abs((u).z))

#define Length_l2(u) sqrt(((u).x * (u).x) + ((u).y * (u).y) + ((u).z * (u).z))

#define DotProduct(u, v) (((u).x * (v).x) + ((u).y * (v).y) + ((u).z * (v).z))

#define DotProduct2D(u, v) (((u).x * (v).x) + ((u).y * (v).y))
...
#define EdgeInBBox(bb_min, bb_max, p, q, eps)                                  \
  (MinMaxQ((p).x, (q).x, basic_h_local_min, basic_h_local_max),                \
   ((lt((bb_max).x - basic_h_local_min, eps))                                  \
        ? false                                                                \
        : ((lt(basic_h_local_max - (bb_min).x, eps))                           \
               ? false                                                         \
               : (MinMaxQ((p).y, (q).y, basic_h_local_min, basic_h_local_max), \
                  ((lt((bb_max).y - basic_h_local_min, eps))                   \
                       ? false                                                 \
                       : ((lt(basic_h_local_max - (bb_min).y, eps))            \
                              ? false                                          \
                              : (MinMaxQ((p).z, (q).z, basic_h_local_min,      \
                                         basic_h_local_max),                   \
                                 ((lt((bb_max).z - basic_h_local_min, eps))    \
                                      ? false                                  \
                                      : ((lt(basic_h_local_max - (bb_min).z,   \
                                             eps))                             \
                                             ? false                           \
                                             : true)))))))))
...
#define EdgeInBBoxPlain(bb_min, bb_max, p, q)                                  \
  (MinMaxQ((p).x, (q).x, basic_h_local_min, basic_h_local_max),                \
   ((lt((bb_max).x - basic_h_local_min, 0.0))                                  \
        ? false                                                                \
        : ((lt(basic_h_local_max - (bb_min).x, 0.0))                           \
               ? false                                                         \
               : (MinMaxQ((p).y, (q).y, basic_h_local_min, basic_h_local_max), \
                  ((lt((bb_max).y - basic_h_local_min, 0.0))                   \
                       ? false                                                 \
                       : ((lt(basic_h_local_max - (bb_min).y, 0.0))            \
                              ? false                                          \
                              : (MinMaxQ((p).z, (q).z, basic_h_local_min,      \
                                         basic_h_local_max),                   \
                                 ((lt((bb_max).z - basic_h_local_min, 0.0))    \
                                      ? false                                  \
                                      : ((lt(basic_h_local_max - (bb_min).z,   \
                                             0.0))                             \
                                             ? false                           \
...
#define TriInBBox(bb_min, bb_max, p, q, r, eps)                                \
  (MinMax3Q((p).x, (q).x, (r).x, basic_h_local_min, basic_h_local_max),        \
   ((lt((bb_max).x - basic_h_local_min, eps))                                  \
        ...
        : ((lt(basic_h_local_max - (bb_min).x, eps))                           \
               ? false                                                         \
               : (MinMax3Q((p).y, (q).y, (r).y, basic_h_local_min,             \
                           basic_h_local_max),                                 \
                  ((lt((bb_max).y - basic_h_local_min, eps))                   \
                       ? false                                                 \
                       : ((lt(basic_h_local_max - (bb_min).y, eps))            \
                              ? false                                          \
                              : (MinMax3Q((p).z, (q).z, (r).z,                 \
                                          basic_h_local_min,                   \
                                          basic_h_local_max),                  \
                                 ((lt((bb_max).z - basic_h_local_min, eps))    \
                                      ? false                                  \
...
#define SphInBBox(bb_min, bb_max, p, rad, eps)                                 \
  ((lt((bb_max).x - (p).x + rad, eps))                                         \
       ? false                                                                 \
       : ((lt((bb_max).y - (p).y + rad, eps))                                  \
              ? false                                                          \
              : ((lt((bb_max).z - (p).z + rad, eps))                           \
                     ? false                                                   \
                     : ((lt((p).x + rad - (bb_min).x, eps))                    \
                            ? false                                            \
                            : ((lt((p).y + rad - (bb_min).y, eps))             \
                                   ? false                                     \
                                   : ((lt((p).z + rad - (bb_min).z, eps))      \
                                          ? false                              \
                                          : true))))))

#define SphInSphBBox(p, r1, q, r2, eps)                                        \
  ((basic_h_local_delta = r1 + r2),                                            \
   ((basic_h_local = (q).x - (p).x), (basic_h_local = Abs(basic_h_local)),     \
    ((gt(basic_h_local - basic_h_local_delta, eps))                            \
         ? false                                                               \
  ...
  {                                                                            \
    ...
    if ((I > 0) || (J > 0)) {                                                  \
      ...
      if (!eq(basic_h_local_delta, zero)) {                                    \
        (xy)[J] =                                                              \
            ((B)[j] - basic_h_local_quot * (B)[i]) / basic_h_local_delta;      \
...
#define SphSphTest3D(p, r_1, q, r_2, eps)                                      \
  ((basic_h_local_delta = (r_1) + (r_2)), (basic_h_v1.x = (q).x - (p).x),      \
   (basic_h_v1.y = (q).y - (p).y), (basic_h_v1.z = (q).z - (p).z),             \
...
#define TriPnt2D(A, B, C, P)                                                   \
  ((Det2D(A, B, P) <= 0.0)                                                     \
       ? 0                                                                     \
       : ((Det2D(B, C, P) <= 0.0) ? 0 : ((Det2D(C, A, P) <= 0.0) ? 0 : 1)))

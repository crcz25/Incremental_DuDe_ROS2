...
public:
  ...
  double getOptimalCuts(list<ply_vertex *> &pms, double tau,
...
protected:
  struct cell {
    ...
    void unionCuts(vector<c_diagonal>
                       &diagonals); // compute the unions of all cuts in cutsets
    double score;
  };

  void initTable(); // initialize the table

  void updateCell(uint n, uint m,
                  uint iteration); // update the solution in cell n,m
...
public:
        ...
        double getIncresed_score(vector< pair<int,int> > cuts_m, cut_set
cut_set_m, int k, int m, int n); void fillTable();//fill out every table item
};
*/

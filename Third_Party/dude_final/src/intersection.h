...
template <class real>
real Area(const real a[2], const real b[2], const real c[2]) {
  return (b[0] - a[0]) * (c[1] - a[1]) - (c[0] - a[0]) * (b[1] - a[1]);
}

template <class real>
int AreaSign(const real a[2], const real b[2], const real c[2]) {
...
template <class real>
int Collinear(const real a[2], const real b[2], const real c[2]) {
...
---------------------------------------------------------------------*/
template <class real>
bool Between(const real a[2], const real b[2], const real c[2]) {
...
template <class real>
bool Between_strict(const real a[2], const real b[2], const real c[2]) {
  // If ab not vertical, check betweenness on x; else on y.
  if (a[0] != b[0]) {
...
template <class real>
char ParallelInt(const real a[2], const real b[2], const real c[2],
                 const real d[2], real p[2]) {
  if (!Collinear(a, b, c))
    return '0';
...
template <class real>
bool Union(const real a[2], const real b[2], const real c[2], const real d[2],
           const real *p[2]) {
  int id = 0;
  if (Equal(a, c)) {
    p[id] = a;
...
template <class real>
bool SegSegInt(const real a[2], const real b[2], const real c[2],
               const real d[2]) {
...
---------------------------------------------------------------------*/
template <class real>
char SegSegInt(const real a[2], const real b[2], const real c[2],
               const real d[2], real p[2]) {
  ...
  denom = a[0] * (d[1] - c[1]) + b[0] * (c[1] - d[1]) + d[0] * (b[1] - a[1]) +
          c[0] * (a[1] - b[1]);
  ...
  // compute s
  num_s = a[0] * (d[1] - c[1]) + c[0] * (a[1] - d[1]) + d[0] * (c[1] - a[1]);

  if (fabs(num_s) < SMALLNUMBER)
    num_s = 0;
  else if (fabs(num_s - denom) < SMALLNUMBER)
    num_s = denom;
  ...
  // compute t
  num_t = -(a[0] * (c[1] - b[1]) + b[0] * (a[1] - c[1]) + c[0] * (b[1] - a[1]));

  if (fabs(num_t) < SMALLNUMBER)
    num_t = 0;
  else if (fabs(num_t - denom) < SMALLNUMBER)
    num_t = denom;
...
template <class real> real VdotV(real V1[3], real V2[3]) {
  return (V1[0] * V2[0] + V1[1] * V2[1] + V1[2] * V2[2]);
}

template <class real>
void VcrossV(real Vr[3], const real V1[3], const real V2[3]) {
  Vr[0] = V1[1] * V2[2] - V1[2] * V2[1];
  Vr[1] = V1[2] * V2[0] - V1[0] * V2[2];
  Vr[2] = V1[0] * V2[1] - V1[1] * V2[0];
}

template <class real> real max(real a, real b, real c) {
  real t = a;
  if (b > t)
    t = b;
  if (c > t)
...
template <class real> real min(real a, real b, real c) {
  real t = a;
  if (b < t)
    t = b;
  if (c < t)
...
template <class real>
int my_project6_2(real *ax, real *p1, real *p2, real *p3, real *q1, real *q2,
                  real *q3) {
  real P1 = VdotV(ax, p1);
  real P2 = VdotV(ax, p2);
  real P3 = VdotV(ax, p3);
...
template <class real>
int my_tri_contact(real *P1, real *P2, real *P3, real *Q1, real *Q2, real *Q3) {
  ...
  if (!my_project6_2(n1, p1, p2, p3, q1, q2, q3))
    return 0;
  if (!my_project6_2(m1, p1, p2, p3, q1, q2, q3))
    return 0;

  if (!my_project6_2(ef11, p1, p2, p3, q1, q2, q3))
    return 0;
  if (!my_project6_2(ef12, p1, p2, p3, q1, q2, q3))
    return 0;
  if (!my_project6_2(ef13, p1, p2, p3, q1, q2, q3))
    return 0;
  if (!my_project6_2(ef21, p1, p2, p3, q1, q2, q3))
    return 0;
  if (!my_project6_2(ef22, p1, p2, p3, q1, q2, q3))
    return 0;
  if (!my_project6_2(ef23, p1, p2, p3, q1, q2, q3))
    return 0;
  if (!my_project6_2(ef31, p1, p2, p3, q1, q2, q3))
    return 0;
  if (!my_project6_2(ef32, p1, p2, p3, q1, q2, q3))
    return 0;
  if (!my_project6_2(ef33, p1, p2, p3, q1, q2, q3))
    return 0;

  if (!my_project6_2(g1, p1, p2, p3, q1, q2, q3))
    return 0;
  if (!my_project6_2(g2, p1, p2, p3, q1, q2, q3))
    return 0;
  if (!my_project6_2(g3, p1, p2, p3, q1, q2, q3))
    return 0;
  if (!my_project6_2(h1, p1, p2, p3, q1, q2, q3))
    return 0;
  if (!my_project6_2(h2, p1, p2, p3, q1, q2, q3))
    return 0;
  if (!my_project6_2(h3, p1, p2, p3, q1, q2, q3))
    return 0;

  return 1;
}

template <class real> real distance2D(real a[2], real b[2]) {
  return sqrtf((a[0] - b[0]) * (a[0] - b[0]) + (a[1] - b[1]) * (a[1] - b[1]));
}

template <class real>
real distance2D(const real &a1, const real &b1, const real &a2,
                const real &b2) {

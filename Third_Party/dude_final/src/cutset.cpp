...
double c_cutset::m_tau =
    0; // private member, this is specified through build_cutsets

inline double angleAt(const Point2d &p1, const Point2d &p2,
                      const Point2d &p3) { // to compute the inner angle at p2
...
inline void sortDiagonalsbyAngle(
    ...
    const vector<c_diagonal> &diagonals,
    vector<pair<double, vector<c_diagonal>::const_iterator>> &angles) {
  typedef vector<c_diagonal>::const_iterator IT;

  {
    double total_angle = angleAt(
        start, v->getPos(), end); // dihedral angle of v between start and end
...
#if DEBUG
      cout << "\t (" << i->getV1()->getVID() << "," << i->getV2()->getVID()
           << ") Angle=" << angle << endl;
#endif
  ...
  // make sure that the angle between (start,v) and (v,end) is at the end of the
  // list
  assert(angles.back().second == diagonals.end());
}

// get angles between two adjacent cuts
inline void getDihedralAngles(
    vector<double> &dihedral_angles,
    const vector<pair<double, vector<c_diagonal>::const_iterator>> &angles) {
...
inline int locateReflex(vector<double> dihedral_angles, int start, int end,
                        bool begin, double tau = PI) {
...
// assuming that the diagonals are bounded between segments (v,start) and
// (v,end)
void c_cutset::build_cutsets(ply_vertex *v, const Point2d &start,
                             const Point2d &end,
...
#if DEBUG
  if (diagonals.empty()) {
    cerr << "! ERROR: no diagonals for building cutsets for PM[" << v->getVID()
         << "]" << endl;
  }
  cout << "- PM[" << v->getVID() << "] has " << diagonals.size() << " diagonals"
       << endl;
#endif
  ...
  // category of tips
  list<c_cutset>
      single_PM; // single diagonal, connecting v to a PM (pocket minimum)
  list<c_cutset>
      single_TIP; // single diagonal, connecting v to a tip (regular vertex)
  list<c_cutset> double_PM_PM; // double diagonal, connecting v to two PMs
  list<c_cutset>
      double_PM_TIP; // double diagonal, connecting v to a PM and a tip
  ...
  uint diagonal_size =
      angles.size() - 1; // minus one because the last angle is (v,end)
  int pos1 = locateReflex(
      dihedral_angles, diagonal_size, 0, false,
      PI * 1.1); // cuts [0 1 ... pos1-1] will not resolve this minimum
  int pos2 = locateReflex(
      dihedral_angles, 0, diagonal_size, true,
      PI * 1.1); // cuts [pos2 ... size] will not resolve this minimum
  ...
  // collect all cut sets [{pos1} ... {pos2-1}] because each cut resolves this
  // minimum by itself classify these diagonals to single_PM and single_PM
  ...
  //
  // to collect cut sets with size 2, there are no cut sets with size larger
  // than 2 [0 1 ... pos1-2] [pos2+1 ... end-1] compute angles between pos2 and
  // 0 and stop at angle larger than 180 which is pos3 if pos3==pos1-1, return
  // no other cut sets else return {pos3+1,pos2}, {pos3+2,pos2}...{pos1-1,pos2}
  // do the same thing to [pos2+1 ... size] may use some intermediate results
  // for pos2+1 if(pos1>pos2-1) return;
  ...
  // now, if there is no cutset of this vertex (because no diagonals can resolve
  // this PM) we simply select one that has the highest score
...
// a static method for buidling all cutsets for a given vertex v  using residual
// concavity assuming that the diagonals are bounded between segments (v,start)
// and (v,end)
void c_cutset::build_cutsets(ply_vertex *v, const Point2d &start,
                             const Point2d &end,
                             const vector<c_diagonal> &diagonals, double tau,
...
// cutset_list stores results
{
  ...
  // collect all cuts that can resolve this pocket minimum by itself
  //
  // go through all diagonals and determine it the residual concavity is too
  // high classify these diagonals to single_PM and single_PM
  //
  ...
  for (uint pos = 0; pos < diagonal_size; pos++) {
    ...
    if (it->is_resolving(v, m_tau) == false)
      continue; // this diagonal does not resolve v...
  ...
  // compute angles between pos2 and 0 and stop at angle larger than 180 which
  // is pos3 if pos3==pos1-1, return no other cut sets else return
  // {pos3+1,pos2}, {pos3+2,pos2}...{pos1-1,pos2} do the same thing to [pos2+1
  // ... size] may use some intermediate results for pos2+1 if(pos1>pos2-1)
  // return;
...
void c_cutset::handle_single_TIP(list<c_cutset> &cutsets) {
...
#else
    if (dia.is_resolving(i->getV(), m_tau) ==
        false) // || dia.is_angle_resolving(i->getV())==false)
#endif
    {
  ...
  if (shortest_cutset_resolve != cutsets.end()) {
    shortest_cutset_resolve->getV()->getExtra().cutsets.push_back(
        *shortest_cutset_resolve);
#if DEBUG
    cout << *shortest_cutset_resolve << " [single tip]" << endl;
#endif
  } else if (shortest_cutset_none_resolve != cutsets.end()) {
    shortest_cutset_none_resolve->getV()->getExtra().cutsets.push_back(
        *shortest_cutset_none_resolve);
#if DEBUG
...
bool c_cutset::conflict(const c_cutset &other) const {
  ...
  if (othersize == 1 && mysize == 1) {
    if (other.getDiagonals().front() == getDiagonals().front())
  ...
  // non of above...
  const c_diagonal &diagonal = other.getDiagonals().front();
  if (conflict(other, diagonal))
    return true;

  const c_diagonal &diagonal2 = getDiagonals().front();
  if (other.conflict(*this, diagonal2))
    return true;
  ...
  if (mysize == 2) {
    ...
    if (other.conflict(*this, diagonal))
      return true;
  }
...
bool c_cutset::conflict(const c_cutset &other,
                        const c_diagonal &diagonal) const {
  ply_vertex *v = diagonal.getOther(other.m_v);
  if (v == m_v) { // check if other is a cutset for m_v
    if (other.isMemberOf(m_v->getExtra().cutsets))
      return true;
  }
  return false;
}
...
bool c_cutset::operator==(const c_cutset &other) const {
  if (other.m_diagonals.size() != m_diagonals.size())
    return false;
...
ostream &operator<<(ostream &out, const c_cutset &cset) {
  ...
  if (cset.getDiagonals().size() == 1) {
    out << "\t cutset {(" << diagonals.back().getV1()->getVID() << ","
        << diagonals.back().getV2()->getVID() << ")}";
    out << " score=" << cset.getWeight();
    out << " "
        << ((cset.getType() == c_cutset::SINGLE_PM) ? "SINGLE_PM"
                                                    : "SINGLE_TIP");
  } else {
    out << "\t cutset {(" << diagonals.front().getV1()->getVID() << ","
        << diagonals.front().getV2()->getVID() << "), "
        << "(" << diagonals.back().getV1()->getVID() << ","
        << diagonals.back().getV2()->getVID() << ")}";
    out << " score=" << cset.getWeight();
    out << " ";
    if (cset.getType() == c_cutset::DOUBLE_PM_PM)
      out << "DOUBLE_PM_PM";
    else if (cset.getType() == c_cutset::DOUBLE_PM_TIP)
      out << "DOUBLE_PM_TIP";
    else if (cset.getType() == c_cutset::DOUBLE_TIP_TIP)
      out << "DOUBLE_TIP_TIP";
    else
      out << "??";
  }
...
void c_cutset::setResolveSize(short n) {
  ...
  //    if(d1.getV1()->getVID()==36 && d1.getV2()->getVID()==21 &&
  //    m_diagonals.size()==1)
  //    {
  //        check=true;

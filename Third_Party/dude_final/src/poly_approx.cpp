...
struct c_chain {
  ...
  float distance(const Point2d &p) {
    assert(pm);
    return fabs((pm->getPos() - p) * normal);
  }
  ...
  void recompute_dist() { dist = estimate_dist(s, e); }
...
inline bool expand(c_chain *chain) {
  ...
  if (t1 != NULL) { // check if the chain can be expanded to t1
    ...
    if (chain->overlap_pre()) { // overlapped, should cp give up cp->e?
      ...
      float old_score =
          chain->distance(cp->e->getPos()) + cp->distance(cp->e->getPos());
      float new_score =
          chain->distance(t1->getPos()) + cp->distance(t1->getPos());
  ...
  if (t2 != NULL) {
    ...
    if (chain->overlap_next()) { // overlapped, should cn give up cn->s?
      ...
      float old_score =
          chain->distance(cn->s->getPos()) + cn->distance(cn->s->getPos());
      float new_score =
          chain->distance(t2->getPos()) + cn->distance(t2->getPos());
      if (new_score < old_score) {
        if (cn->s == cn->pm) {
          update2 = false;
        }
...
// inline float approximate(const list<ply_vertex*>& seeds, list<ply_vertex*>&
// approx)
inline float approximate(const vector<c_chain *> &chains,
  ...
  while (open.empty() == false) {
    ...
    if (expanded) {
      ...
      make_heap(open.begin(), open.end(),
                chain_comp); // make heap b/c other chain may have changed...
    }
  ...
  for (vector<c_chain *>::const_iterator i = chains.begin(); i != chains.end();
       i++) {
...
//
void approximate(const c_plyline &plyline, double tau,
                 list<ply_vertex *> &approx) {

...
void drawRefCircle() {
  ...
  glColor3d(0, 1, 0);
  drawCircle(
      Point2d(draw_decoration.box[1] - radius, draw_decoration.box[3] - radius),
      radius);
}
...
inline void drawNormals(const c_ply &P) {
  ...
  do {
    ...
    Point2d mid2(mid1[0] + n[0] * draw_decoration.normal_length,
                 mid1[1] + n[1] * draw_decoration.normal_length);
...
void drawVID(vector<ply_vertex *> &vl) {
  ...
  char tmp[36];
  glColor3d(0, 0, 0);
  for (vector<ply_vertex *>::const_iterator i = vl.begin(); i != vl.end();
       i++) {
...
void drawBPC(c_BPC *bpc, float largest_concavity, float smallest_concavity) {
  ...
  /*
      ...
      if(showConcavity)
      {
          ...
          const vector<ply_vertex *>& pms= VIP_vertices;
     //bpc->getConcavities(); for(vector<ply_vertex *>::const_iterator
     i=pms.begin();i!=pms.end();i++)
          {
...
void drawBPCs() {
  ...
  static float smallest_concavity =
      draw_decoration.MINCONCAVITY / pR; //         FLT_MAX;
  ...
  //            for(list<ply_vertex *>::const_iterator
  //            j=pms.begin();j!=pms.end();j++){
  ...
  //        }
  //
  //        const vector<ply_vertex *>&
  //        holefp=allFeaturePMs;//dude.holeFeatPnts; for(vector<ply_vertex
  //        *>::const_iterator j=holefp.begin();j!=holefp.end();j++){
  //            if( (*j)->getExtra().isPM()==false ||
  //            (*j)->getExtra().diagonals.empty() ) continue; float
  //            c=(*j)->getExtra().concavity; if(c<smallest_concavity){
  //                smallest_concavity=c;
  //            }
  //            if(c>largest_concavity){
  //                largest_concavity=c;
  ...
  if (draw_options.showConstraints) {
    ...
    //        for(vector<RiBox*>::iterator bit = gridBoxes.begin(); bit !=
    //        gridBoxes.end(); ++bit)
    //        {
    //        	RiBox* tb = *bit;
    //        	glBegin(GL_POLYGON);
    ...
    glBegin(GL_LINES);
    // for testing evaluation
    for (vector<c_diagonal>::iterator dit = dude.holeCutDiagonals.begin();
         dit != dude.holeCutDiagonals.end(); ++dit) {
      c_diagonal &diag = *dit;
    ...
    if (draw_decoration.holecutRoot.getV1() != NULL) {
      ...
      glBegin(GL_LINES);
      glVertex2f(draw_decoration.holecutRoot.getV1()->getPos()[0],
                 draw_decoration.holecutRoot.getV1()->getPos()[1]);
      glVertex2f(draw_decoration.holecutRoot.getV2()->getPos()[0],
                 draw_decoration.holecutRoot.getV2()->getPos()[1]);
    ...
    glBegin(GL_LINES);
    // for testing evaluation
    for (vector<pair<Point2d, Point2d>>::iterator dit =
             draw_decoration.holeCutTreeLines.begin();
         dit != draw_decoration.holeCutTreeLines.end(); ++dit) {
  ...
  if (draw_options.showRepUserCuts) {
    ...
    glBegin(GL_LINES);
    for (vector<c_diagonal>::iterator dit = draw_decoration.repDiags.begin();
         dit != draw_decoration.repDiags.end(); ++dit) {
      c_diagonal &diag = *dit;
      glVertex2f(diag.getV1()->getPos()[0], diag.getV1()->getPos()[1]);
      glVertex2f(diag.getV2()->getPos()[0], diag.getV2()->getPos()[1]);
    }
  ...
  if (false) {
    ...
    glBegin(GL_LINES);
    for (vector<c_diagonal>::iterator dit = draw_decoration.compDiags.begin();
         dit != draw_decoration.compDiags.end(); ++dit) {
      c_diagonal &diag = *dit;
      glVertex2f(diag.getV1()->getPos()[0], diag.getV1()->getPos()[1]);
      glVertex2f(diag.getV2()->getPos()[0], diag.getV2()->getPos()[1]);
    }
  ...
  if (draw_options.showAllUserCutsInClustering) {
    static float colors[][3] = {
        {0.5, 1.0, 0.5},    {1.0, 1.0, 0.5},    {1.0, 0.5, 0.5},
        {0.5, 1.0, 1.0},    {1.0, 0.5, 1.0},    {0.5, 0.75, 0.5},
        {0.75, 0.75, 0.5},  {0.75, 0.5, 0.5},   {0.5, 0.75, 0.75},
        {0.75, 0.5, 0.75},  {0.25, 1.0, 0.25},  {1.0, 1.0, 0.25},
        {1.0, 0.25, 0.25},  {0.25, 1.0, 1.0},   {1.0, 0.25, 1.0},
        {0.25, 0.75, 0.25}, {0.75, 0.75, 0.25}, {0.75, 0.25, 0.25},
    ...
    for (uint i = 0; i < size; i++) {
      glColor3fv(colors[i]);
      glBegin(GL_LINES);
      for (vector<c_diagonal>::iterator dit =
               draw_decoration.userDiags[i].begin();
           dit != draw_decoration.userDiags[i].end(); ++dit) {
        c_diagonal &diag = *dit;
  ...
  if (draw_options.showVertexID)
    drawVID(draw_decoration.VIP_vertices);
  ...
  if (draw_decoration.g_selected_PM != NULL) {
    ...
    double scale_avg = average * (c - smallest_concavity + 0.2) /
  ...
  if (draw_options.showConcavity) {
    // cerr<<"max concavity and min concavity
    // "<<largest_concavity<<"\t"<<smallest_concavity<<endl;
    float scale_avg = average;
    float pR = getP().front().getRadius();
    glTranslated(0, 0, 0.1);
    glColor3d(1, 0, 0);
    ...
    vector<float>::const_iterator cit =
        draw_decoration.allFeatureConcavities.begin();
    for (vector<ply_vertex *>::const_iterator i = pms.begin(); i != pms.end();
         i++, ++cit) {
      // float c=(*i)->getExtra().concavity/pR;
      float c = *cit;
      const Point2d &pos = (*i)->getPos();
      scale_avg = average * (c - smallest_concavity + 0.2) /
                  (largest_concavity - smallest_concavity + 0.2) * 2;
      // cerr<<scale_avg<<endl;
      drawCircle(pos, scale_avg);
    }

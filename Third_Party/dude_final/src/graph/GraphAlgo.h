 ...
 */
template <class GRAPH> class visitor_base {
  /**
   * GRAPH's iterators
   */
...
public:
  ...
  visitor_base() {}
  /**
   * All the visitors will be related with a graph. This constructor can be used
   * when you nees to store the Graph associated with the current
   * traversal/visitor.
   */
...
int _functor_BFS(GRAPH &_g, visitor_base<GRAPH> &_f, VID _startVid,
                 map<VID, int> &_color) {
      ...
      for (ei = v1->edgelist.begin(); ei < v1->edgelist.end(); ei++) {
        ...
        if (_g.IsVertex(v2id, &v2)) {
          if (_color[v2id] == WHITE) {
            ...
            cout << "ERROR: colors in BFS functor" << endl;
        }
      }
      _color[v1id] = BLACK;
      _f.finish_vertex(v1);
 ...
 * @return ERROR if something wrong happened
 */
template <class GRAPH> int BFS_functor(GRAPH &_g, visitor_base<GRAPH> &_f) {
  map<VID, int> _color;
  int temp;
...
*/
template <class GRAPH>
int _recursive_DFS(GRAPH &_g, visitor_base<GRAPH> &_f, map<VID, int> &_color,
                   VID _v1id) {
  ...
  while (ei != v1->edgelist.end()) {
    ...
    if (_g.IsVertex(v2id, &v2)) {
      if (_color[v2id] == WHITE) {
        ...
        cout << "ERROR: colors in DFS functor" << endl;
    } else
 ...
 * @return ERROR if something wrong happened
 */
template <class GRAPH> int DFS_functor(GRAPH &_g, visitor_base<GRAPH> &_f) {
  map<VID, int> _color;
  int temp;
  // for all vertices check if it was touched
  for (typename GRAPH::VI vi = _g.begin(); vi != _g.end(); vi++) {
...
template <class GRAPH>
int _functor_BFS_EQ(GRAPH &_g, visitor_base<GRAPH> &_f, VID _startVid,
                    map<VID, int> &_color) {
  ...
  while (!q.empty()) {
    ...
    if (_g.IsVertex(v1id, &v1)) {
      for (ei = v1->edgelist.begin(); ei != v1->edgelist.end(); ei++) {
        ...
        if (_g.IsVertex(v2id, &v2)) {
          if (_color[v2id] == WHITE) {
            if (_f.vertex(v2) == EARLY_QUIT)
              return EARLY_QUIT;
            if (_f.tree_edge(v1, ei) == EARLY_QUIT)
              return EARLY_QUIT;
            q.push_back(v2id);
            _color[v2id] = GRAY;
          } else if (_color[v2id] == GRAY) {
            if (_f.back_edge(v1, ei) == EARLY_QUIT)
              return EARLY_QUIT;
          } else if (_color[v2id] == BLACK) {
            if (_f.ft_edge(v1, ei) == EARLY_QUIT)
              return EARLY_QUIT;
          } else
            cout << "ERROR: BFS EQ colors" << endl;
        } else
          cout << "\nIn GraphBFS: OOPS! vertex=" << v2id << " not in graph";
      }
      _color[v1id] = BLACK;
      if (_f.finish_vertex(v1) == EARLY_QUIT)
        return EARLY_QUIT;
    } else {
      cout << "\nIn GraphBFS: OOPS! vertex=" << v1id << " not in graph";
    }
 ...
 * from _startV in a BFS manner; If one of the visitor methods returns
 *EARLY_QUIT the BFS search will be cancelled(EARLY QUIT)
 *@param _g the graph for which we apply DFS
 *@param _f the visitor that will be used for traversal
 ...
 */
template <class GRAPH>
int BFS_functor_EQ(GRAPH &_g, visitor_base<GRAPH> &_f,
                   typename GRAPH::VERTEX_TYPE &_startV) {
  typename GRAPH::CVI cv1;
  if (_g.IsVertex(_startV, &cv1)) {
    map<VID, int> _color;
...
/**BFS functor EARLY QUIT; Apply visitor _f to all the nodes in _g accesible
 * from _startVid in a BFS manner; If one of the visitor methods returns
 *EARLY_QUIT the BFS search will be cancelled(EARLY QUIT)
 *@param _g the graph for which we apply DFS
 *@param _f the visitor that will be used for traversal
 *@param _startVid start vertex for traversal
...
template <class GRAPH>
int _recursive_DFS_EQ(GRAPH &_g, visitor_base<GRAPH> &_f, map<VID, int> &_color,
                      VID _v1id) {
 ...
 * from _startV in a DFS manner; If one of the visitor methods returns
 *EARLY_QUIT the DFS search will be cancelled(EARLY QUIT)
 *@param _g the graph for which we apply DFS
 *@param _f the visitor that will be used for traversal
 *@param _startV start vertex for traversal
 * @return the success of the operation
...
/**DFS functor EARLY QUIT; Apply visitor _f to all the nodes in _g accesible
 * from _startVid in a DFS manner; If one of the visitor methods returns
 *EARLY_QUIT the DFS search will be cancelled(EARLY QUIT)
 *@param _g the graph for which we apply DFS
 *@param _f the visitor that will be used for traversal
 *@param _startVid start vertex for traversal
 ...
 * @return the success of the operation
 */
template <class GRAPH> int DFS_functor_EQ(GRAPH &_g, visitor_base<GRAPH> &_f) {
  map<VID, int> _color;
  int temp;
  // for all vertices check if it was touched
  for (typename GRAPH::VI vi = _g.begin(); vi != _g.end(); vi++) {
...
//////////////////////////////////////////////////
// original Graph.h DFS ;; returns a Graph; do not use it anymore it will be
// taken out
//////////////////////////////////////////////////
...
template <class GRAPH> void aux_DFS(GRAPH &g, dfsinfo &dfs) {
  ...
  typedef Graph<DG<typename GRAPH::VERTEX_TYPE, typename GRAPH::WEIGHT_TYPE>,
                ...
                typename GRAPH::VERTEX_TYPE, typename GRAPH::WEIGHT_TYPE>
      DGRAPH;
...
template <class GRAPH> class _visitor_dfsvid : public visitor_base<GRAPH> {
  vector<VID> *ccverts;

public:
  _visitor_dfsvid(GRAPH &g, vector<VID> *_v) { ccverts = _v; };
 ...
 *@param _V the vector where the data will be stored;
 */
template <class GRAPH> int DFSVID(GRAPH &_g, VID &_startVID, vector<VID> &_V) {
  _V.clear();
  _visitor_dfsvid<GRAPH> _vis(_g, &_V);
 ...
 *@param _g the graph for which we apply DFS
 *@param _V the vector of traversal trees(Directed Graphs) where the data will
 *be stored;
 */
template <class DGRAPH, class GRAPH> int DFS_OLD(GRAPH &g, vector<DGRAPH> &_V) {
 ...
 *be stored;
 */
template <class DGRAPH, class GRAPH> int DFS(GRAPH &_g, vector<DGRAPH> &_V) {
  map<VID, int> _color;
  int temp;
  // for all vertices check if it was touched
  for (typename GRAPH::VI vi = _g.begin(); vi != _g.end(); vi++) {
...
template <class GRAPH> class _visitor_cycle : public visitor_base<GRAPH> {
public:
 ...
 */
template <class GRAPH> bool IsCycle(GRAPH &_g) {
...
template <class GRAPH> bool IsCycle_OLD(GRAPH &g) {
  dfsinfo dfs(g.GetVertexCount());
  aux_DFS(g, dfs);
...
*********************/
template <class GRAPH> class _visitor_back_edges : public visitor_base<GRAPH> {
 ...
 *@see dfs.backedge_vector and aux_DFS
 */
template <class GRAPH> int GetBackedge(GRAPH &_g, vector<pair<VID, VID>> &_V) {
  _V.clear();
...
****************************************/
template <class T> struct __FinishLate : public binary_function<T, T, bool> {
  bool operator()(T x, T y) { return x.second > y.second; }
};

template <class GRAPH> int TopologicalSort(GRAPH &g, vector<VID> &_V) {
...
template <class GRAPH> class _visitor_topsort : public visitor_base<GRAPH> {
  vector<VID> *ccverts;

public:
  _visitor_topsort(GRAPH &g, vector<VID> *_v) { ccverts = _v; };
 ...
 */
template <class GRAPH> int TopologicalSort_VIS(GRAPH &_g, vector<VID> &_V) {
...
int true_BFS(GRAPH &g, DGRAPH &bfstree, VID _startVid) {
  ...
  while (!q.empty()) {
    ...
    if (g.IsVertex(v1id, &v1)) {
      for (typename GRAPH::CEI e = v1->edgelist.begin(); e < v1->edgelist.end();
           e++) {
        v2id = e->vertex2id;
...
template <class GRAPH> class _visitor_bfsvid : public visitor_base<GRAPH> {
  vector<VID> *ccverts;

public:
  _visitor_bfsvid(GRAPH &g, vector<VID> *_v) { ccverts = _v; };
 ...
 *@see BFS, GetCC
 */
template <class GRAPH> int BFSVID(GRAPH &_g, VID &_startVID, vector<VID> &_V) {
  _V.clear();
  _visitor_bfsvid<GRAPH> _vis(_g, &_V);
...
template <class GRAPH>
int FindPathBFS(
    GRAPH &_g, typename GRAPH::VERTEX_TYPE &_startV,
    typename GRAPH::VERTEX_TYPE &_endV,
    vector<pair<typename GRAPH::VERTEX_TYPE, typename GRAPH::WEIGHT_TYPE>>
        &_path) {
...
template <class GRAPH>
int FindPathBFS(
    GRAPH &g, VID _startVid, VID _endVid,
    vector<pair<typename GRAPH::VERTEX_TYPE, typename GRAPH::WEIGHT_TYPE>>
        &_path) {
  ...
  if (bfstree.IsVertex(_endVid, &v1) && bfstree.IsVertex(_startVid, &v2)) {
    ...
    _path.insert(_path.begin(),
                 pair<typename GRAPH::VERTEX_TYPE, typename GRAPH::WEIGHT_TYPE>(
                     v1->data, wtt));
...
template <class GRAPH> class _visitorCC : public visitor_base<GRAPH> {
  VID v2id;

public:
  _visitorCC(GRAPH &g, VID _v2) { v2id = _v2; };
  inline int vertex(typename GRAPH::VI v) {
    if (v->vid == v2id)
      return EARLY_QUIT;
    else
 ...
 *@see BFS(VID)
 */
template <class GRAPH> bool IsSameCC(GRAPH &_g, VID _v1id, VID _v2id) {
  _visitorCC<GRAPH> vis(_g, _v2id);
  if (BFS_functor_EQ(_g, vis, _v1id) == EARLY_QUIT)
    return true;
  else
    return false;
}
 ...
 */
template <class GRAPH>
bool IsSameCC(GRAPH &_g, typename GRAPH::VERTEX_TYPE &_v1,
              typename GRAPH::VERTEX_TYPE &_v2) {
  typename GRAPH::CVI cv1, cv2;
  vector<char> color(_g.GetVertexCount(), 0);
  if (!_g.IsVertex(_v1, &cv1))
    return false;
  if (!_g.IsVertex(_v2, &cv2))
    return false;

  _visitorCC<GRAPH> vis(_g, cv2->vid);
  if (BFS_functor_EQ(_g, vis, cv1->vid) == EARLY_QUIT)
    return true;
  else
    return false;
}
...
public:
  _visitor_GetCC(GRAPH &g, CONTAINER *_v) { ccverts = _v; };
  inline int vertex(typename GRAPH::VI v) {
    ccverts->push_back(v->vid);
    return 1;
  }
 ...
 *@see BFS(VERTEX&)
 */
template <class GRAPH> int GetCC(GRAPH &_g, VID _v1id, vector<VID> &_ccverts) {
...
public:
  _visitor_GetCCVERTEX(GRAPH &g, CONTAINER *_v) { ccverts = _v; };
  inline int vertex(typename GRAPH::VI v) {
 ...
 */
template <class GRAPH>
int GetCC(GRAPH &_g, typename GRAPH::VERTEX_TYPE &_v1,
          vector<typename GRAPH::VERTEX_TYPE> &_ccverts) {
  typename GRAPH::CVI cv1;

  if (!_g.IsVertex(_v1, &cv1))
    return -1;
  _visitor_GetCCVERTEX<GRAPH, vector<typename GRAPH::VERTEX_TYPE>> _vis(
      _g, &_ccverts);
  BFS_functor(_g, _vis, cv1->vid);
  return _ccverts.size();
}
...
public:
  _visitor_GetCCEdges(GRAPH &g, CONTAINER *_v) { ccedges = _v; };
  inline int tree_edge(typename GRAPH::VI vi, typename GRAPH::EI ei) {
...
public:
  _visitor_GetCCEdgesWt(GRAPH g, CONTAINER *_v) { ccedges = _v; };
  inline int tree_edge(typename GRAPH::VI vi, typename GRAPH::EI ei) {
    pair<VID, VID> nextedge(vi->vid, ei->vertex2id);
    pair<pair<VID, VID>, typename GRAPH::WEIGHT_TYPE> nextedgewt(nextedge,
                                                                 ei->weight);
...
// bool _compare_pair_vid(pair<VID,VID> a, pair<VID,VID> b) {    return a.first<
// b.first;    }

template <class PVID> class _compare_pair_vid {
public:
  bool operator()(PVID a, PVID b) { return a.first < b.first; }
};
// TESTED
...
template <class PVID_WT> class _compare_pair_vid_weight {
public:
  bool operator()(PVID_WT a, PVID_WT b) {
...
/*
template<class GRAPH>
int GetCCEdges (GRAPH& _g, vector<pair<pair<VID,VID>,typename
GRAPH::WEIGHT_TYPE> >& _ccedges,  VID _v1id) {
  _visitor_GetCCEdgesWt<GRAPH,vector<pair<pair<VID,VID>,typename
GRAPH::WEIGHT_TYPE> > > _vis(_g, &_ccedges); BFS_functor(_g ,_vis , _v1id);
  _compare_pair_vid_weight<  pair<pair<VID,VID>,typename GRAPH::WEIGHT_TYPE>  >
_compare; stable_sort(_ccedges.begin(),_ccedges.end(),_compare ); return
_ccedges.size();
}
*/
 ...
 *vector<pair<VID,VID>> for unweighted or vector<pair<pair<VID,VID>,Weihght> for
 *weighted Graphs
 ...
 */
template <class GRAPH, class CONTAINER>
int GetCCEdges(GRAPH &_g, CONTAINER &_ccedges,
 ...
 */
template <class GRAPH, class CONTAINER>
int GetCCEdgesVData(GRAPH &_g, CONTAINER &ccedges, VID _v1id) {
  CONTAINER newedges;
 ...
 */
template <class GRAPH, class CONTAINER>
int GetCCEdgesVData(GRAPH &g, CONTAINER &ccedges,
                    typename GRAPH::VERTEX_TYPE _v1) {
 ...
 *this returned list.
 *@param _g the graph
 *@param ccedges 2D vector ccedges[i,j] = jth edge of ith CC, edge is VERTEX
 *pair.
...
int GetEdgesByCCVDataOnly(GRAPH &g, vector<CONTAINER> &ccedges) {
  ...
  // accept and return different containers depending on the weighted/unweighted
  // property also is not tested yet
...
//==================================
template <class T> struct __CCVID_Compare : public binary_function<T, T, bool> {
 ...
 *@see GetCCStats
 */
template <class GRAPH> int GetCCcount(GRAPH &_g) {
  vector<pair<int, VID>> ccstats;
 ...
 *@see GetCC(VID)
 */
template <class GRAPH> void DisplayCC(GRAPH &g, VID _v1id) {
 ...
 */
template <class GRAPH> void DisplayEdgesByCCVDataOnly(GRAPH &g) {

  vector<vector<pair<typename GRAPH::VERTEX_TYPE, typename GRAPH::VERTEX_TYPE>>>
      ccedges;
 ...
 */
template <class GRAPH> void DisplayCCStats(GRAPH &g, int _maxCCprint = -1) {
...
public:
  bool VertexValid; // Keeps track of whether the second vertex is valid or not
                    // In previous Graph.h we used INVALID_VID to do the same
                    // purpose

  VID vid; // This vertex ID
  // If the VertexValid flag is set to false then the next two variables does
  // not have any meaning and should not be used
  ...
  void print() const {
    cout << "Vertex : " << vid << "  Prev Vertex : " << predvid
         << "  Distance : " << dist << "  Valid ? " << VertexValid << endl;
  }
...
template <class T> struct less_cmp : public binary_function<T, T, bool> {
  bool operator()(T x, T y) { return x.second > y.second; }
};
 ...
 *@note _startV, _endV represent vertices id; This function works only for
 *weighted graphs.
 */
template <class GRAPH>
int FindPathDijkstra(
    GRAPH &_g, VID _v1id, VID _v2id,
    vector<pair<typename GRAPH::VERTEX_TYPE, typename GRAPH::WEIGHT_TYPE>>
        &_V) {
  ...
  if (!_g.IsVertex(_v1id, &cvi)) {
    cout << "ERROR while trying to retrieve the CVI in FindPathDijkstra"
         << endl;
    return -1;
  }
...
template <class T> struct __SCC_Compare : public binary_function<T, T, bool> {
  bool operator()(T x, T y) { return x.second > y.second; }
};

template <class GRAPH>
int dfs_pred(GRAPH &_g, map<VID, int> &_color, vector<vector<VID>> &_scc,
             VID _start, int scc_no) {
  ...
  for (typename GRAPH::EI ei = vi->predecessors.begin();
...
template <class GRAPH> int scc(GRAPH &_g, vector<vector<VID>> &_scc) {

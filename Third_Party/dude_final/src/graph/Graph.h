...
///////////////////////////////////////////////////////
/**Undirected Graph class. Here we declare methods that has to handle the fact
 *that the graph is undirected. for example AddEdge, DeleteEdge are different
 *from similar methods for Directed Graph.All these methods are inheritted by
 *Graph when the class is used.
 */
template <class VERTEX, class WEIGHT = int,
          class Base = BaseGraph<VERTEX, WEIGHT>>
...
public:
  ...
  ~UG() {}
  inline int check_directed() const { return 0; }
  ...
  inline int AddEdge(VERTEX &_v1, VERTEX &_v2, WEIGHT _weight) {
    if (Base::AddEdge(_v1, _v2, _weight) == ERROR)
      return ERROR;
  ...
  int AddEdge(VID _v1, VID _v2, pair<WEIGHT, WEIGHT> &_p) {
    if (Base::AddEdge(_v1, _v2, _p.first) == ERROR)
      return ERROR;
  ...
  int DeleteEdge(VID _v1id, VID _v2id, int _n = -1) {
    ...
    if (IsVertex(_v1id, &v1) && IsVertex(_v2id, &v2) &&
        this->IsEdge(_v1id, _v2id, &vi1, &ei)) {
  ...
  int DeleteWtEdge(VID _v1id, VID _v2id, WEIGHT _w, int _n = -1) {
    ...
    VI cv1;
    if (IsVertex(_v1id, &v1) && IsVertex(_v2id, &v2) &&
        this->IsEdge(_v1id, _v2id, _w, &cv1, &ei)) {
      int ok1 = v1->DeleteXEdges(_v2id, -1);
      int ok2 = v2->DeleteXEdges(_v1id, -1);
      if (ok1 == 1 && ok2 == 1) { // each should have found only 1
        this->numEdges--;
   ...
   *@return Degree if specified VID is found in graph. Otherwise ERROR will be
   *returned.
   */
   ...
   *@param _succ vector of VIDs which are the VIDs of those who are next to the
   *specified vertex.
   *@return the number of elements pushed in _succ
   *If this specified VID is not found, an empty list will be returned and error
   *message will be in standard output.
   *@see GetSuccessors
   */
   ...
   *specified vertex.
   *@return the number of elements pushed in _succ
   *If this specified VERTEX is not found, an empty list will be returned and
   *error message will be in standard output.
   *@see GetSuccessors
   */
  ...
  /**Get data for vertices which are adjacent to this specified vertex(user
   *data).
   *@param _v1id the vertex for which we want adjacent information
   *@param _succ vector of user data which is the data of those who are next to
   *the specified vertex.
   *@return the number of elements pushed in _succ
   *If the specified VID is not found, an empty list will be returned and error
   *message will be in standard output.
   *@see GetSuccessors
   */
   ...
   *the specified vertex.
   *@return the number of elements pushed in _succ
   *If the specified VERTEX is not found, an empty list will be returned and
   *error message will be in standard output.
   *@see GetSuccessors
   */
   ...
   */
  /*
  int GetIncidentEdgesVData(VID _v1id, vector< pair<VERTEX,VERTEX> >& iedges)
  const{ CVI v1; if ( IsVertex(_v1id,&v1) ) { iedges.clear(); iedges.reserve(
  v1->edgelist.size() ); for (CEI ei = v1->edgelist.begin(); ei !=
  v1->edgelist.end(); ei++) { pair<VERTEX,VERTEX> nextedge(
  this->GetData(_v1id), this->GetData(ei->vertex2id)); iedges.push_back(
  nextedge );
      }
    } else {
  ...
  */
  ...
  /**Get Edges which is incident to this specified vertex.
   *Incident edges are edges around this specified vertex.
   *@return a list of edges which is defined by 2 VIDs of its endpoints and the
   *edge weight. Empty list will be returned if this specified VID is not in
   *graph and error message will be in standard output.
   */
  int GetIncidentEdges(VID _v1id,
                       vector<pair<pair<VID, VID>, WEIGHT>> &iedges) const {
    CVI v1;
    if (IsVertex(_v1id, &v1)) {
      iedges.clear();
      iedges.reserve(v1->edgelist.size());
   ...
   *is not in graph and error message will be in standard output.
   */
  int GetIncidentEdgesVData(
      VID _v1id, vector<pair<pair<VERTEX, VERTEX>, WEIGHT>> &iedges) const {
    CVI v1;
    if (IsVertex(_v1id, &v1)) {
      iedges.clear();
      iedges.reserve(v1->edgelist.size());
...
/**This class encapsulates the methods related with the fact the graph is
 *directed. We have methods to handle the successors and predecessors.All these
 *methods are inheritted by Graph when the class is used.
 */
template <class VERTEX, class WEIGHT = int,
          class Base = BaseGraph<VERTEX, WEIGHT>>
...
public:
  ...
  ~DG() {}
  inline int check_directed() const { return 1; }
  ...
  int GetPredecessors(VI _vi, vector<VID> &_pred) const {
    ...
    for (CEI ei = _vi->predecessors.begin(); ei != _vi->predecessors.end();
         ei++) {
      _pred.push_back(ei->vertex2id);
    }
    return _pred.size();
...
///////////////////////////////////////////////////////
/**Weighted Graph class. Here we have methods that deal with the fact is
 *weighted or not. these methods are inheritted by Graph when used. The methods
 *are inheritted by graph.
 */
template <class VERTEX, class WEIGHT = int,
          class Base = BaseGraph<VERTEX, WEIGHT>>
...
public:
  /**Check if the graph is weighted or not. This function for this class will
   *return one alll the time. there is a similar function in NWG that will
   *return 0 all the time.
   */
  inline int check_weighted() const { return 1; }
   ...
   */
  bool IsEdge(VID _v1id, VID _v2id) { return (Base::IsEdge(_v1id, _v2id)); }
 ...
 *but the user doesn't have to be concerned with them. Also some methods related
 *with weight are not implemented in this class as opposed to WG.The methods are
 *inheritted by graph.
 */
template <class VERTEX, class WEIGHT = int,
          class Base = BaseGraph<VERTEX, WEIGHT>>
...
public:
  /**Check if the graph is weighted or not. This function for this class will
   *return zero all the time. There is a similar function in WG that will return
   *1 all the time.
   */
  inline int check_weighted() const { return 0; }
...
///////////////////////////////////////////////////////
/**Multiple Edge Graph class. Here for right now we have only chacks but in the
 *feature new methods related with multiplicity will be added. These methods are
 *inheritted by graph.
 */
template <class VERTEX, class WEIGHT = int,
          class Base = BaseGraph<VERTEX, WEIGHT>>
...
public:
  /**
   *return 1 all the time; the same function is implemented in NMG and there
   *returns zero all the time.
   */
  inline int check_multi() const { return 1; }
  /**
   *This function returns 1 all the time since the graph will allow multiple
   *edges.
   */
...
/**Non Multiple Edge Graph class. Here for right now we have only chacks but in
 *the feature new methods related with multiplicity will be added. These methods
 *are inheritted by graph.
 */
template <class VERTEX, class WEIGHT = int,
          class Base = BaseGraph<VERTEX, WEIGHT>>
...
public:
  ...
  /**
   *return 0 all the time; the same function is implemented in MG and there
   *returns one all the time.
   */
  inline int check_multi() const { return 0; }

  /**
   *This function check if the edge _v1->_v2 exist already in the graph and
   *returns zero if not and one if the edge is in the graph.
   */
 ...
 *NWG (methods related with weights disabled(missing); if the user will try to
 *use them for non-weighted graphs it will generate compiler errors);
 */
...
public:
   ...
   */
  VI begin() { return this->v.begin(); }
  /**
   *@return an interator that points immediately after the last vertex of the
   *graph.
   */
  VI end() { return this->v.end(); }

  /**
   *@return const iterator to the first vertex of the graph;
   */
  CVI begin() const { return this->v.begin(); }
  /**
   *@return an const interator that points immediately after the last vertex of
   *the graph.
   */
  CVI end() const { return this->v.end(); }

  /**
   *@return the id of the last node inserted in the graph +1
   */
  inline int getVertIDs() { return this->vertIDs; }
  /**
   *Set an internal variable such that when a new vertex is added it's id will
   *be _vids;
   */
  inline void setVertIDs(int _vids) { this->vertIDs = _vids; }
  /**
   *Check if the graph is directed or not;
   *@return 1 if the graph is directed zero if the graph is undirected;
   */
  inline int IsDirected() const { return D::check_directed(); }
  /**
   *Check if the graph is weighted or not;
   *@return 1 if the graph is weighted zero if the graph is unweighted;
   */
  inline int IsWeighted() const { return W::check_weighted(); }

  /**
   *Check if the graph support multiple edges or not;
   *@return 1 if the graph supports and zero if the graph doesn't support;
   */
  inline int IsMulti() const { return M::check_multi(); }
  /**
   *@return the number of vertices in the graph
   */
  inline int size() const { return this->v.size(); }
  /**
  ...
  /**
   *Add an edge into a graph. The functions takes care about directness,
   *multiplicity, weightness. If the graph is unweighted the user doesn't have
   ...
   *@note The weight of the edge doesn't haveto be specified if the graph is
   *unweighted.
  ...
  pair<VID, VID> Get_kth_Edge(int k) {
    ...
    if (i == n) {
      cout << " FATAL ERROR Get_kth_Edge(" << k << ") : Value of k[" << k
           << "] > Total number of edges[" << count << "]" << endl;
   ...
   *This functions has different functionality for Directed and Undirected
   *graphs. If the graph is directed this function will add two edges into the
   *graph. The first one is from _v1 to _v2 and has the weight _p.first and the
   *second one is from _v2 to _v1 and has weight _p.second. for the undirected
   *graphs this function will add one edge  from _v1 to _v2 but the weight
   *associated with this edge is different depending on the way you traverse the
   *edge.
   *@param _v1 the first vertex
   *@param _v2 the second vertex
  ...
  int AddEdge(VID _v1, VID _v2, pair<WEIGHT, WEIGHT> &_p) {
    ...
    if (M::check_edge(_v1, _v2) && M::check_edge(_v2, _v1))
      return D::AddEdge(_v1, _v2, _p);
   ...
   *@param WEIGHT the weight for all new edges. This doesn't have to be
   *specified for unweighted graphs.
  ...
  int AddPath(vector<VID> &_path, WEIGHT _wt = WEIGHT(-1)) {
    ...
    for (i = 0; i < _path.size(); i++) {
      if (!this->IsVertex(_path[i]))
        return ERROR;
    }
    for (i = 0; i < _path.size() - 1; i++) {
      if (AddEdge(_path[i], _path[i + 1], _wt) != ERROR)
        cnt++;
    }
    return cnt;
  }
  ...
  int AddPath(vector<VERTEX> &_path, WEIGHT _wt = WEIGHT(-1)) {
    int i;
    int cnt;
    if (!this->IsVertex(_path[0]))
      AddVertex(_path[0]);
    for (i = 0; i < _path.size() - 1; i++) {
      if (!this->IsVertex(_path[i + 1]))
        AddVertex(_path[i + 1]);
      if (AddEdge(_path[i], _path[i + 1], _wt) != ERROR)
        cnt++;
    }
  ...
  int AddPath( vector< pair<VID,WEIGHT> >& _path) {
    ...
    for (i = 0; i < _path.size() - 1; i++){
        if(AddEdge(_path[i].first,_path[i+1].first,_path[i].second)!=ERROR)
  cnt++;
    }
    return cnt;
  }
  ...
  */
  ...
  int AddPath(vector<pair<VERTEX, pair<WEIGHT, WEIGHT>>> &_path) {
    int i;
    int cnt = 0;
    if (!this->IsVertex(_path[0].first))
      AddVertex(_path[0].first);
    for (i = 0; i < _path.size() - 1; i++) {
      if (!this->IsVertex(_path[i + 1].first))
        AddVertex(_path[i + 1].first);
      if (AddEdge(_path[i].first, _path[i + 1].first, _path[i].second) != ERROR)
        cnt++;
    }
    return cnt;
  ...
  int GetEdges(vector<pair<VID, VID>> &edges) const {
    ...
    for (CVI vi = this->v.begin(); vi != this->v.end(); vi++) {
      for (CEI ei = vi->edgelist.begin(); ei != vi->edgelist.end(); ei++) {
        if (!(IsDirected()) && (vi->vid > ei->vertex2id))
          continue;
  ...
  /**Get All edges in this graph.
   *@param edges vector where the edges are stored
   *@return A edge list. One edge is defined as 2 VERTEX data structure(user
   *data).
   *@note This methods works both for weighted and unweighted graphs.
   */
  /*
  ...
  */
    ...
    for (CVI vi = this->v.begin(); vi != this->v.end(); vi++) {
      for (CEI ei = vi->edgelist.begin(); ei != vi->edgelist.end(); ei++) {
        if (!(IsDirected()) && (vi->vid > ei->vertex2id))
          continue;
        VERTEX v2data =
            ((BaseGraph<VERTEX, WEIGHT> *)(this))->GetData(ei->vertex2id);
  ...
  void ReadDotGraph(istream &_myistream) {
    ...
    while (v1id != -1) {
      ...
      if (AddEdge(v1id, v2id, weight) < 0) {
        cout << "Error while trying to insert edge " << v1id << " " << v2id
             << endl;
      }
      nEdges++;
      _myistream >> v1id;
    ...
    for (i = 0; i < nVerts; i++) {
      ...
      for (int j = 0; j < nedges; j++) {
        if (W::check_weighted()) {
          ...
          _myistream >> v2id; // read only the id and put a default weight
  ...
  void WriteGraph(ostream &_myostream) const {
    ...
    if (this->IsDirected())
      _myostream << endl
                 << this->numVerts << " " << this->numEdges << " "
                 << this->vertIDs;
    else
      _myostream << endl
                 << this->numVerts << " " << this->numEdges / 2 << " "
                 << this->vertIDs;

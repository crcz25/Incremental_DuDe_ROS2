...
bool c_ply::doInit() {
  ...
  if (this->area < 0 && type == POUT) {
    cerr << "! Warning: polygon type is POUT but has negative area. Reverse "
            "the vertex ordering."
         << endl;
    reverse();
    ...
    cerr << "! Warning: polygon type is PIN but has positive area. Reverse the "
            "vertex ordering."
         << endl;
    reverse();
...
void c_ply::re_triangulate() { triangulation.clear(); }
// check if convex
...
bool c_polygon::valid() // check if this is a valid polygon
{
  ...
  if (empty())
    return false;
  if (front().getType() != c_ply::POUT)
    return false;
  for (iterator i = ++begin(); i != end(); i++)
    if (i->getType() != c_ply::PIN)
      return false;
...
list<c_polygon> c_polygon::split() {
  ...
  for (iterator i = begin(); i != end(); i++) {
    if (i->getType() == c_ply::POUT) {
      ...
      if (sub.empty()) {
        cerr << "! Error: Invalid polygon type: Holes are defined without "
                "external boundary."
             << endl;

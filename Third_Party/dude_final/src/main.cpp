...
int dual_main(int argc, char ** argv)
{
        ...
        vector<c_polygon*> finalPolygonPieces; //the result polygons, they can
be exported as individual files decompose(finalPolygonPieces);

        double end_dude = getTime();
        //---------------------------------------------------------------

        cerr << "- Compute Dual-space Decomposition of "<<dude_param.Pfile<<"
with tau "<<dude_param.concavity_tau<<" takes " << end_dude - start_dude
                        << " ms" << endl;
...
void save_cut() {
  ...
  for (vector<c_diagonal>::iterator dit = dude.m_cuts.begin();
       dit != dude.m_cuts.end(); ++dit) {
    c_diagonal &diag = *dit;
...
void loda_cuts(vector<c_polygon *> &finalPolygonPieces) {
  ...
  if (dude_param.decomposeIteratively) {
    ...
    iterativeDecompose(*p1, dude.m_cuts, dude_param.concavity_tau,
                       finalPolygonPieces, draw_decoration.allAccumulatedCuts,
                       draw_decoration.se, true);
  } else {
    decomposeMoreTimes(*p1, dude.m_cuts, dude_param.concavity_tau,
                       finalPolygonPieces, draw_decoration.allAccumulatedCuts,
                       draw_decoration.se, true, 0);
  }
...
void export_files(vector<c_polygon *> &finalPolygonPieces) {
  ...
  if (draw_options.outputSegFile)
    saveDiagonals(draw_decoration.allAccumulatedCuts,
                  dude_param.Pfile + ".seg.dude2d");
...
void export_individual_svg_files(vector<c_polygon *> &finalPolygonPieces) {
  ...
  svg::Polygon original(svg::Fill(svg::Color::Yellow),
                        svg::Stroke(stroke_width / 2, svg::Color::Black));
  P.toSVG(original);
  ...
  for (int i = 0; i < psize; i++) {
    ...
    svg::Polygon p_i(svg::Fill(svg::Color::Lime),

...
c_dude::~c_dude() {
  //*
  for (list<c_BPC *>::iterator iter = m_bpcs.begin(); iter != m_bpcs.end();
       iter++) {
    c_BPC *current = (*iter);
    delete current;
  }
  ...
  ////////////
  /*
          for( list<ply_vertex *>::iterator iter = m_PMs.begin(); iter !=
  m_PMs.end(); iter++){ ply_vertex * current = (*iter); delete current;
          }
      m_PMs.clear();
          ////////////
          for( list<ply_vertex *>::iterator iter = hole_PMs.begin(); iter !=
  hole_PMs.end(); iter++){ ply_vertex * current = (*iter); delete current;
          }
          hole_PMs.clear();
          ////////////
          for( list<ply_vertex *>::iterator iter = holeFeatPnts.begin(); iter !=
  holeFeatPnts.end(); iter++){ ply_vertex * current = (*iter); delete current;
          }
          holeFeatPnts.clear();
          ////////////
          for( list<ply_vertex *>::iterator iter = convexFeatPnts.begin(); iter
  != convexFeatPnts.end(); iter++){ ply_vertex * current = (*iter); delete
  current;
          }
          convexFeatPnts.clear();
          ////////////
          for( list<ply_vertex *>::iterator iter = poutFeatPnts.begin(); iter !=
  poutFeatPnts.end(); iter++){ ply_vertex * current = (*iter); delete current;
          }
          poutFeatPnts.clear();
          ////////////
          for( list<ply_vertex *>::iterator iter = totalFeatPnts.begin(); iter
  != totalFeatPnts.end(); iter++){ ply_vertex * current = (*iter); delete
  current;
          }
          totalFeatPnts.clear();
          ////////////
          for( list<ply_vertex *>::iterator iter = combine_PMs.begin(); iter !=
  combine_PMs.end(); iter++){ ply_vertex * current = (*iter); delete current;
          }
          combine_PMs.clear();
          ////////////
          for( vector<ply_vertex *>::iterator iter = bpcStartPnts.begin(); iter
  != bpcStartPnts.end(); iter++){ ply_vertex * current = (*iter); delete
  current;
          }
          bpcStartPnts.clear();
          ////////////
          for( vector<ply_vertex *>::iterator iter = bpcEndPnts.begin(); iter !=
  bpcEndPnts.end(); iter++){ ply_vertex * current = (*iter); delete current;
          }
          bpcEndPnts.clear();
...
void c_dude::build(c_polygon &P, double tau, bool isInitPolygon) {
  ...
  //     if(P[i]->getExtra().concavity_bpc)
  //     cout<<P[i]->getExtra().concavity_bpc<<endl;
  //    }
  ...
  // cout<<"total hole featured point number is
  // "<<this->holeFeatPnts.size()<<endl;
...
void c_dude::combineMinimumPnts(list<ply_vertex *> &extraPMs,
                                list<ply_vertex *> &holePMs,
                                list<ply_vertex *> &totalPMs) {
...
void c_dude::collectOutConvexFeatPnts(c_ply &plyOut,
                                      list<ply_vertex *> &tmpconvexFeatPnts) {
  ...
  // map points from plyline to initial ply
  {
    ...
    for (list<ply_vertex *>::iterator i = vList1.begin(); i != vList1.end();
         ++i, j = j->getNext()) {
      vmap1[j] = *i;
    }
  }
  // map points from plyline to initial ply
  {
    ...
    for (list<ply_vertex *>::iterator i = vList2.begin(); i != vList2.end();
         ++i, j = j->getNext()) {
      vmap2[j] = *i;
    }
  }
  ...
  tmpconvexFeatPnts.push_back(m1);
  tmpconvexFeatPnts.push_back(m2);
  ...
  for (list<ply_vertex *>::iterator vlIte = approx1.begin();
       vlIte != approx1.end(); ++vlIte) {
    ply_vertex *tmpPnt = vmap1[*vlIte];
    // assert(tmpPnt != m1->getPre() && tmpPnt != m2->getPre() && tmpPnt !=
    // m1->getNext() && tmpPnt != m2->getNext());
  ...
  for (list<ply_vertex *>::iterator vlIte = approx2.begin();
       vlIte != approx2.end(); ++vlIte) {
...
void c_dude::collectConvexFeatPnts(c_polygon &P,
                                   list<ply_vertex *> &boundFeatPnts) {
  bpcStartPnts.clear();
  bpcEndPnts.clear();
  ...
  for (c_polygon::iterator pit = P.begin(); pit != P.end(); pit++) {
    ...
    do {
      ...
      if (tVert->getExtra().flag != tempFlag) {
        if (!splitTag) {
          ...
          if (vidx != -1) // find a start point of a pocket, use as the
                          // splitting point of the list
          {
            splitTag = true;
          }
    ...
    if ((vList1.size() + vList2.size() == (uint)vertSize) &&
        (extrBound.getType() == c_ply::POUT)) // the ply is convex
    {
    ...
    while (curIt != vList.end()) {
      // use the start points and end points to split the line into several
      // parts, and approximate each part individually
      ...
      if (tempList.size() > 2) {
        ...
        // map from tmp (store in "tmp") to the original polygon
        {
          ...
          for (list<ply_vertex *>::iterator i = tempList.begin();
               i != tempList.end(); i++) {
            vmap[j] = *i;
            j = j->getNext();
          }
...
void c_dude::collectListFeatPnts(list<ply_vertex *> &vlist,
                                 list<ply_vertex *> &resultVList) {
  list<ply_vertex *> approx;
  map<ply_vertex *, ply_vertex *> vmap, vmap2;
  ...
  // map from hullply to tmp (store in "hull")
  {
    ...
    for (list<ply_vertex *>::iterator i = vlist.begin(); i != vlist.end();
         i++) {
      vmap2[j] = *i;
      j = j->getNext();
    }
  ...
  // mark the approximation
  for (list<ply_vertex *>::iterator ait = approx.begin(); ait != approx.end();
       ++ait) {
...
//
//	for(list<ply_vertex*>::iterator pite =
//totalFeatPnts.begin();pite!=totalFeatPnts.end();++pite)
//	{
...
////
////				//build the map between from vertices and to
///vertices /				new_v->getExtra().other_v=ptr; /
...
void c_dude::create_simplified_and_reduced(c_polygon &P) {
  ...
  for (list<ply_vertex *>::iterator pite = totalFeatPnts.begin();
       pite != totalFeatPnts.end(); ++pite) {
...
void c_dude::decompose_hole(c_ply &hole, list<ply_vertex *> &tmpholePMs,
                            list<ply_vertex *> &tmpholeFeatPnts,
                            list<c_BPC *> &bpcs) {
  ...
  if (holecuts.size() == 0 /*||dude.m_bpcs.size()==0*/) {
    ...
    double maxConcavity =
        determineConcavityForConvexHole(hole, m1, m2, furthestV);
    ...
    tmpholePMs.push_back(m1);
    tmpholePMs.push_back(m2);
    tmpholeFeatPnts.push_back(m1);
    tmpholeFeatPnts.push_back(m2);

    int approxVNum = 2;

    //		//put the approximate points into the approximate vertex list
    //		for (list<ply_vertex*>::iterator
    //vlIte=approx1.begin();vlIte!=approx1.end();++vlIte)
    //		{
    //			ply_vertex* tmpPnt = vmap1[*vlIte];
    //					//assert(tmpPnt != m1->getPre() &&
    //tmpPnt != m2->getPre() && tmpPnt != m1->getNext() && tmpPnt !=
    //m2->getNext()); 					if(tmpPnt==m1||tmpPnt==m2) 						continue;
    //					//update the concavity for this
    //approximate point 					tmpPnt->getExtra().concavity =
    //distToSeg(m1->getPos(),m2->getPos(),tmpPnt->getPos());
    //
    ...
    //				}
    //				for (list<ply_vertex*>::iterator
    //vlIte=approx2.begin();vlIte!=approx2.end();++vlIte)
    //				{
    //					ply_vertex* tmpPnt = vmap2[*vlIte];
    //					//assert(tmpPnt != m1->getPre() &&
    //tmpPnt != m2->getPre() && tmpPnt != m1->getNext() && tmpPnt !=
    ...
    draw_decoration.holecutRoot.setEndPoints(rootcutTmp->startPnt,
                                             rootcutTmp->endPnt);

    // just to record the cut diagonals
    for (vector<c_diagonal>::const_iterator dit = holecuts.begin();
         dit != holecuts.end(); ++dit) {
    ...
    findFeatAndConcavityForComplexHole(hole, tmpcutPoints, tmpholePMs,
                                       tmpholeFeatPnts, m_tau, rootcutTmp);
  }
}

void c_dude::findFeatAndConcavityForComplexHole(
    c_ply &hole, vector<ply_vertex *> &cutPnts, list<ply_vertex *> &tmpHolePMs,
    list<ply_vertex *> &tmpHoleFeatPnts, double tau,
    HoleDiagonal *rootHoleCut) {
  ...
  collectFeatPntBetweenCuts(hole, cutPnts, tmpHolePMs, tmpHoleFeatPnts, tau,
                            holeDiagVec, REDUCEPOCKETMINIMUM);

  for (vector<ply_vertex *>::iterator vit = cutPnts.begin();
       vit != cutPnts.end(); ++vit) {
    ply_vertex *tv = *vit;
    ...
    if (tv->isReflex()) {
      tv->getExtra().concavity =
          tv->getExtra().holeCutConcavity; // distToSeg(prePM->getPos(),
                                           // nextPM->getPos(), tv->getPos());//
                                           // + min(prePM->getExtra().concavity,
                                           // nextPM->getExtra().concavity);
  ...
  if (rootReflexV != NULL) {
    ...
    float tmpDist = dist2Seg(preReflex->getPos(), nextReflex->getPos(),
                             rootNonReflex->getPos());
    rootReflexV->getExtra().concavity =
        distance2Point(rootReflexV->getPos(), rootNonReflex->getPos()) +
        tmpDist;
...
void c_dude::decompose_bridge(c_BPC *bpc) {
  ...
  if (ply.getHead()->getPos() != bpc->getSource2()->getPos())
    assert(false);
  ...
  for (vector<c_diagonal>::const_iterator j = cuts.begin(); j != cuts.end();
       j++) {
...
void c_dude::addDiagonal(ply_vertex *s, ply_vertex *t, c_polygon &polygon) {
  ply_vertex *v1 =
      (polygon[0] == m_simpilifed_P[0]) ? s->getExtra().other_v : s;
  ply_vertex *v2 =
      (polygon[0] == m_simpilifed_P[0]) ? t->getExtra().other_v : t;
  // assert(v1 && v2);

  c_diagonal diagonal(v1, v2);

  //    if(v1->getExtra().parentPly==v2->getExtra().parentPly &&
  //    v1->getExtra().parentPly->getType()==c_ply::PIN)
  //    {
  // #if DEBUG
...
bool c_dude::isDiagonal(ply_vertex *s, ply_vertex *t, c_polygon &polygon,
  ...
  if (!isHoleDiagonalValid(
          v1, v2)) // check if both of two end points of this diagonal is in
                   // hole and they are approximated by a single line
    return false;

  if (is_bridge_breaking(v1, v2) == false)
    return false;
  ...
  if (polygon.enclosed(mid) == false)
    return false;

  if (initPolygon.enclosed(mid) == false)
    return false;
...
bool c_dude::is_bridge_breaking(const ply_vertex *v, const ply_vertex *u) {
  ...
  if (bpc1 == NULL && bpc2 == NULL && hole1 == NULL && hole2 == NULL)
    return false; // no bridge to break...

  if ((bpc1 == bpc2) && (hole1 == hole2))
    return false; // same bridge or same hole...
...
void c_dude::collectOutBoundFeatPnts() {
  ...
  for (BIT i = m_bpcs.begin(); i != m_bpcs.end(); i++) {
    ...
    for (list<ply_vertex *>::const_iterator j = bes.begin(); j != bes.end();
         j++) {
      outFeatPntSet.insert(*j);
    }
  } // end i

  for (list<ply_vertex *>::iterator pite = convexFeatPnts.begin();
       pite != convexFeatPnts.end(); ++pite) {
    outFeatPntSet.insert(*pite);
  }

  // make sure that there is no duplicates
  poutFeatPnts.insert(poutFeatPnts.end(), outFeatPntSet.begin(),
                      outFeatPntSet.end());
}
void c_dude::collectTotalFeatPnts() {
  // totalFeatPnts.clear();
  set<ply_vertex *> totalFeatPntSet;
  for (list<ply_vertex *>::iterator pite = poutFeatPnts.begin();
       pite != poutFeatPnts.end(); ++pite) {
    totalFeatPntSet.insert(*pite);
  }

  for (list<ply_vertex *>::iterator pite = holeFeatPnts.begin();
       pite != holeFeatPnts.end(); ++pite) {
    totalFeatPntSet.insert(*pite);
  }

  // make sure that there is no duplicates
  totalFeatPnts.insert(totalFeatPnts.end(), totalFeatPntSet.begin(),
                       totalFeatPntSet.end());
}
// build cutset
void c_dude::buildCutSets(list<ply_vertex *> &pms) {
  ...
  for (IT i = pms.begin(); i != pms.end(); i++) {
    ...
    c_cutset::build_cutsets(v, start, end, extra.diagonals, m_tau,
                            extra.cutsets);

    // remember
    m_cutsets.insert(m_cutsets.end(), extra.cutsets.begin(),
                     extra.cutsets.end());
...
void c_dude::resolveIntersectedCuts(c_polygon &tmpPolygon) {
  for (vector<c_diagonal>::iterator dit = m_cuts.begin(); dit != m_cuts.end();
    ...
    double s[2], e[2];
    s[0] = tdig.getV1()->getPos()[0];
    s[1] = tdig.getV1()->getPos()[1];
    e[0] = tdig.getV2()->getPos()[0];
    e[1] = tdig.getV2()->getPos()[1];

    for (c_polygon::iterator plyit = tmpPolygon.begin();
         plyit != tmpPolygon.end(); ++plyit) {
      c_ply &tply = *plyit;
      ...
      do {
        ...
        cur[0] = pcur->getPos()[0];
        cur[1] = pcur->getPos()[1];
        next[0] = pnext->getPos()[0];
        next[1] = pnext->getPos()[1];

        char intersectCode = SegSegInt(s, e, cur, next, inter);
        Point2d interPos(inter[0], inter[1]);
        // char intersectCode = SegSegInt(tdig.getV1()->getPos(),
        // tdig.getV2()->getPos(), pcur->getPos(), pnext->getPos(), interPos);
        if (intersectCode != 'e' && intersectCode != '0') {
          if (distance2Point(tdig.getV1()->getPos(), interPos) >= SMALLZERO &&
              distance2Point(tdig.getV2()->getPos(), interPos) >= SMALLZERO) {
    ...
    vector<pair<ply_vertex *, ply_vertex *>>::iterator svitEdge =
        intersectionEdges.begin();
    vector<pair<ply_vertex *, ply_vertex *>>::iterator evitEdge =
        intersectionEdges.begin();
    ...
    vector<pair<ply_vertex *, ply_vertex *>>::iterator edgeit =
        intersectionEdges.begin();
    for (; vit != intersectionPositions.end() &&
           edgeit != intersectionEdges.end();
         ++vit, ++edgeit) {
      Point2d &tp = *vit;
    ...
    if (distance2Point(tdig.getV1()->getPos(), sInterOtherV->getPos()) >
        distance2Point(tdig.getV1()->getPos(), sOtherEdge.second->getPos()))
      sInterOtherV = sOtherEdge.second;
    if (distance2Point(tdig.getV2()->getPos(), eInterOtherV->getPos()) >
        distance2Point(tdig.getV2()->getPos(), eOtherEdge.second->getPos()))
    ...
    //		if(tdig.getV1()->getNext() == tdig.getV2() ||
    //tdig.getV2()->getNext() == tdig.getV1())
    //		{
  ...
  for (vector<c_diagonal>::iterator dit = m_cuts.begin();
       dit != m_cuts.end();) {
    c_diagonal &tdig = *dit;
    if (tdig.getV1()->getNext() == tdig.getV2() ||
        tdig.getV2()->getNext() == tdig.getV1()) {
    ...
    for (; cdit != m_cuts.end(); ++cdit) {
      ...
      if ((tdig.getV1() == otherDiag.getV2() &&
           tdig.getV2() == otherDiag.getV1()) ||
          (tdig.getV1() == otherDiag.getV1() &&
           tdig.getV2() == otherDiag.getV2())) {
        dit = m_cuts.erase(dit);
        isRedudant = true;
        break;
      }
...
void c_dude::clear() {
  ...
  convexFeatPnts.clear(); // to record the features points on the convex of the
                          // extra-boundary
  poutFeatPnts.clear(); // to record the feature points of the whole
                        // extra-boundary, that is to combine the pocket
                        // features points and convex feature points
  totalFeatPnts.clear(); // all the feature points of this polygon
  combine_PMs
      .clear(); // combine the extra-boundary and hole boundary feature points
  // holeAxisPnts.clear();

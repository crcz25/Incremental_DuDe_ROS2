...
c_polygon *cutHole(c_diagonal &cut_l, vector<c_diagonal> &allCuts,
  ...
  p1.set(c_ply::UNKNOWN, v1);
  p2.set(c_ply::UNKNOWN, v2);
  ...
  for (; dIte != allCuts.end(); ++dIte) {
    ...
    Point2d midPnt((s->getPos()[0] + e->getPos()[0]) / 2,
                   (s->getPos()[1] + e->getPos()[1]) / 2);
...
#if debug
// double check
void test(c_ply *p1, c_ply *p2, vector<c_diagonal> &allCuts,
          vector<c_diagonal>::iterator tmpIte) {
...
////////////////////////////////////////////////////////////////////////////////
// the new cut polys
bool cutPolys_new(pair<c_polygon *, c_polygon *> &polygons, c_diagonal &cut_l,
                  vector<c_diagonal> &allCuts,
                  vector<c_diagonal>::iterator tmpIte, bool &p1hasmorecut,
                  bool &p2hasmorecut) {
  ply_vertex *v1 = cut_l.getV1();
  ply_vertex *v2 = cut_l.getV2();
  ply_vertex *v1n = v1->getNext();
  ...
  for (; dIte != allCuts.end(); ++dIte) {
    ...
    if (tmpDiag.getV1() == v2) {
      ...
      //			if
      //(SegSegInt(v2->getPos().get(),otherVert->getPos().get(),
    ...
    assert(tmpDiag.getV1()->getExtra().parentPolygon ==
           tmpDiag.getV2()->getExtra().parentPolygon);
  }
...
bool cutPolys(pair<c_polygon *, c_polygon *> &polygons, c_diagonal &cut_l,
              vector<c_diagonal> &allCuts, vector<c_diagonal>::iterator tmpIte,
              bool &p1hasmorecut, bool &p2hasmorecut) {
  // the basic information of the cut
  ply_vertex *v1 = cut_l.getV1();
  ply_vertex *v2 = cut_l.getV2();
  ...
  for (; dIte != allCuts.end(); ++dIte) {
    ...
    if (prePolygon_s == initPolygon || prePolygon_s == polygons.first ||
        prePolygon_s == polygons.second)
      assert(prePolygon_e == initPolygon || prePolygon_e == polygons.first ||
             prePolygon_e == polygons.second);

    if (prePolygon_e == initPolygon || prePolygon_e == polygons.first ||
        prePolygon_e == polygons.second)
      assert(prePolygon_s == initPolygon || prePolygon_s == polygons.first ||
             prePolygon_s == polygons.second);

    //		bool b1 = checkContain(*prePolygon_s, s->getExtra().parentPly);
    //		bool b2 = checkContain(*prePolygon_e, e->getExtra().parentPly);
    //		bool b3 = checkContain(*polygons.first,
    //s->getExtra().parentPly); 		bool b3_2 = checkContain(*polygons.first,
    //e->getExtra().parentPly); 		bool b4 = checkContain(*polygons.second,
    //s->getExtra().parentPly); 		bool b4_2 = checkContain(*polygons.second,
    //e->getExtra().parentPly);
    ...
    if (e->getExtra().parentPolygon != initPolygon &&
        e->getExtra().parentPolygon != polygons.first &&
        e->getExtra().parentPolygon != polygons.second) {
      assert(s->getExtra().parentPolygon != initPolygon &&
             s->getExtra().parentPolygon != polygons.first &&
             s->getExtra().parentPolygon != polygons.second);
      continue;
    }
    ...
    if (tmpDiag.getV1() == v1) {
      ...
      } else if (s->getExtra().parentPly != pply1 &&
                 s->getExtra().parentPly != pply2 &&
                 new2old.find(s->getExtra().parentPly) == new2old.end()) {
    ...
    if (tmpDiag.getV2() == v1) {
      ...
      } else if (e->getExtra().parentPly != pply1 &&
                 e->getExtra().parentPly != pply2 &&
                 new2old.find(e->getExtra().parentPly) == new2old.end()) {
    ...
    if (tmpDiag.getV1()->getExtra().parentPly->getType() == c_ply::POUT &&
        tmpDiag.getV2()->getExtra().parentPly->getType() == c_ply::POUT)
      assert(tmpDiag.getV1()->getExtra().parentPly ==
             tmpDiag.getV2()->getExtra().parentPly);
    assert(tmpDiag.getV1()->getExtra().parentPolygon ==
           tmpDiag.getV2()->getExtra().parentPolygon);
    ...
    // bool bb1 = checkContain(*tmpDiag.getV1()->getExtra().parentPolygon,
    // tmpDiag.getV1()->getExtra().parentPly); bool bb2 =
    // checkContain(*tmpDiag.getV2()->getExtra().parentPolygon,
    // tmpDiag.getV2()->getExtra().parentPly);
...
void merge2Holes(c_diagonal &cut, vector<c_diagonal> &allCuts,
                 vector<c_diagonal>::iterator tmpIte, c_ply *result_ply,
                 bool &p1hasmorecut, bool &p2hasmorecut) {
  ply_vertex *v1, *v2;
  v1 = cut.getV1();
  v2 = cut.getV2();

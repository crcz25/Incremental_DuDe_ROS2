...
struct HoleDiagonal {
  ...
  void clear() {
    ...
    initParent = NULL;
    tempParent = NULL;
    for (vector<HoleDiagonal *>::iterator dit = childHoleDiags.begin();
         dit != childHoleDiags.end(); ++dit) {
...
void collectFeatPntBetweenCuts(c_ply &hole, vector<ply_vertex *> &cuts,
                               ...
                               bool reducePMs = false);

// check if both of two end points of this diagonal is in hole and they are

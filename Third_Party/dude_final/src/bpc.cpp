...
//
// build: determine the order of bridge vertices and compute the length of
// pocket
//
...
bool c_BPC::build_for_hole(ply_vertex *s, ply_vertex *e, bool ordered) {
  ...
  if (ordered == false) {
    ...
    if (area1 > 0 || area2 > 0)
      return false; // should be both negative...
...
void c_BPC::getConvexApproximate(list<ply_vertex *> &approxList) {
  ...
  while (curIt != vList.end()) {
    // use the start points and end points to split the line into several parts,
    // and approximate each part individually
    list<ply_vertex *> tempList;
    int tmpEndIdx = checkEndPoint(*curIt);
    int tmpStartIdx = checkStartPoint(*curIt);
    if (tmpStartIdx !=
        -1) // if the current point is a start point, skip this kid pocket
      ++curIt;
    if (curIt == vList.end())
      break;
    ...
    if (tempList.size() > 2) {
      ...
      /////
      // Here mapping the convex hull points to the points on the converted
      // polyline
      //
      {
        ply_vertex *j = hullply.getHead();
        for (list<ply_vertex *>::iterator i = hull.begin(); i != hull.end();
             i++) {
          vmap2[j] = *i;
          j = j->getNext();
        }
      ...
      for (list<ply_vertex *>::iterator pit = tempList.begin();
           pit != tempList.end(); ++pit) {
...
void c_BPC::determineConcavity_real(float tau, bool reducePMs) {
  ...
  for (IT i = approx.begin(); i != approx.end(); i++) {
    // for(list<ply_vertex*>::iterator i =
    // approxList.begin();i!=approxList.end();++i)
    //{
    ...
    if (v == m_source[0] || v == m_source[1])
      continue; // cannot be end point of the bridge
    bool is_bridge = false;
    for (list<c_BPC *>::iterator k = m_kids.begin(); k != m_kids.end(); k++) {
      c_BPC *kid = *k;
      if (kid->getConcavities().empty())
        continue;
    ...
    // commented by Guilin
    if (is_bridge)
      continue; // cannot be kids' bridge
  ...
  for (IT i = features.begin(); i != features.end(); i++) {
    ...
    if (c_bpc == NULL)
      continue; // not PM (bridge end points)
    if (c_bpc != this) {
...
void c_BPC::determine_PM(float tau) {
  ...
  for (IT i = tmp.begin(); i != tmp.end(); i++) {
    ...
    float concavity = dist2Bridge(
        v->getPos()); // dist_in_hierarchy(extra.concavity_bpc,v->getPos());

    // float concavity=this->dist2Bridge(v->getPos());
    if (concavity < tau)
      continue;

    //        cout<<"["<<v->getVID()<<"], concavity="<<concavity<<"
    //        bpc="<<this<<endl;
...
float c_BPC::dist_in_hierarchy(c_BPC *bpc, const Point2d &p) {
  if (bpc->m_parent == NULL) {
    ...
    Point2d new_p =
        proj2Seg(bpc->m_source[0]->getPos(), bpc->m_source[1]->getPos(), p);
...
char c_BPC::checkBridge(ply_vertex *p1, ply_vertex *p2) {
  if (p1->getNext() == p2 && (p1->isReflex() || p2->isReflex()))
    return 'b';
  if (p2->getNext() == p1 && (p1->isReflex() || p2->isReflex()))
    return 'r';

  float area = bridgeArea(p1, p2);

  char code = 'b';
  if (area <= 1e-10 && area >= -1e-10)
    code = '0';
  else {
    if (area > 0)
      code = 'r';
...
void c_BPC::getVerticesExcludingKidPockets_f(list<ply_vertex *> &vlist) {
  ...
  for (list<c_BPC *>::iterator i = m_kids.begin(); i != m_kids.end(); i++) {
    ...
    for (ply_vertex *k = kid->m_source[0]->getNext(); k != kid->m_source[1];
         k = k->getNext()) {
...
void c_BPC::getSortedFeatures_f(list<ply_vertex *> &PMs,
                                list<ply_vertex *> &features) {
  uint flag = ply_vertex_extra::getFlagID();
  for (list<c_BPC *>::iterator k = m_kids.begin(); k != m_kids.end(); k++) {
    c_BPC *kid = *k;
    kid->m_source[0]->getExtra().flag = kid->m_source[1]->getExtra().flag =
        flag;
  }
  ...
  for (ply_vertex *ptr = m_source[0]; ptr != m_source[1];
       ptr = ptr->getNext()) {
...
void c_BPC::reorganize_kids_f() {
  ...
  for (ply_vertex *i = m_source[0]; i != m_source[1]; i = i->getNext())
    i->getExtra().flag =
        count++; // dangerous way of using flag since the value is not from
                 // ply_vertex_extra::getFlagID need to reset after use
  ...
  for (ply_vertex *i = m_source[0]; i != m_source[1]->getNext();
       i = i->getNext())
    i->getExtra().flag = 0;
  m_source[1]->getExtra().flag = 0;
}
...
void c_BPC::insert2tree(c_BPC *other) {
  ...
  for (IT i = m_kids.begin(); i != m_kids.end(); i++) {
    ...
    if (kid_flag1 <= other_flag0 || kid_flag0 >= other_flag1)
      continue; // no overlap
    if (kid_flag0 <= other_flag0 &&
        kid_flag1 >= other_flag1) { // kid contains other
    ...
    //        else{
    //
    //        	cerr<<other->m_source[0]->getPos()[0]<<",
    //        "<<other->m_source[0]->getPos()[1]<<endl;
    //        	cerr<<other->m_source[1]->getPos()[0]<<",
    //        "<<other->m_source[1]->getPos()[1]<<endl;
    //
    //        	cerr<<kid->m_source[0]->getPos()[0]<<",
    //        "<<kid->m_source[0]->getPos()[1]<<endl;
    //        	cerr<<kid->m_source[1]->getPos()[0]<<",
    //        "<<kid->m_source[1]->getPos()[1]<<endl;
    //
    //        	bool b1 = (other->m_source[0]->getPos() ==
    //        kid->m_source[0]->getPos());
    //
    //        	assert(false);
    //
    //        }
...
bool c_BPC::isKid(c_BPC *other) {
  ...
  for (ply_vertex *ptr = m_source[0]; ptr != m_source[1];
       ptr = ptr->getNext()) {
    if (ptr == other->m_source[0])
      s1_found = true;
    if (ptr == other->m_source[1])
      s2_found = true;
  }

  if (m_source[1] == other->m_source[0])
    s1_found = true;
  if (m_source[1] == other->m_source[1])
    s2_found = true;

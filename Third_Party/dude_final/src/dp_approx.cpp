...
void dpReduction(ply_vertex *stPnt, ply_vertex *endPnt, double tolerance,
  ...
  while (pcursor != endPnt) {
    double tmpDist =
        distToSeg(stPnt->getPos(), endPnt->getPos(), pcursor->getPos());
...
void dp_approximate(const c_plyline &plyline, double tau,
...
// startV would be able to get endV by using getNext(), the approx doesn't
// include startV and endV
void dp_approximate(ply_vertex *startV, ply_vertex *endV, double tau,
                    list<ply_vertex *> &approx) {
  approx.clear();
  //	ply_vertex* stPnt = plyline.getHead();
  //	ply_vertex* endPnt = plyline.getTail();
...
void approximateMedthod(const c_plyline &plyline, double tau,
                        list<ply_vertex *> &approx, bool bUseDpApprox) {
  if (!bUseDpApprox)

...
void collectCutPoints(const vector<c_diagonal> &cuts,
                      vector<ply_vertex *> &collectList) {
  collectList.clear();
...
int computeHierarchyDepth(HoleDiagonal *root) {
  if (root->childHoleDiags.size() == 0)
    return 0;
  int depth = 0;
  for (vector<HoleDiagonal *>::iterator dit = root->childHoleDiags.begin();
       dit != root->childHoleDiags.end(); ++dit) {
    if (*dit == NULL)
      continue;
...
void buildHoleDiagonalHierarchy(HoleDiagonal *root,
                                const vector<c_diagonal> &holecuts) {
  ...
  for (vector<c_diagonal>::const_iterator j = holecuts.begin();
       j != holecuts.end(); j++) {
    const c_diagonal &dia = *j;
    // always make sure start point has smaller flag id than end point
    ply_vertex *stPnt =
        dia.getV1()->getExtra().flag < dia.getV2()->getExtra().flag
            ? dia.getV1()
            : dia.getV2();
    ply_vertex *endPnt =
        dia.getV1()->getExtra().flag < dia.getV2()->getExtra().flag
            ? dia.getV2()
            : dia.getV1();
    if ((stPnt == root->startPnt) &&
        (endPnt == root->endPnt)) // if this is the root cut, continue;
      continue;
...
void splitCutsToTwo(ply_vertex *s, ply_vertex *e,
                    const vector<c_diagonal> &holecuts,
                    vector<c_diagonal> &left, vector<c_diagonal> &right) {
  for (vector<c_diagonal>::const_iterator dit = holecuts.begin();
       dit != holecuts.end(); ++dit) {
    const c_diagonal &diag = *dit;
    if ((diag.getV1() == s && diag.getV2() == e) ||
        (diag.getV1() == e && diag.getV2() == s))
      continue;

    if (diag.getV1()->getExtra().flag <= e->getExtra().flag &&
        diag.getV2()->getExtra().flag <= e->getExtra().flag) {
      left.push_back(diag);
    } else {
...
HoleDiagonal *getCutDiagRoot(const vector<c_diagonal> &holecuts) {
  ...
  if (holecuts.size() == 1)
    return (
        new HoleDiagonal(holecuts.front().getV1(), holecuts.front().getV2()));
  ...
  for (vector<c_diagonal>::const_iterator dit = holecuts.begin();
    ...
    root1->childHoleDiags.insert(root1->childHoleDiags.end(),
                                 root2->childHoleDiags.begin(),
                                 root2->childHoleDiags.end());
...
bool checkParentChild(HoleDiagonal *parent, HoleDiagonal *child) {
  for (vector<HoleDiagonal *>::iterator hit = parent->childHoleDiags.begin();
       hit != parent->childHoleDiags.end(); ++hit) {
    if ((*hit) == child)
      return true;
  }

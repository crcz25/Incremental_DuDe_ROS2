...
void draw_PS_PolygonPieces(PSDoc *ps,
                           const vector<c_polygon *> &polygonPieces) {
  vector<c_polygon *>::const_iterator pit = polygonPieces.begin();
  for (; pit != polygonPieces.end(); ++pit) {
    // const c_polygon* & tmpPolygon = *pit;

    for (c_polygon::const_iterator i = (*pit)->begin(); i != (*pit)->end();
         i++) {
...
void draw_PS_Skeleton(PSDoc *ps, const SE2d &se2d) {
  ...
  for (MIT im = se2d.getTodoList().begin(); im != se2d.getTodoList().end();
       im++) {
...
inline void draw_PS_Bridge(PSDoc *ps, c_BPC *bpc) {
  // draw bridge
  draw_PS_line(ps, bpc->getSource1()->getPos().get(),
               bpc->getSource2()->getPos().get());
...
void draw_PS_BPC(PSDoc *ps, c_BPC *bpc, float largest_concavity,
                 float smallest_concavity) {
  ...
  // draw concavity
  if (draw_options.showConcavity && !draw_options.showPolygonPieceForPS &&
      !draw_options.showSkeletonForPS) {
    PS_setcolor(ps, "fill", "rgb", 1, 0, 0, 0.0);
    PS_setcolor(ps, "stroke", "gray", 0.0, 0.0, 0.0, 0.0);
    const list<ply_vertex *> &pms = bpc->getConcavities();
    for (list<ply_vertex *>::const_iterator i = pms.begin(); i != pms.end();
         i++)
...
void drawALL(PSDoc *ps) {
  ...
  if (draw_options.showConcavity && !draw_options.showPolygonPieceForPS &&
    ...
    //        i=holefp.begin();i!=holefp.end();i++)
    //        {
    //            if( (*i)->getExtra().isPM()==false ||
    //            (*i)->getExtra().diagonals.empty() ) continue;
    //            draw_PS_circle(ps,(*i)->getPos(),scale_avg);
    //        }
  }
}
...
void save2PS(const string &name) {
  PS_page_scale =
      PS_page_width * 1.0f / (draw_decoration.box[1] - draw_decoration.box[0]);
  PSDoc *ps = NULL;
  PS_boot();
  ps = PS_new();

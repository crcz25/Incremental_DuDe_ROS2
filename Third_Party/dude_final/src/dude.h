...
public:
  ...
  bool isDiagonal(/*const*/ ply_vertex *s, /*const*/ ply_vertex *t,
                  c_polygon &polygon, c_polygon &initPolygon);
...
protected:
  ...
  void decompose_hole(c_ply &hole, list<ply_vertex *> &holePMPnts,
                      list<ply_vertex *> &holeFeatPnt, list<c_BPC *> &bpcs);
...
public:
  ...
  //    map<const ply_vertex*, ply_vertex*> simp2input; //mapping vertices in
  //    simplified polygon to that of input polygon
  ...
  vector<c_cutset>
      m_cutsets; // all cutsets generated from bpcs in build function
  ...
  // added by Guilin
  list<ply_vertex *> hole_PMs; // to record the pocket minimum points of the
                               // hole
  list<ply_vertex *> holeFeatPnts;   // to record the feature points in the hole
  list<ply_vertex *> convexFeatPnts; // to record the features points on the
                                     // convex of the extra-boundary
  list<ply_vertex *>
      poutFeatPnts; // to record the feature points of the whole extra-boundary,
                    // that is to combine the pocket features points and convex
                    // feature points
  list<ply_vertex *> totalFeatPnts; // all the feature points of this polygon
  list<ply_vertex *> combine_PMs;   // combine the extra-boundary and hole
  ...
  void combineMinimumPnts(list<ply_vertex *> &extraPMs,
  ...
  // collect the convex outboundary feature points, the outboundary is (nearly)
  // already
  void collectOutConvexFeatPnts(c_ply &plyOut,
                                list<ply_vertex *> &convexFeatPnts);

  // collect the features points in a list
  void collectListFeatPnts(list<ply_vertex *> &vlist,
                           list<ply_vertex *> &resVList);

  uint getDudeFlag() { return TMPFLAG++; }

  // find the feature points and concavity for complex hole
  void findFeatAndConcavityForComplexHole(c_ply &hole,
                                          vector<ply_vertex *> &cutPnts,

...
inline void createVIP_vertex_list() {
  ...
  for (list<ply_vertex *>::const_iterator i = holefp.begin(); i != holefp.end();
       i++) {
    if ((*i)->getExtra().isPM() == false || (*i)->getExtra().diagonals.empty())
      continue;
...
void selectVIPV(int tmp) {
  if (draw_decoration.selectedVIPVertex < 0 ||
      draw_decoration.selectedVIPVertex >
          (int)(draw_decoration.VIP_vertices.size()))
    draw_decoration.g_selected_PM = NULL;
  else
    draw_decoration.g_selected_PM =
        draw_decoration.VIP_vertices[draw_decoration.selectedVIPVertex];
}
...
void printUsage(char *name) {
  ...
  //		<< " to compare the segmenation with a segmentation from
  //file\n";
...
void printGUIUsage() {
  ...
  cout << left << setw(offset) << "C:"
       << "show all potential cuts one by one,increase the index by pressing "
          "I\n";
  cout << left << setw(offset) << "S:"
       << "show all simplified polygons one by one,increase the index by "
          "pressing I\n";
  ...
  cout << left << setw(offset) << "R:"
       << "show all the generated rest plys excluding child pockets one by "
          "one,\n"
       << left << setw(offset) << " "
       << "increase the index by pressing I\n";
  cout << left << setw(offset) << "H:"
       << "show all the generated parent pocket and its children pockets one "
          "by one, \n"
       << left << setw(offset) << " "
       << "increase the index by pressing I\n";
...
void estimate_COM_R_Box() {
  ...
  draw_decoration.COM.set((draw_decoration.box[1] + draw_decoration.box[0]) / 2,
                          (draw_decoration.box[3] + draw_decoration.box[2]) /
                              2);
  draw_decoration.R = sqr(draw_decoration.COM[0] - draw_decoration.box[0]) +
                      sqr(draw_decoration.COM[1] - draw_decoration.box[2]);
...
void DuDe() {
  ...
  cout << "- Dual-space Decomposition with tau=" << dude_param.concavity_tau
       << endl;
  double start_dude = getTime();
  dude = c_dude();
  dude.build(getP(), dude_param.concavity_tau, true);
  draw_decoration.allFeaturePMs.insert(draw_decoration.allFeaturePMs.end(),
                                       dude.m_PMs.begin(), dude.m_PMs.end());
  ...
  // cout<<"- Compute Dual-space Decomposition takes "<<end_dude-start_dude<<"
  // ms"<<endl; glutPostRedisplay();
}
...
void createGLUI() {
  ...
  editTau = glui->add_edittext_to_panel(top_panel,
                                        "concavity tau:", GLUI_EDITTEXT_FLOAT);
  ...
  GLUI_Button *buttonRun =
      glui->add_button_to_panel(top_panel, "Run", -1, (GLUI_Update_CB)DuDe);
  buttonRun->set_name("Decompose Again");

  glui->add_checkbox_to_panel(top_panel, "Show Polygon (p)",
                              &draw_options.showP, -1, (GLUI_Update_CB)Display)
      ->set_int_val(draw_options.showP);
  glui->add_checkbox_to_panel(top_panel, "Show Concavity (c)",
                              &draw_options.showConcavity, -1,
                              (GLUI_Update_CB)Display)
      ->set_int_val(draw_options.showConcavity);
  glui->add_checkbox_to_panel(top_panel, "Show Bridges (b)",
                              &draw_options.showBridges, -1,
                              (GLUI_Update_CB)Display)
      ->set_int_val(draw_options.showBridges);
  glui->add_checkbox_to_panel(top_panel, "Show Decomposition (d)",
                              &draw_options.showDecompose, -1,
                              (GLUI_Update_CB)Display)
      ->set_int_val(draw_options.showDecompose);
  glui->add_checkbox_to_panel(top_panel, "Show Constraints (G)",
                              &draw_options.showConstraints, -1,
                              (GLUI_Update_CB)Display)
      ->set_int_val(draw_options.showConstraints);
  glui->add_checkbox_to_panel(top_panel, "Show Potential Cuts (w)",
                              &draw_options.showCuts, -1,
                              (GLUI_Update_CB)Display)
      ->set_int_val(draw_options.showCuts);
  // glui->add_checkbox_to_panel(top_panel,"Show All Cuts (C)", &showAllCuts,
  // -1, (GLUI_Update_CB)Display)->set_int_val(showAllCuts);
  // glui->add_checkbox_to_panel(top_panel,"Show All SimPolygons (S)",
  // &showAllSimPolygons, -1,
  // (GLUI_Update_CB)Display)->set_int_val(showAllSimPolygons);
  glui->add_checkbox_to_panel(top_panel, "Show Normal directions (n)",
                              &draw_options.showNormal, -1,
                              (GLUI_Update_CB)Display)
      ->set_int_val(draw_options.showNormal);
  glui->add_checkbox_to_panel(top_panel, "Show Polygon Pieces (Q)",
                              &draw_options.showPolygonPieces, -1,
                              (GLUI_Update_CB)Display)
      ->set_int_val(draw_options.showPolygonPieces);
  glui->add_checkbox_to_panel(top_panel, "Show Circle with Diameter tau",
                              &draw_options.showTauCircle, -1,
                              (GLUI_Update_CB)Display)
      ->set_int_val(draw_options.showTauCircle);
  glui->add_checkbox_to_panel(top_panel, "Show Vertex ID",
                              &draw_options.showVertexID, -1,
                              (GLUI_Update_CB)Display)
      ->set_int_val(draw_options.showVertexID);

  glui->add_checkbox_to_panel(top_panel, "Dump PDF files (9)",
                              &draw_options.savePDF, -1,
                              (GLUI_Update_CB)Display)
      ->set_int_val(draw_options.savePDF);
  glui->add_checkbox_to_panel(top_panel, "Dump PPM files (0)",
                              &draw_options.saveImg, -1,
                              (GLUI_Update_CB)Display)
      ->set_int_val(draw_options.saveImg);

  ///////////////////////////////////////////////////////////////
  glui->add_checkbox_to_panel(top_panel, "Show few (d)",
                              &draw_options.showFewCut, -1,
                              (GLUI_Update_CB)Display)
      ->set_int_val(draw_options.showFewCut);
...
void save_PDF(const string &pdf_name) {
  ...
  gl2psBeginPage("mksum 2d", "MASC Group@GMU", viewport,
                 ...
                 GL2PS_SILENT | GL2PS_POLYGON_OFFSET_FILL | GL2PS_NO_BLENDING |
...
void Display(void) {
  ...
  glOrtho((draw_decoration.box[0] - draw_decoration.COM[0]) * scale +
              draw_decoration.COM[0],
          (draw_decoration.box[1] - draw_decoration.COM[0]) * scale +
              draw_decoration.COM[0],
          (draw_decoration.box[2] - draw_decoration.COM[1]) * scale +
              draw_decoration.COM[1],
          (draw_decoration.box[3] - draw_decoration.COM[1]) * scale +
              draw_decoration.COM[1],
          -1000, 1000);
...
void Reshape(int w, int h) {
  ...
  glMatrixMode(GL_PROJECTION);
  glLoadIdentity();
  glOrtho(draw_decoration.box[0], draw_decoration.box[1],
          draw_decoration.box[2], draw_decoration.box[3], -1000, 1000);
  glMatrixMode(GL_MODELVIEW);
  glLoadIdentity();
}
...
void Keyboard(unsigned char key, int x, int y) {
  ...
  case 'G':
    ...
    break;
  case 'L':
    draw_options.showAllUserCutsInClustering =
        !draw_options.showAllUserCutsInClustering;
    break;
  case 'U':
  ...
  case '?':
    if (draw_decoration.g_selected_PM == NULL)
      ...
      vector<c_cutset> &csets =
          draw_decoration.g_selected_PM->getExtra().cutsets;
      uint cset_size = csets.size();
      cout << "PM[" << draw_decoration.g_selected_PM->getVID() << "] has "
           << cset_size << " cutsets, concavity="
           << draw_decoration.g_selected_PM->getExtra().concavity << endl;
      for (uint j = 0; j < cset_size; j++) {
        cout << csets[j] << endl;
      }
      cout << "\t dihedral Pre Vid="
           << draw_decoration.g_selected_PM->getExtra()
                  .getDihedralPre()
                  ->getVID()
           << " dihedral Next Vid="
           << draw_decoration.g_selected_PM->getExtra()
...
void Mouse(int button, int state, int x, int y) {
  ...
  if (state == GLUT_UP && glutGetModifiers() == GLUT_ACTIVE_CTRL) {
    ...
    double scaled_box[4] = {draw_decoration.box[0], draw_decoration.box[1],
                            draw_decoration.box[2], draw_decoration.box[3]};
    scaled_box[0] = (draw_decoration.box[0] - draw_decoration.COM[0]) * scale +
                    draw_decoration.COM[0];
    scaled_box[1] = (draw_decoration.box[1] - draw_decoration.COM[0]) * scale +
                    draw_decoration.COM[0];
    scaled_box[2] = (draw_decoration.box[2] - draw_decoration.COM[1]) * scale +
                    draw_decoration.COM[1];
    scaled_box[3] = (draw_decoration.box[3] - draw_decoration.COM[1]) * scale +
                    draw_decoration.COM[1];

    for (vector<ply_vertex *>::const_iterator i =
             draw_decoration.VIP_vertices.begin();

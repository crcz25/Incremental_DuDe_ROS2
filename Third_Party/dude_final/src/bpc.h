...
public:
  ...
  //    void collectKids(); //collect kids of this BPC by going through the
  //    pocket vertices

  bool build_for_hole(ply_vertex *s, ply_vertex *e, bool ordered = false);

  ////build for hole, check dude.decompose_hole() function for detailed
  ///description
  ...
  const c_BPC *getRoot() const {
    if (m_parent == NULL)
      return this;
    return m_parent->getRoot();
  }
  //    c_BPC * getLeaf(ply_vertex * v); //get a leaf bpc that has the given
  //    vertex as concavity
  int getPocketSize() const { return m_pocketsize; }

  void setNext(c_BPC *bpc) {
    m_next = bpc;
    if (bpc != NULL)
      bpc->m_pre = this;
  }
...
protected:
  ...
  void getSortedFeatures_f(list<ply_vertex *> &PMs,

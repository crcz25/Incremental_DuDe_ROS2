...
c_dude_use::c_dude_use() { todo_list.clear(); }
void c_dude_use::destory() { todo_list.clear(); }
...
void updateCut(c_diagonal &cut, vector<c_diagonal> &allCuts,
               vector<c_diagonal>::iterator tmpIte, c_ply *srcPly,
               c_ply *dstPly) {
...
void c_dude_use::decomposePolygonUsingCuts(c_polygon &cpolygon,
                                           vector<c_diagonal> &allCuts,
                                           SE2d &se, bool bExtractSkeleton) {
  ...
  for (vector<c_diagonal>::iterator dit = allCuts.begin(); dit != allCuts.end();
       ++dit) {
    ...
    if (v1->getExtra().parentPly->getType() == c_ply::POUT &&
      ...
      // bool b =
      // cutPolys(subPolygons,*dit,allCuts,dit,p1hasmorecut,p2hasmorecut);
      bool b = cutPolys_new(subPolygons, *dit, allCuts, dit, p1hasmorecut,
                            p2hasmorecut);
      ...
      for (list<se_m *>::iterator sit = se.todo_list.begin();
           sit != se.todo_list.end(); ++sit) {
        if ((*sit) == otherM) {
      ...
      se_m *m =
          se.ply2MouthMap[parentPlygon2]; // getSEMFromTodolist(*initPlygon);

      merge2Holes(*dit, allCuts, dit, ply1, p1hasmorecut, p2hasmorecut);

      /*********************************************/
      // remove the redundant ply
      for (c_polygon::iterator pit = parentPlygon.begin();
           pit != parentPlygon.end(); ++pit) {
        if (ply2 == (&(*pit))) {
      ...
      // se_m* m = se.ply2MouthMap[parentPlygon1];
      // //getSEMFromTodolist(*initPlygon);
      se_m *m = NULL;
      if (bExtractSkeleton)
      ...
      if (parentPlygon1 != parentPlygon2)
        cerr
            << "warning! the cut's two end points belong two different polygons"
            << endl;
      bool p1hasmorecut = false;
      bool p2hasmorecut = false;
    ...
    // else if(v)
      ...
      // cerr<<"warning! the cut might be not correct! (both from same
      // ply)"<<endl;
      c_polygon &parentPlygon = *(v1->getExtra().parentPolygon);
      c_ply *ply2 = v2->getExtra().parentPly;
      c_ply *ply1 = v1->getExtra().parentPly;
      ...
      updateCut(*dit, allCuts, dit, v2->getExtra().parentPly,
                v1->getExtra().parentPly);
  ...
  for (vector<c_polygon *>::iterator pit = todo_list.begin();
       pit != todo_list.end();) {
...
void updateMINMAXConcavity(c_dude &dude2d) {
  ...
  for (list<ply_vertex *>::const_iterator j = holefp.begin(); j != holefp.end();
       j++) {
    float c = (*j)->getExtra().concavity;
    draw_decoration.allFeatureConcavities.push_back(c);
    if ((*j)->getExtra().isPM() == false || (*j)->getExtra().diagonals.empty())
      continue;
    cerr << c << endl;
  ...
  for (list<ply_vertex *>::const_iterator j = holefp2.begin();
       j != holefp2.end(); j++) {
    float c = (*j)->getExtra().concavity;
    draw_decoration.allFeatureConcavities.push_back(c);
    if ((*j)->getExtra().isPM() == false || (*j)->getExtra().diagonals.empty())
...
void extractSkeletonFromDiagonals(c_polygon &initPolygon,
                                  vector<c_diagonal> &initCuts,
                                  vector<c_polygon *> &finalPolygonPieces,
                                  SE2d &se, bool bExtractSkeleton) {
  /*************************************************/
  ...
  c_dude_use initUse;
  initUse.decomposePolygonUsingCuts(initPolygon, initCuts, se,
                                    bExtractSkeleton);
  finalPolygonPieces.insert(finalPolygonPieces.end(), initUse.todo_list.begin(),
                            initUse.todo_list.end());

  if (bExtractSkeleton)
    se.UpdateSkeleton();
}

void iterativeDecompose(c_polygon &initPolygon, vector<c_diagonal> &initCuts,
                        double tau, vector<c_polygon *> &finalPolygonPieces,
                        vector<c_diagonal> &finalAllCuts, SE2d &se,
                        bool bExtractSkeleton) {
  float tempTau = tau;
  /*************************************************/
  // initialize for extracting skeletons
  if (bExtractSkeleton) {
  ...
  pTodolist.insert(pTodolist.end(), initUse.todo_list.begin(),
                   initUse.todo_list.end());
  ...
  while (!pTodolist.empty()) {
    ...
    for (vector<c_polygon *>::iterator pvit = olist.begin();
         pvit != olist.end(); ++pvit) {
      ...
      draw_decoration.allFeaturePMs.insert(draw_decoration.allFeaturePMs.end(),
                                           ...
                                           tmpDude.m_PMs.end());
      draw_decoration.allFeaturePMs.insert(draw_decoration.allFeaturePMs.end(),
                                           tmpDude.hole_PMs.begin(),
                                           tmpDude.hole_PMs.end());
      updateMINMAXConcavity(tmpDude);

      // record the cuts
      if (tmpDude.m_cuts.size() > 0) {
        finalAllCuts.insert(finalAllCuts.end(), tmpDude.m_cuts.begin(),
                            tmpDude.m_cuts.end());

        // record  the cutted polygon pieces
        c_dude_use tmpUse;
        tmpUse.decomposePolygonUsingCuts(*tmpPolygon, tmpDude.m_cuts, se,
                                         bExtractSkeleton);

        pTodolist.insert(pTodolist.end(), tmpUse.todo_list.begin(),
                         tmpUse.todo_list.end());
...
void decomposeMoreTimes(c_polygon &initPolygon, vector<c_diagonal> &initCuts,
                        double tau, vector<c_polygon *> &finalPolygonPieces,
                        vector<c_diagonal> &finalAllCuts, SE2d &se,
                        bool bExtractSkeleton, int iterateTimes) {
  /*************************************************/
  // initialize for extracting skeletons
  if (bExtractSkeleton) {
    se_m *initM = new se_m(&initPolygon);
  ...
  for (int i = 0; i < iterateTimes; i++) {
    ...
    tmpTodolist.insert(tmpTodolist.end(), finalPolygonPieces.begin(),
                       finalPolygonPieces.end());
    finalPolygonPieces.clear();

    int k = 0;
    for (vector<c_polygon *>::iterator pvit = tmpTodolist.begin();
         pvit != tmpTodolist.end(); ++pvit, ++k) {
      c_polygon *tmpPolygon = *pvit;
      ...
      if (tmpDude.m_cuts.size() > 0) {
        ...
        tmpUse.decomposePolygonUsingCuts(*tmpPolygon, tmpDude.m_cuts, se,
                                         bExtractSkeleton);
        finalPolygonPieces.insert(finalPolygonPieces.end(),
                                  tmpUse.todo_list.begin(),
                                  tmpUse.todo_list.end());

...
c_diagonal::c_diagonal() { c_diagonal(NULL, NULL); }
...
bool c_diagonal::is_angle_resolving(ply_vertex *v, double tau) const {
  if (v->getExtra().isPM() == false)
    return true; // any diagonal will resolve non pocket minimum
...
// check if this diagonal can resolve vertex v by determining the diagonal
// angles and residual concavity
bool c_diagonal::is_resolving(ply_vertex *v, double tau_concavity,
                              double tau_angle) const {
  if (v->getExtra().isPM() == false)
    return true; // any diagonal will resolve non pocket minimum
  ...
  if (angle1 < tau_angle && angle2 < tau_angle)
    return true;

  if (angle1 >= tau_angle) {
    double rc = dist2Seg(v1, other, v0);
    if (rc > tau_concavity)
      return false; // residual concavity is too large
  }
...
bool c_diagonal::is_resolving(const c_diagonal &od, ply_vertex *v,
                              double tau_concavity, double tau_angle) const {
  if (v->getExtra().isPM() == false)
    return true; // any diagonal will resolve non pocket minimum
  ...
  if (angle1 < tau_angle && angle2 < tau_angle && angle3 < tau_angle)
    return true;
...
double c_diagonal::angleAt(
    const Point2d &p1, const Point2d &p2,
    const Point2d &p3) const { // to compute the inner angle at p2
...
bool c_diagonal::operator==(const c_diagonal &other) const {
  if (m_v[0] == other.m_v[0] && m_v[1] == other.m_v[1])
    return true;
  if (m_v[0] == other.m_v[1] && m_v[1] == other.m_v[0])
    return true;
  return false;
}

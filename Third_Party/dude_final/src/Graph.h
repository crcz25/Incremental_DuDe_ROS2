...
public:
  ...
  /**Adding a predecessor edge by given id for the first endpoint and the weight
   *of this new edge.
   */
  ...
  /**Check if any edge having this given vid and return the address of this
   *edge. return true if this kind of edge exists in edgelist.
   ...
   *@return WtEdge which has this ID
   */
  WtEdge *my_find_EID2_eq(const WtEdge *_start, const WtEdge *_end,
  ...
  vector<WtEdge> predecessors; ///< call WeightedMultiDiGraph::SetPredecessors()
                               ///< to initialize
...
template <class VERTEX> class AbstractGraph {
...
public:
   ...
   *
   *@param vector< pair<VID, pair<WEIGHT,WEIGHT> > >& a list of vids and
   *weights.
   *@note 1 will be returned if one of vid in the path
   *could not be found in the graph.
   *@return 0 if every thing is fine.
   ...
   *vector[2].second.first,vector[2].second.second..
   *
   *@param vector< pair<VERTEX, pair<WEIGHT,WEIGHT> > >& a list of user data and
   *weights.
   *@note if vi is not found in graph then a new vertex will be added.
   *@return Always 0.
   */
   ...
   *@note if _vid is not found in this graph, then a empty vector will be
   *returned, and an error message will be output to standard output.
   */
   ...
   *Source vertex is the vertex that has no incoming edges (i.e. no
   *predecessors) This method returns all sources in the graph. Need to call
   *SetPredecessors() for the graph before using this method
   */
  ...
  /**Find BFS path between 2 specified vertices. Return the path as a
   *vector<VID>
   *@see FindPathBFS(VID,VID) and IsVertex(VERTEX&)
   */
   ...
   *This tree starts from VID and to all vertices which are reachable from this
   *vertex.
   */
  ...
  typedef typename VERTEX_VECTOR::const_iterator
      CVI; ///< CVI Constant Vertex Iterator
  typedef typename VERTEX_VECTOR::reverse_iterator
      RVI; ///< RVI Reverse Vertex Iterator
  typedef typename VERTEX_VECTOR::const_reverse_iterator
      CRVI; ///< CRVI Constant Reverse Vertex Iterator

  typedef vector<WtEdge> WtEdge_VECTOR;
  typedef typename WtEdge_VECTOR::iterator EI; ///< EI Edge Iterator
  typedef typename WtEdge_VECTOR::const_iterator
      CEI; ///< CEI Constant Edge Iterator
  typedef typename WtEdge_VECTOR::reverse_iterator
      REI; ///< REI Reverse Edge Iterator
  typedef typename WtEdge_VECTOR::const_reverse_iterator
      CREI; ///< CREI Constant Reverse Edge Iterator
   ...
   *there is no edge from v1 to v2.
   */
  bool IsEdge(VERTEX &, VERTEX &, WEIGHT, const Vertex **,
              const WtEdge **) const;
  ...
  public:
    dkinfo() {}
    dkinfo(VID _vid, VID _pvid, double _d) {
      vid = _vid;
      predvid = _pvid;
      dist = _d;
    }
...
public:
   ...
   */
  virtual vector<pair<pair<VERTEX, VERTEX>, WEIGHT>>
  GetEdgesVData() const; // vertex information

  /**Get Degree of this specified Vertex.
   *@note For undirected graph, outgoing degree is the same as incoming degree.
   *@retrun Degree if specified VID is found in graph. Otherwise 1 will be
   *returned.
   ...
   *@return a list of VID which are VIDs of those who are next to the specified
   *vertex. If this specified VID is not found, an empty list will be returned
   *and error message will be in standard output.
   *@see GetSuccessors
   */
  ...
  /**Get Edges which is incident to this specified vertex and user data
   *associated with these edges. Incident edges are edges around this specified
   *vertex.
   ...
   */
  virtual vector<pair<pair<VERTEX, VERTEX>, WEIGHT>>
      GetIncidentEdgesVData(VID) const;
   ...
   */
#ifdef __HP_aCC
  static bool CCVID_Compare(const pair<int, VID> _cc1,
                            const pair<int, VID> _cc2);
#else
  static bool CCVID_Compare(const pair<int, VID> &_cc1,
                            const pair<int, VID> &_cc2);
#endif
...
//==================================
template <class VERTEX> AbstractGraph<VERTEX>::AbstractGraph() {
  vertIDs = numVerts = numEdges = 0;
  reserveEdgesPerVertex = 0;
}

template <class VERTEX>
AbstractGraph<VERTEX>::AbstractGraph(int _reserveEdgesPerVertex) {
  vertIDs = numVerts = numEdges = 0;
...
template <class VERTEX> int AbstractGraph<VERTEX>::GetVertexCount() const {
  return numVerts;
}

template <class VERTEX> int AbstractGraph<VERTEX>::GetEdgeCount() const {
  return numEdges;
}

template <class VERTEX> VID AbstractGraph<VERTEX>::GetNextVID() const {
  return vertIDs;
}
...
template <class VERTEX> void AbstractGraph<VERTEX>::SetStartID(VID _startvid) {
  vertIDs = _startvid;
}

template <class VERTEX> void AbstractGraph<VERTEX>::SetnumVerts(int _num) {
  numVerts = _num;
}

template <class VERTEX> void AbstractGraph<VERTEX>::SetnumEdges(int _num) {
  numEdges = _num;
}
...
template <class VERTEX, class WEIGHT>
WeightedMultiDiGraph<VERTEX, WEIGHT>::WeightedMultiDiGraph() {}

template <class VERTEX, class WEIGHT>
WeightedMultiDiGraph<VERTEX, WEIGHT>::WeightedMultiDiGraph(int _sz) {
  v.reserve(_sz);
}
...
template <class VERTEX, class WEIGHT>
VID WeightedMultiDiGraph<VERTEX, WEIGHT>::AddVertex(VERTEX &_v) {
...
template <class VERTEX, class WEIGHT>
VID WeightedMultiDiGraph<VERTEX, WEIGHT>::AddVertex(vector<VERTEX> &_v) {
...
template <class VERTEX, class WEIGHT>
VID WeightedMultiDiGraph<VERTEX, WEIGHT>::AddVertex(VERTEX &_v, VID _vid) {
  VID vid = _vid;
  Vertex newVertex(_v, vid, this->reserveEdgesPerVertex);
  v.push_back(newVertex);
  this->numVerts++;
...
template <class VERTEX, class WEIGHT>
int WeightedMultiDiGraph<VERTEX, WEIGHT>::DeleteVertex(VERTEX &_v1) {
  CVI cv1;
  VI v1;
  if (IsVertex(_v1, &cv1)) {
...
template <class VERTEX, class WEIGHT>
int WeightedMultiDiGraph<VERTEX, WEIGHT>::DeleteVertex(VID _v1id) {
  CVI cv1;
  VI v1;
...
template <class VERTEX, class WEIGHT>
int WeightedMultiDiGraph<VERTEX, WEIGHT>::EraseGraph() {
  while (v.size() != 0)
    v.pop_back();
...
template <class VERTEX, class WEIGHT>
void WeightedMultiDiGraph<VERTEX, WEIGHT>::PutData(VID _vid,
                                                   VERTEX _v) { // lkd: 7-7-99
  CVI cv1;
  VI v1;
...
template <class VERTEX, class WEIGHT>
void WeightedMultiDiGraph<VERTEX, WEIGHT>::SetPredecessors() {
  VI v1, v2;
  CVI cv2;
  VID _v2id;
  bool DoneSet = false;
...
template <class VERTEX, class WEIGHT>
int WeightedMultiDiGraph<VERTEX, WEIGHT>::AddEdge(VID _v1id, EI _ei) {
  VI v1 /*, v2*/;
  CVI cv1, cv2;
...
template <class VERTEX, class WEIGHT>
int WeightedMultiDiGraph<VERTEX, WEIGHT>::AddEdge(VID _v1id, VID _v2id,
                                                  WEIGHT _weight) {
  CVI cv1;
  VI v1;
  if (IsVertex(_v1id, &cv1) && IsVertex(_v2id)) {
    ...
    cout << endl
         << "AddEdge: v1 " << _v1id << " and/or v2 " << _v2id << "not in graph";
    return 1;
  }
}

template <class VERTEX, class WEIGHT>
int WeightedMultiDiGraph<VERTEX, WEIGHT>::AddEdge(
    VID _v1id, VID _v2id, pair<WEIGHT, WEIGHT> _wtpair) {
  CVI cv1, cv2;
  VI v1, v2;
...
template <class VERTEX, class WEIGHT>
int WeightedMultiDiGraph<VERTEX, WEIGHT>::AddEdge(VERTEX &_v1, VERTEX &_v2,
                                                  WEIGHT _weight) {
  CVI cv1, cv2;
  VI v1, v2;
...
template <class VERTEX, class WEIGHT>
int WeightedMultiDiGraph<VERTEX, WEIGHT>::AddEdge(
    VERTEX &_v1, VERTEX &_v2, pair<WEIGHT, WEIGHT> _wtpair) {
  CVI cv1, cv2;
  VI v1, v2;
  if (IsVertex(_v1, &cv1) && IsVertex(_v2, &cv2)) {
    v1 = const_cast<VI>(cv1);
...
template <class VERTEX, class WEIGHT>
int WeightedMultiDiGraph<VERTEX, WEIGHT>::AddPredecessorEdge(VID _v1id,
                                                             VID _v0id,
                                                             WEIGHT _weight) {
  CVI cv1;
  VI v1;
  if (IsVertex(_v1id, &cv1) && IsVertex(_v0id)) {
    ...
    cout << endl
         << "AddPredecessorEdge: v1 " << _v1id << " and/or v0 " << _v0id
         << "not in graph";
    return 1;
  }
}
...
template <class VERTEX, class WEIGHT>
void WeightedMultiDiGraph<VERTEX, WEIGHT>::DeleteAllEdgesToV(VID _v2id) {
  CVI v2;
...
template <class VERTEX, class WEIGHT>
void WeightedMultiDiGraph<VERTEX, WEIGHT>::DeleteAllEdgesFromV(VID _v1id) {
  CVI cv1;
  VI v1;
  if (IsVertex(_v1id, &cv1)) {
    v1 = const_cast<VI>(cv1);
...
template <class VERTEX, class WEIGHT>
void WeightedMultiDiGraph<VERTEX, WEIGHT>::DeleteAllEdges(VID _vid) {
...
template <class VERTEX, class WEIGHT>
int WeightedMultiDiGraph<VERTEX, WEIGHT>::DeleteEdge(VID _v1id, VID _v2id,
                                                     int _n) {
  VI v1;
  CVI cv1;
  if (IsVertex(_v1id, &cv1)) {
...
template <class VERTEX, class WEIGHT>
int WeightedMultiDiGraph<VERTEX, WEIGHT>::DeleteWtEdge(VID _v1id, VID _v2id,
                                                       WEIGHT _weight, int _n) {
  VI v1;
  CVI cv1;
  if (IsVertex(_v1id, &cv1)) {
    v1 = const_cast<VI>(cv1);
...
template <class VERTEX, class WEIGHT>
void WeightedMultiDiGraph<VERTEX, WEIGHT>::DeleteAllEdgesToV(VERTEX &_v2) {
  CVI v2;
  if (IsVertex(_v2, &v2)) {
    for (VI vi = v.begin(); vi < v.end(); vi++) {
...
template <class VERTEX, class WEIGHT>
void WeightedMultiDiGraph<VERTEX, WEIGHT>::DeleteAllEdgesFromV(VERTEX &_v1) {
  CVI cv1;
  VI v1;
  if (IsVertex(_v1, &cv1)) {
    v1 = const_cast<VI>(cv1);
...
template <class VERTEX, class WEIGHT>
void WeightedMultiDiGraph<VERTEX, WEIGHT>::DeleteAllEdges(VERTEX &_v) {
...
// default: delete all edges (v1,v2), otherwise delete _n
template <class VERTEX, class WEIGHT>
int WeightedMultiDiGraph<VERTEX, WEIGHT>::DeleteEdge(VERTEX &_v1, VERTEX &_v2,
                                                     int _n) {
  VI v1, v2;
  CVI cv1, cv2;
  if (IsVertex(_v1, &cv1) && IsVertex(_v2, &cv2)) {
...
// default: delete all edges (v1,v2) of specified weight, otherwise delete _n
template <class VERTEX, class WEIGHT>
int WeightedMultiDiGraph<VERTEX, WEIGHT>::DeleteWtEdge(VERTEX &_v1, VERTEX &_v2,
                                                       WEIGHT _weight, int _n) {
  VI v1, v2;
  CVI cv1, cv2;
  if (IsVertex(_v1, &cv1) && IsVertex(_v2, &cv2)) {
...
template <class VERTEX, class WEIGHT>
bool WeightedMultiDiGraph<VERTEX, WEIGHT>::IsVertex(VID _v1id) const {
  CVI v1;
  return (IsVertex(_v1id, &v1));
}

template <class VERTEX, class WEIGHT>
bool WeightedMultiDiGraph<VERTEX, WEIGHT>::IsVertex(
    VID _v1id, const Vertex **_v1ptr) const {
...
template <class VERTEX, class WEIGHT>
bool WeightedMultiDiGraph<VERTEX, WEIGHT>::IsVertex(VERTEX &_v1) const {
  CVI v1;
...
template <class VERTEX, class WEIGHT>
bool WeightedMultiDiGraph<VERTEX, WEIGHT>::IsVertex(
    VERTEX &_v1, const Vertex **_v1ptr) const {
...
template <class VERTEX, class WEIGHT>
bool WeightedMultiDiGraph<VERTEX, WEIGHT>::IsEdge(VID _v1id, VID _v2id) const {
  CVI v1;
  CEI e12;
...
template <class VERTEX, class WEIGHT>
bool WeightedMultiDiGraph<VERTEX, WEIGHT>::IsEdge(VID _v1id, VID _v2id,
                                                  WEIGHT _weight) const {
  CVI v1;
  CEI e12;
...
template <class VERTEX, class WEIGHT>
bool WeightedMultiDiGraph<VERTEX, WEIGHT>::IsEdge(
    VID _v1id, VID _v2id, const Vertex **_v1ptr, const WtEdge **_e12ptr) const {
  CVI v1;
  CEI e12;
...
template <class VERTEX, class WEIGHT>
bool WeightedMultiDiGraph<VERTEX, WEIGHT>::IsEdge(
    VID _v1id, VID _v2id, WEIGHT _weight, const Vertex **_v1ptr,
    const WtEdge **_e12ptr) const {
  CVI v1;
  CEI e12;
  if (IsVertex(_v1id, &v1)) {
...
template <class VERTEX, class WEIGHT>
bool WeightedMultiDiGraph<VERTEX, WEIGHT>::IsEdge(VERTEX &_v1,
                                                  VERTEX &_v2) const {
  CVI v1;
  CEI e12;
...
template <class VERTEX, class WEIGHT>
bool WeightedMultiDiGraph<VERTEX, WEIGHT>::IsEdge(VERTEX &_v1, VERTEX &_v2,
                                                  WEIGHT _weight) const {
  CVI v1;
  CEI e12;
...
template <class VERTEX, class WEIGHT>
bool WeightedMultiDiGraph<VERTEX, WEIGHT>::IsEdge(
    VERTEX &_v1, VERTEX &_v2, const Vertex **_v1ptr,
    const WtEdge **_e12ptr) const {
  CVI v1, v2;
  CEI e12;
...
template <class VERTEX, class WEIGHT>
bool WeightedMultiDiGraph<VERTEX, WEIGHT>::IsEdge(
    VERTEX &_v1, VERTEX &_v2, WEIGHT _weight, const Vertex **_v1ptr,
    const WtEdge **_e12ptr) const {
  CVI v1, v2;
  CEI e12;
  if (IsVertex(_v1, &v1) && IsVertex(_v2, &v2)) {
...
template <class VERTEX, class WEIGHT>
vector<VID> WeightedMultiDiGraph<VERTEX, WEIGHT>::GetVerticesVID() const {
  vector<VID> verts;
...
template <class VERTEX, class WEIGHT>
vector<VID> WeightedMultiDiGraph<VERTEX, WEIGHT>::GetVerticesVID(VID _vid,
                                                                 int _n) const {
  vector<VID> verts;
  verts.reserve(_n);
  CVI v1, v2;
  ...
  if (IsVertex(_vid, &v1)) {
    ...
    cout << "\nIn GetVerticesVID(VID,int): no vertex VID=" << _vid
         << " in graph\n";
  }
  sort(verts.begin(), verts.end());
  return verts;
...
template <class VERTEX, class WEIGHT>
vector<VERTEX> WeightedMultiDiGraph<VERTEX, WEIGHT>::GetVerticesData() const {
  vector<VERTEX> verts;
  verts.reserve(v.size());
  for (CVI vi = v.begin(); vi < v.end(); vi++) {
...
template <class VERTEX, class WEIGHT>
vector<VERTEX>
WeightedMultiDiGraph<VERTEX, WEIGHT>::GetVerticesData(VID _vid, int _n) const {
  vector<VERTEX> verts;
  verts.reserve(_n);
  CVI v1, v2;
  unsigned int i;
  if (IsVertex(_vid, &v1)) {
    ...
    cout << "\nIn GetVerticesData(VID,int): no vertex VID=" << _vid
         << " in graph\n";
  }
  return verts;
}
...
WeightedMultiDiGraph<VERTEX, WEIGHT>::GetEdges() const {
...
WeightedMultiDiGraph<VERTEX, WEIGHT>::GetEdgesVData() const {
...
template <class VERTEX, class WEIGHT>
VERTEX WeightedMultiDiGraph<VERTEX, WEIGHT>::GetData(VID _v1id) const {
  CVI v1;
  if (IsVertex(_v1id, &v1)) {
    return v1->data;
  } else {
...
template <class VERTEX, class WEIGHT>
VERTEX *WeightedMultiDiGraph<VERTEX, WEIGHT>::GetReferenceofData(VID _v1id) {
  CVI v1;
  if (IsVertex(_v1id, &v1)) {
...
template <class VERTEX, class WEIGHT>
vector<VERTEX> WeightedMultiDiGraph<VERTEX, WEIGHT>::GetData(VID _v1id,
                                                             VID _v2id) const {
  CVI v1, v2;
  vector<VERTEX> vset;
...
template <class VERTEX, class WEIGHT>
VID WeightedMultiDiGraph<VERTEX, WEIGHT>::GetVID(VERTEX &_v1) const {
  CVI v1;
  if (IsVertex(_v1, &v1)) {
    return v1->vid;
  } else {
...
template <class VERTEX, class WEIGHT>
int WeightedMultiDiGraph<VERTEX, WEIGHT>::GetVertexOutDegree(VID _v1) const {
  CVI v1;
  if (IsVertex(_v1, &v1)) {
...
template <class VERTEX, class WEIGHT>
vector<VID>
WeightedMultiDiGraph<VERTEX, WEIGHT>::GetSuccessors(VID _v1id) const {
  vector<VID> succ;
  CVI v1;
  if (IsVertex(_v1id, &v1)) {
...
template <class VERTEX, class WEIGHT>
vector<VERTEX>
WeightedMultiDiGraph<VERTEX, WEIGHT>::GetSuccessorsDATA(VID _v1id) const {
  vector<VERTEX> succ;
  CVI v1, v2;
  if (IsVertex(_v1id, &v1)) {
    succ.reserve(v1->edgelist.size());
...
template <class VERTEX, class WEIGHT>
vector<VID>
WeightedMultiDiGraph<VERTEX, WEIGHT>::GetSuccessors(VERTEX &_v1) const {
  return GetSuccessors(GetVID(_v1));
}

template <class VERTEX, class WEIGHT>
vector<VERTEX>
WeightedMultiDiGraph<VERTEX, WEIGHT>::GetSuccessorsDATA(VERTEX &_v1) const {
  return GetSuccessorsDATA(GetVID(_v1));
}

template <class VERTEX, class WEIGHT>
vector<VID>
WeightedMultiDiGraph<VERTEX, WEIGHT>::GetPredecessors(VID _v1id) const {
  vector<VID> pred;
  CVI cv1, v1;
  if (IsVertex(_v1id, &cv1)) {
    v1 = const_cast<VI>(cv1);
    pred.reserve(v1->predecessors.size());
    for (CEI ei = v1->predecessors.begin(); ei != v1->predecessors.end();
         ei++) {
...
template <class VERTEX, class WEIGHT>
vector<VERTEX>
WeightedMultiDiGraph<VERTEX, WEIGHT>::GetPredecessorsDATA(VID _v1id) const {
  vector<VERTEX> pred;
  CVI cv1, v1, v2;
  if (IsVertex(_v1id, &cv1)) {
    v1 = const_cast<VI>(cv1);
...
template <class VERTEX, class WEIGHT>
vector<VID>
WeightedMultiDiGraph<VERTEX, WEIGHT>::GetPredecessors(VERTEX &_v1) const {
  return GetPredecessors(GetVID(_v1));
}

template <class VERTEX, class WEIGHT>
vector<VERTEX>
WeightedMultiDiGraph<VERTEX, WEIGHT>::GetPredecessorsDATA(VERTEX &_v1) const {
  return GetPredecessorsDATA(GetVID(_v1));
}

template <class VERTEX, class WEIGHT>
vector<VID> WeightedMultiDiGraph<VERTEX, WEIGHT>::GetSources() const {
  ...
  for (cv1 = v.begin(); cv1 != v.end(); cv1++) {
    v1 = const_cast<VI>(cv1);
    if (v1->predecessors.empty())
      sourcevids.push_back(v1->vid);
  }
  return sourcevids;
}

template <class VERTEX, class WEIGHT>
vector<VID> WeightedMultiDiGraph<VERTEX, WEIGHT>::GetSinks() const {
  vector<VID> sinkvids;
  CVI cv1;
  VI v1;
  for (cv1 = v.begin(); cv1 != v.end(); cv1++) {
    //        v1 = const_cast<VI> (cv1);
    if (cv1->edgelist.empty())
      sinkvids.push_back(cv1->vid);
  }
  return sinkvids;
}

template <class VERTEX, class WEIGHT>
WEIGHT WeightedMultiDiGraph<VERTEX, WEIGHT>::GetEdgeWeight(VID _v1id,
                                                           VID _v2id) const {
  CVI v1;
  CEI e12;
...
template <class VERTEX, class WEIGHT>
WEIGHT WeightedMultiDiGraph<VERTEX, WEIGHT>::GetEdgeWeight(VERTEX &_v1,
                                                           VERTEX &_v2) const {
...
template <class VERTEX, class WEIGHT>
WeightedMultiDiGraph<VERTEX, WEIGHT>
WeightedMultiDiGraph<VERTEX, WEIGHT>::BFS(VERTEX &_startV) const {
  CVI cv1;
...
template <class VERTEX, class WEIGHT>
WeightedMultiDiGraph<VERTEX, WEIGHT>
WeightedMultiDiGraph<VERTEX, WEIGHT>::BFS(VID _startVid) const {
...
template <class VERTEX, class WEIGHT>
vector<VID>
WeightedMultiDiGraph<VERTEX, WEIGHT>::BFSVID(VERTEX &_startV) const {
  WeightedMultiDiGraph<VERTEX, WEIGHT> bfstree = BFS(_startV);
  return bfstree.GetVerticesVID();
}

template <class VERTEX, class WEIGHT>
vector<VID> WeightedMultiDiGraph<VERTEX, WEIGHT>::BFSVID(VID _startVID) const {
...
template <class VERTEX, class WEIGHT>
WeightedMultiDiGraph<VERTEX, WEIGHT>
WeightedMultiDiGraph<VERTEX, WEIGHT>::DFS(VERTEX &_startV) const {
  CVI cv1;
  VI v1;
  if (IsVertex(_startV, &cv1)) {
    v1 = const_cast<VI>(cv1);
...
template <class VERTEX, class WEIGHT>
WeightedMultiDiGraph<VERTEX, WEIGHT>
WeightedMultiDiGraph<VERTEX, WEIGHT>::DFS(VID &vid) const {
...
template <class VERTEX, class WEIGHT>
vector<VID>
WeightedMultiDiGraph<VERTEX, WEIGHT>::DFSVID(VERTEX &_startV) const {
  WeightedMultiDiGraph<VERTEX, WEIGHT> dfstree = DFS(_startV);
  return dfstree.GetVerticesVID();
}

template <class VERTEX, class WEIGHT>
vector<VID> WeightedMultiDiGraph<VERTEX, WEIGHT>::DFSVID(VID &_startVID) const {
...
WeightedMultiDiGraph<VERTEX, WEIGHT>::DFS() const {
...
template <class VERTEX, class WEIGHT>
bool WeightedMultiDiGraph<VERTEX, WEIGHT>::IsCycle() const {
  dfsinfo dfs(this->GetVertexCount());
  aux_DFS(dfs);
  if (dfs.backedge_vector.empty())
    return false;
  else
    return true;
}
...
template <class VERTEX, class WEIGHT>
vector<pair<VID, VID>>
WeightedMultiDiGraph<VERTEX, WEIGHT>::GetBackedge() const {
  dfsinfo dfs(this->GetVertexCount());
  aux_DFS(dfs);
...
template <class VERTEX, class WEIGHT>
vector<VID> WeightedMultiDiGraph<VERTEX, WEIGHT>::TopologicalSort() const {
  unsigned int i, n;
  vector<VID> tps;
...
template <class VERTEX, class WEIGHT>
bool WeightedMultiDiGraph<VERTEX, WEIGHT>::
#ifdef __HP_aCC
...
template <class VERTEX, class WEIGHT>
void WeightedMultiDiGraph<VERTEX, WEIGHT>::aux_DFS(dfsinfo &dfs) const {
...
template <class VERTEX, class WEIGHT>
WeightedMultiDiGraph<VERTEX, WEIGHT>
WeightedMultiDiGraph<VERTEX, WEIGHT>::true_DFS(VID &vid, dfsinfo &dfs) const {
  // core DFS algorithm, start w/ vertex vid
...
WeightedMultiDiGraph<VERTEX, WEIGHT>::FindPathBFS(VERTEX &_startV,
                                                  VERTEX &_endV) const {
...
template <class VERTEX, class WEIGHT>
vector<pair<VERTEX, WEIGHT>>
WeightedMultiDiGraph<VERTEX, WEIGHT>::FindPathBFS(VID _startVid,
                                                  VID _endVid) const {
  ...
  if (bfstree.IsVertex(_endVid, &cv1) && bfstree.IsVertex(_startVid, &cv2)) {
    path.insert(path.begin(),
                pair<VERTEX, WEIGHT>(cv1->data, WEIGHT::InvalidWeight()));
...
template <class VERTEX, class WEIGHT>
vector<VID>
WeightedMultiDiGraph<VERTEX, WEIGHT>::FindVIDPathBFS(VID _startVid,
                                                     VID _endVid) const {
  WeightedMultiDiGraph<VERTEX, WEIGHT> bfstree;
  vector<VID> path;
  list<VID> q;
...
template <class VERTEX, class WEIGHT>
WeightedMultiDiGraph<VERTEX, WEIGHT>
WeightedMultiDiGraph<VERTEX, WEIGHT>::DijkstraSSSP(VID _startVid) const {
...
template <class VERTEX, class WEIGHT>
vector<pair<VERTEX, WEIGHT>>
WeightedMultiDiGraph<VERTEX, WEIGHT>::FindPathDijkstra(VID _v1id,
                                                       VID _v2id) const {
...
template <class VERTEX, class WEIGHT>
vector<pair<VERTEX, WEIGHT>>
WeightedMultiDiGraph<VERTEX, WEIGHT>::FindPathDijkstra(VERTEX &_startV,
                                                       VERTEX &_endV) const {
...
template <class VERTEX, class WEIGHT>
void WeightedMultiDiGraph<VERTEX, WEIGHT>::DisplayGraph() const {
  CVI vi;
  unsigned int i;
  cout << endl;
...
template <class VERTEX, class WEIGHT>
void WeightedMultiDiGraph<VERTEX, WEIGHT>::DisplayVertices() const {
  for (CVI vi = v.begin(); vi != v.end(); vi++) {
...
template <class VERTEX, class WEIGHT>
void WeightedMultiDiGraph<VERTEX, WEIGHT>::DisplayVertex(VID _v1id) const {
  CVI v1;
  if (IsVertex(_v1id, &v1)) {
...
template <class VERTEX, class WEIGHT>
void WeightedMultiDiGraph<VERTEX, WEIGHT>::DisplayVertexAndEdgelist(
    VID _v1id) const {
  CVI v1;
  if (IsVertex(_v1id, &v1)) {
    cout << "vertex: ";
...
template <class VERTEX, class WEIGHT>
void WeightedMultiDiGraph<VERTEX, WEIGHT>::WriteGraph(
...
template <class VERTEX, class WEIGHT>
void WeightedMultiDiGraph<VERTEX, WEIGHT>::WriteGraph(
    ostream &_myostream) const {

  _myostream << endl << "#####GRAPHSTART#####";
  //_myostream << endl << "GRAPHSTART";
  _myostream << endl
             << this->numVerts << " " << this->numEdges << " " << this->vertIDs;
...
template <class VERTEX, class WEIGHT>
void WeightedMultiDiGraph<VERTEX, WEIGHT>::ReadGraph(const char *_fname) {
...
template <class VERTEX, class WEIGHT>
void WeightedMultiDiGraph<VERTEX, WEIGHT>::ReadGraph(istream &_myistream) {
...
template <class VERTEX, class WEIGHT>
void WeightedMultiDiGraph<VERTEX, WEIGHT>::ReadGraphwithAutoVID(
    const char *_fname) {
...
WeightedMultiDiGraph<VERTEX, WEIGHT>::my_find_VID_eq(const VID _vid) const {
  CVI vi, startvi;
...
template <class VERTEX, class WEIGHT>
WtVertexType<VERTEX, WEIGHT> *
WeightedMultiDiGraph<VERTEX, WEIGHT>::my_find_VDATA_eq(const VERTEX &_v) const {
  CVI cvi = v.begin();
  VI vi;
  bool found = false;
...
class
WeightedMultiDiGraph<VERTEX,WEIGHT>::
VID_eq : public unary_function<
WeightedMultiDiGraph<VERTEX,WEIGHT>::Vertex,bool> { public: explicit
VID_eq(const VID vid) : testid (vid) {} bool operator() (Vertex v) { return
v.vid == testid;
}
protected:
private:
VID testid;
  ...
  class
  WeightedMultiDiGraph<VERTEX,WEIGHT>::
  VDATA_eq : public unary_function< WeightedMultiDiGraph<VERTEX,WEIGHT>::Vertex,
bool> { public: explicit VDATA_eq(const VERTEX vt) : testdata (vt) {} bool
operator() (Vertex v) { return v.data == testdata;
  }
  protected:
  private:
  VERTEX testdata;
...
template <class VERTEX, class WEIGHT>
WeightedGraph<VERTEX, WEIGHT>::WeightedGraph() {}

template <class VERTEX, class WEIGHT>
WeightedGraph<VERTEX, WEIGHT>::WeightedGraph(int _sz)
    : WeightedMultiDiGraph<VERTEX, WEIGHT>(_sz) {}

template <class VERTEX, class WEIGHT>
WeightedGraph<VERTEX, WEIGHT>::WeightedGraph(int _sz, int _edgelistsz)
...
template <class VERTEX, class WEIGHT>
int WeightedGraph<VERTEX, WEIGHT>::AddEdge(VID _v1id, VID _v2id,
                                           WEIGHT _weight) {
  CVI cv1, cv2;
  VI v1, v2;
...
template <class VERTEX, class WEIGHT>
int WeightedGraph<VERTEX, WEIGHT>::AddEdge(VERTEX &_v1, VERTEX &_v2,
                                           WEIGHT _weight) {
  CVI cv1, cv2;
  VI v1, v2;
...
template <class VERTEX, class WEIGHT>
int WeightedGraph<VERTEX, WEIGHT>::AddEdge(VID _v1id, EI _ei) {
  CVI cv1, cv2;
  VI v1, v2;
  VID v2id = _ei->vertex2id;
  WEIGHT weight = _ei->weight;
...
template <class VERTEX, class WEIGHT>
int WeightedGraph<VERTEX, WEIGHT>::DeleteEdge(VID _v1id, VID _v2id, int _n) {
  CVI cv1, cv2;
  VI v1, v2;

  if (IsVertex(_v1id, &cv1) && IsVertex(_v2id, &cv2) &&
      this->IsEdge(_v1id, _v2id)) {
    v1 = const_cast<VI>(cv1);
    v2 = const_cast<VI>(cv2);
...
template <class VERTEX, class WEIGHT>
int WeightedGraph<VERTEX, WEIGHT>::DeleteWtEdge(VID _v1id, VID _v2id, WEIGHT _w,
                                                int _n) {
  CVI cv1, cv2;
  VI v1, v2;

  if (IsVertex(_v1id, &cv1) && IsVertex(_v2id, &cv2) &&
      this->IsEdge(_v1id, _v2id, _w)) {
    v1 = const_cast<VI>(cv1);
    v2 = const_cast<VI>(cv2);
    int ok1 = v1->DeleteXEdges(_v2id, -1);
    int ok2 = v2->DeleteXEdges(_v1id, -1);
...
template <class VERTEX, class WEIGHT>
int WeightedGraph<VERTEX, WEIGHT>::DeleteEdge(VERTEX &_v1, VERTEX &_v2,
                                              int _n) {
  CVI cv1, cv2;
  VI v1, v2;
...
template <class VERTEX, class WEIGHT>
int WeightedGraph<VERTEX, WEIGHT>::DeleteWtEdge(VERTEX &_v1, VERTEX &_v2,
                                                WEIGHT _w, int _n) {
  CVI cv1, cv2;
  VI v1, v2;

  if (IsVertex(_v1, &cv1) && IsVertex(_v2, &cv2) &&
      this->IsEdge(_v1, _v2, _w)) {
    v1 = const_cast<VI>(cv1);
    v2 = const_cast<VI>(cv2);
    int ok1 = v1->DeleteXEdges(v2->vid, -1);
    int ok2 = v2->DeleteXEdges(v1->vid, -1);
...
template <class VERTEX, class WEIGHT>
int WeightedGraph<VERTEX, WEIGHT>::ChangeEdgeWeight(VID _v1id, VID _v2id,
                                                    WEIGHT _weight) {
...
template <class VERTEX, class WEIGHT>
int WeightedGraph<VERTEX, WEIGHT>::ChangeEdgeWeight(VERTEX &_v1, VERTEX &_v2,
                                                    WEIGHT _weight) {
...
template <class VERTEX, class WEIGHT>
vector<pair<pair<VID, VID>, WEIGHT>>
WeightedGraph<VERTEX, WEIGHT>::GetEdges() const {
  vector<pair<pair<VID, VID>, WEIGHT>> edges;
...
template <class VERTEX, class WEIGHT>
vector<pair<pair<VERTEX, VERTEX>, WEIGHT>>
WeightedGraph<VERTEX, WEIGHT>::GetEdgesVData() const {
  vector<pair<pair<VERTEX, VERTEX>, WEIGHT>> edges;
...
template <class VERTEX, class WEIGHT>
int WeightedGraph<VERTEX, WEIGHT>::GetVertexDegree(VID _v1id) const {
  CVI v1;
  if (IsVertex(_v1id, &v1)) {
    return v1->edgelist.size();
  } else {
...
template <class VERTEX, class WEIGHT>
vector<VID>
WeightedGraph<VERTEX, WEIGHT>::GetAdjacentVertices(VID _v1id) const {
  return this->GetSuccessors(_v1id);
}

template <class VERTEX, class WEIGHT>
vector<pair<pair<VID, VID>, WEIGHT>>
WeightedGraph<VERTEX, WEIGHT>::GetIncidentEdges(VID _v1id) const {
...
template <class VERTEX, class WEIGHT>
vector<pair<pair<VERTEX, VERTEX>, WEIGHT>>
WeightedGraph<VERTEX, WEIGHT>::GetIncidentEdgesVData(VID _v1id) const {
  ...
  if (IsVertex(_v1id, &v1)) {
    iedges.reserve(v1->edgelist.size());
    for (CEI ei = v1->edgelist.begin(); ei != v1->edgelist.end(); ei++) {
      pair<VERTEX, VERTEX> nextedge(this->GetData(_v1id),
                                    this->GetData(ei->vertex2id));
...
template <class VERTEX, class WEIGHT>
bool WeightedGraph<VERTEX, WEIGHT>::IsSameCC(VID _v1id, VID _v2id) const {
...
template <class VERTEX, class WEIGHT>
bool WeightedGraph<VERTEX, WEIGHT>::IsSameCC(VERTEX &_v1, VERTEX &_v2) const {
...
template <class VERTEX, class WEIGHT>
vector<VID> WeightedGraph<VERTEX, WEIGHT>::GetCC(VID _v1id) const {
...
template <class VERTEX, class WEIGHT>
vector<VERTEX> WeightedGraph<VERTEX, WEIGHT>::GetCC(VERTEX &_v1) const {
...
template <class VERTEX, class WEIGHT>
vector<pair<pair<VID, VID>, WEIGHT>>
WeightedGraph<VERTEX, WEIGHT>::GetCCEdges(VID _v1id) const {
...
template <class VERTEX, class WEIGHT>
vector<pair<pair<VID, VID>, WEIGHT>>
WeightedGraph<VERTEX, WEIGHT>::GetCCEdges(VERTEX &_v1) const {
  return GetCCEdges(GetVID(_v1));
}

template <class VERTEX, class WEIGHT>
vector<pair<pair<VERTEX, VERTEX>, WEIGHT>>
WeightedGraph<VERTEX, WEIGHT>::GetCCEdgesVData(VID _v1id) const {
...
template <class VERTEX, class WEIGHT>
vector<pair<pair<VERTEX, VERTEX>, WEIGHT>>
WeightedGraph<VERTEX, WEIGHT>::GetCCEdgesVData(VERTEX &_v1) const {
...
WeightedGraph<VERTEX, WEIGHT>::GetEdgesByCCVDataOnly() const {

  vector<vector<pair<VERTEX, VERTEX>>> ccedges;

  vector<pair<int, VID>> cc = GetCCStats();
  for (unsigned int i = 0; i < cc.size(); i++) {
    vector<pair<pair<VERTEX, VERTEX>, WEIGHT>> thiscc =
        GetCCEdgesVData(cc[i].second);
    vector<pair<VERTEX, VERTEX>> edges;
    for (int j = 0; j < thiscc.size(); j++) {
      pair<VERTEX, VERTEX> thisedge(thiscc[j].first.first,
                                    thiscc[j].first.second);
...
template <class VERTEX, class WEIGHT>
vector<pair<int, VID>> WeightedGraph<VERTEX, WEIGHT>::GetCCStats() const {
  ...
  while (verts.size() != 0) {
    ...
    set_difference(verts.begin(), verts.end(), CC.begin(), CC.end(),
                   verts.begin());
...
/// Modified for VC
#if defined(WIN32)
  typedef bool (*Compare_Fun_Ptr)(const pair<int, VID> &,
                                  const pair<int, VID> &);
...
template <class VERTEX, class WEIGHT>
int WeightedGraph<VERTEX, WEIGHT>::GetCCcount() const {
...
template <class VERTEX, class WEIGHT>
void WeightedGraph<VERTEX, WEIGHT>::DisplayCC(VID _v1id) const {
...
template <class VERTEX, class WEIGHT>
void WeightedGraph<VERTEX, WEIGHT>::DisplayEdgesByCCVDataOnly() const {
...
template <class VERTEX, class WEIGHT>
void WeightedGraph<VERTEX, WEIGHT>::DisplayCCStats(int _maxCCprint) const {
...
template <class VERTEX, class WEIGHT>
WeightedGraph<VERTEX, WEIGHT> WeightedGraph<VERTEX, WEIGHT>::DagToUndirected(
    WeightedMultiDiGraph<VERTEX, WEIGHT> &_dag) {
  VI v1, v2;
  CVI cv2;
...
template <class VERTEX, class WEIGHT>
WtVertexType<VERTEX, WEIGHT>::WtVertexType() {}

template <class VERTEX, class WEIGHT>
WtVertexType<VERTEX, WEIGHT>::WtVertexType(VERTEX &_data, VID _id) {
  data = _data;
  vid = _id;
}
...
template <class VERTEX, class WEIGHT>
void WtVertexType<VERTEX, WEIGHT>::AddEdge(VID _v2id, WEIGHT _weight) {
  WtEdge newEdge(_v2id, _weight);
  edgelist.push_back(newEdge);
}

template <class VERTEX, class WEIGHT>
void WtVertexType<VERTEX, WEIGHT>::AddEdgewCheck(VID _v2id, WEIGHT _weight) {
  bool found = false;
  for (EI ei = edgelist.begin(); ei != edgelist.end(); ei++) {
    if (_v2id == ei->vertex2id)
      found = true;
  }
  if (!found) {
    WtEdge newEdge(_v2id, _weight);
...
template <class VERTEX, class WEIGHT>
void WtVertexType<VERTEX, WEIGHT>::AddPredecessorEdge(VID _v0id,
                                                      WEIGHT _weight) {
...
template <class VERTEX, class WEIGHT>
int WtVertexType<VERTEX, WEIGHT>::DeleteXEdges(VID _v2id, int _x) {
  int num_to_delete;
  int num_deleted = 0;
  if (_x == -1)
...
template <class VERTEX, class WEIGHT>
int WtVertexType<VERTEX, WEIGHT>::DeleteXEdges(VID _v2id, WEIGHT _weight,
                                               int _x) {
  int num_to_delete;
  int num_deleted = 0;
  if (_x == -1)
...
template <class VERTEX, class WEIGHT>
bool WtVertexType<VERTEX, WEIGHT>::IsEdge(VID _v2id) const {
  EI ei;
  return (IsEdge(_v2id, &ei));
}

template <class VERTEX, class WEIGHT>
bool WtVertexType<VERTEX, WEIGHT>::IsEdge(VID _v2id, const WtEdge **_ei) const {
...
template <class VERTEX, class WEIGHT>
bool WtVertexType<VERTEX, WEIGHT>::IsEdge(VID _v2id, WEIGHT _weight,
                                          const WtEdge **_ei) const {
...
template <class VERTEX, class WEIGHT>
VERTEX WtVertexType<VERTEX, WEIGHT>::GetData() const {
  return data;
}

template <class VERTEX, class WEIGHT>
int WtVertexType<VERTEX, WEIGHT>::GetVID() const {
  return vid;
}

template <class VERTEX, class WEIGHT>
int WtVertexType<VERTEX, WEIGHT>::GetEdgeCount() const {
  return edgelist.size();
}

template <class VERTEX, class WEIGHT>
WEIGHT WtVertexType<VERTEX, WEIGHT>::GetEdgeWeight(VID _v2id) const {
  EI ei;
  CEI cei;
  if (IsEdge(_v2id, &cei)) {
...
template <class VERTEX, class WEIGHT>
void WtVertexType<VERTEX, WEIGHT>::DisplayEdgelist() const {
  /*
...
template <class VERTEX, class WEIGHT>
void WtVertexType<VERTEX, WEIGHT>::WriteEdgelist(ostream &_myostream) const {
...
WtVertexType<VERTEX, WEIGHT>::my_find_EID1_eq(const EID _eid) const {
...
template <class VERTEX, class WEIGHT>
WtEdgeType<VERTEX, WEIGHT> *WtVertexType<VERTEX, WEIGHT>::my_find_EID2_eq(
    const WtEdgeType<VERTEX, WEIGHT> *_start,
    const WtEdgeType<VERTEX, WEIGHT> *_end, const EID _eid) const {
  CEI ei = _start;
  bool found = false;
  while (ei != _end && !found) {
    if (ei->vertex2id == _eid) {
...
template <class VERTEX, class WEIGHT>
WtEdgeType<VERTEX, WEIGHT> *
WtVertexType<VERTEX, WEIGHT>::my_find_EWT_eq(const WEIGHT _wt) const {
  CEI ei = edgelist.begin();
  bool found = false;
  while (ei != edgelist.end() && !found) {
    if (ei->weight == _wt) {
...
template <class VERTEX, class WEIGHT>
WtEdgeType<VERTEX, WEIGHT> *WtVertexType<VERTEX, WEIGHT>::my_find_EIDWT_eq(
    const pair<VID, WEIGHT> _wtpair) const {
...
template <class VERTEX, class WEIGHT>
WtEdgeType<VERTEX, WEIGHT>::WtEdgeType() {}

template <class VERTEX, class WEIGHT>
WtEdgeType<VERTEX, WEIGHT>::WtEdgeType(VID _v2id, WEIGHT _weight) {
  vertex2id = _v2id;
  weight = _weight;
}
...
template <class VERTEX, class WEIGHT>
void WtEdgeType<VERTEX, WEIGHT>::DisplayEdge() const {
  cout << vertex2id << "(" << weight << ")";
}

template <class VERTEX, class WEIGHT>
void WtEdgeType<VERTEX, WEIGHT>::WriteEdge(ostream &_myostream) const {

  ...
  if (size > 1) {
    ...
    for (int iu = 1; iu < size; iu++) {
      if (tmp.empty())
        ...
        if ((ma_pt[iu] - tmp.back()) < bound)
          continue; // too small
        tmp.push_back(ma_pt[iu]);
      }
    }
    if (ma_pt.size() == 0)
      ma_pt.push_back(ma_pt[(size + 1) / 2]);
    if (tmp.front() > 0.6)
      tmp.push_front(0);
    if (1 - tmp.back() > 0.6)
      tmp.push_back(1);
...
inline double MaxLength(Line &PA, MouthLink &mouth_link, vector<float> &ma_pt) {
  typedef MouthLink::iterator MIT;
  double max_len = 0;
  ma_pt.push_back(0);
  ma_pt.push_back(1);
  ...
  for (MIT im = mouth_link.begin(); im != mouth_link.end(); im++) {
    ...
    for (int i = 0; i < 2; i++)
      pt[i] = (1 - u) * PA.first[i] + u * PA.second[i];
...
inline void SimplifyLink(Line &PA, MouthLink &mouth_link,
                         vector<float> &ma_pt) {
  typedef MouthLink::iterator MIT;
...
inline double ClosestPt(const Line &l, const Point2d &p, const Vector2d &n) {
  Line l2(p + (n * 100), p + (n * -100));

  double s = ClosestToLine(l, l2);
  Point2d result;
  for (int i = 0; i < 2; i++)
    result[i] = (1 - s) * l.first[i] + s * l.second[i];
  if ((p - result).normsqr() > (p - l.first).normsqr())
    s = 0;
  if ((p - result).normsqr() > (p - l.second).normsqr())
    s = 1;

  return s;
}

inline void Mouth2PA(Line &PA, list<se_mouth *> &mouth, MouthLink &mouth_link) {
...
inline pair<double, double> FindExtrem(const vector<Point2d> &ptV,
                                       const Point2d &com, const Vector2d &v) {
  double min_v = 1e01;
  double max_v = -1e10;
...
///////////////////////////////////////////////////////////////////////////////
inline SKG ExtracSkeleton_PA(c_polygon &poly, list<se_mouth *> &mouth) {
  SKG bone;
  // if( mouth.size()!=2 ){

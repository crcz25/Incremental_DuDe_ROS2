...
///////////////////////////////////////////////////////////////////////////////
inline double intersect(const Point2d &p1, const Point2d &p2, const Point2d &o,
                        const Vector2d &n) {
  double b = n * (p2 - p1);
  if (b == 0)
    return -1;
...
inline se_m *belongs(se_m *m1, se_m *m2, const Point2d &p1, const Point2d &p2) {
  ///////////////////////////////////////////////////////////////////////////
  ply_vertex *head = (*(m1->cd)).front().getHead();
  ply_vertex *ptr = head;
  do {
    if (ptr->getPos().almost_equ(p1) && ptr->getPre()->getPos().almost_equ(p2))
      return m1;
    if (ptr->getPos().almost_equ(p2) && ptr->getPre()->getPos().almost_equ(p1))
      return m1;
...
inline void splitMouth(se_m *m1, se_m *m2, se_m *m, const c_diagonal &dia,
                       const Vector2d &n) {
...
inline void newMouth(se_m *m1, se_m *m2, const c_diagonal &dia,
                     const Vector2d &n) {
...
void updateMouth(se_m *m1, se_m *m2, se_m *m, const c_diagonal &dia) {
...
// reassign mouths in m to m1 and m2
inline void splitSingleMouth(se_m *m1, se_m *m, const c_diagonal &dia,
                             const Vector2d &n) {
  const Point2d &o = dia.getV1()->getPos(); //.v[0];
...
inline void newSingleMouth(se_m *m1, const c_diagonal &dia, const Vector2d &n) {
  const Point2d p1 = dia.getV1()->getPos(); // v[0];
  const Point2d p2 = dia.getV2()->getPos(); // v[1];
  se_mouth *m = new se_mouth();
  // create mouth
...
void decomposePolygon(pair<c_polygon *, c_polygon *> &polygons,
                      c_diagonal &cut_l, vector<c_diagonal> &allCuts,
                      vector<c_diagonal>::iterator tmpIte, se_m *m,

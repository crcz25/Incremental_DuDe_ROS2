...
void SE2d::removeItem(se_m *tm) { todo_list.remove(tm); }
void SE2d::addHoleDiagBranch(ply_vertex *v1, ply_vertex *v2, se_m *m) {
  c_diagonal cdiag1(v1, v1->getNext());
  Vector2d vec =
      (cdiag1.getV2()->getPos() - cdiag1.getV1()->getPos()).normalize();
...
void SE2d::Build(double d) {
  ...
  { // eaxtract mouth from "new" model
    ...
    for (vector<c_diagonal>::iterator dit = tmpCuts.begin();
         dit != tmpCuts.end(); ++dit) {
      ...
      if (v1->getExtra().parentPly->getType() == c_ply::POUT &&
        ...
        se_m *m =
            ply2MouthMap[tmpInitPlygon]; // getSEMFromTodolist(*initPlygon);
        ...
        cutPolys(subPolygons, *dit, dude.m_cuts, dit, p1hasmorecut,
                 p2hasmorecut);
        ...
        for (c_polygon::iterator pit = tmpInitPlygon->begin();
             pit != tmpInitPlygon->end(); ++pit) {
          if (ply1 == (&(*pit))) {
        ...
        //				newm1->com.set((p1[0] + p2[0])/2, (p1[1]
        //+ p2[1])/2); 				newm1->connect[0] = NULL; newm->connect[1] = m;//NULL;
        //				newm1->n = -n;
        ...
        //				vector<c_polygon*>::iterator itr =
        //todo_listPolygon.begin(); 				while (itr != todo_listPolygon.end())
        //				{
        //					if ((*itr) == tmpInitPlygon)
        //					{
        //						todo_listPolygon.erase(itr);

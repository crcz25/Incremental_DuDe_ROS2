...
void c_knapsack::build() {
  uint size = 0;
  for (vector<c_cutset>::iterator i = m_all_cutsets.begin();
       i != m_all_cutsets.end(); i++) {
...
inline int count_valid_cutsets(const vector<int> &cset_vector,
                               const vector<char> &flags) {
...
inline void get_remaining_cutsets(const vector<int> &cset_vector,
                                  const vector<char> &flags,
                                  vector<int> &remains) {
  int cset_vector_size = cset_vector.size();
...
inline void collect_resolvable_vertex(const c_cutset &cset,
  ...
  case c_cutset::DOUBLE_PM_PM:
    if (cset.getResolveSize() > 1) {
      ...
      if (d1.is_resolving(ov1, tau))
        resolv_v.push_back(ov1); // this cutset also resolves ov
      if (d2.is_resolving(ov2, tau))
  ...
  case c_cutset::DOUBLE_PM_TIP:
    ...
    break;
  case c_cutset::DOUBLE_TIP_TIP:
    break; // do nothing
  }
}

// check if a cutset can be removed
inline bool can_be_removed(int cset_id, const vector<c_cutset> &cutsets,
  ...
  for (list<ply_vertex *>::iterator i = resolv_v.begin(); i != resolv_v.end();
       i++) {
    ply_vertex *v = *i;
...
double c_knapsack::getOptimalCuts(list<ply_vertex *> &pms, double tau,
                                  vector<c_diagonal> &cuts) {
  if (m_table.empty())
    return 0;
  if (m_table.back().empty())
    return 0;
  ...
  // make sure that cutsets are unique...
  {
    ...
    for (int i = 0; i < c_cutsets_size; i++) {
      ...
      if (!found)
        all_cutsets.push_back(c.cutsets[i]);
    } // end i
  }
  ...
  for (int i = 0; i < cutset_size; i++) {
    ...
    // mark them
    for (list<ply_vertex *>::iterator j = resolv_v.begin(); j != resolv_v.end();
         j++) {
  ...
  //
  vector<char> book_keeping(cutset_size,
                            0); // flag '1' if the cutset cannot be removed
  ...
  for (VIT i = pms.begin(); i != pms.end(); i++) {
    ...
    if (cset_vector.size() == 1)
      book_keeping[cset_vector.front()] = '1';
  }
  ...
  for (VIT i = pms.begin(); i != pms.end(); i++) {
    ...
    if (cset_vector_size == 0) // nothing resolves this PM....
    {
      ...
      if (best_cutset_id >= 0)
        missing_cutsets.push_back(v_cutsets[best_cutset_id]);
    } else if (cset_vector_size >
               1) // there are more than 1 cutsets resolving this vertex
    {
      // remove redundant cuts....
      int reminaing_cutset_count =
          count_valid_cutsets(cset_vector, book_keeping);
      assert(reminaing_cutset_count); // we must still have some cutset
                                      // resolving this vertex
      ...
      if (reminaing_cutset_count > 1 && removal_candidates_size > 0) {
        ...
        if (reminaing_cutset_count >
            removal_candidates_size) // remove all cuts in removal_candidates
        {
          for (int j = 0; j < removal_candidates_size; j++) {
            int csetid = removal_candidates[j];

            if (can_be_removed(csetid, all_cutsets, map_v2cutsets, book_keeping,
                               tau)) {
              book_keeping[csetid] = '2'; // marked for removal
#if DEBUG
              cout << "- Remove cutset: id="
                   << all_cutsets[csetid].getV()->getVID() << ": "
                   << all_cutsets[csetid] << endl;
#endif
            }
          }
        } else // keep one cut and remove all the others
        {
          ...
          for (int j = 0; j < removal_candidates_size; j++) {
            ...
            if (best_cutset == cset_id) {
              ...
              if (can_be_removed(cset_id, all_cutsets, map_v2cutsets,
                                 book_keeping, tau)) {
  ...
  // add missing cutsets
  final_cutsets.insert(final_cutsets.end(), missing_cutsets.begin(),
                       missing_cutsets.end());
  ...
  for (uint i = 0; i < size; i++)
    cout << "\t " << cuts[i].getV1()->getVID() << ", "
         << cuts[i].getV2()->getVID() << endl;
#endif

  return c.score;
}

void c_knapsack::updateCell(uint n, uint m,
                            uint iteration) // update the table cell (n,m)
{
  c_cutset &cset = m_all_cutsets[m];
  uint csetsize = cset.getDiagonals().size();

  if (m == 0 &&
      iteration == 0) { // first iteration, first cell, bootstrap this row
    cell &c = m_table[n][0];
...
void c_knapsack::cell::addCutset(c_cutset &cset) {
  ...
  for (IT i = cutsets.begin(); i != cutsets.end(); i++) {
    if (i->conflict(cset) == false)
      new_cutset_list.push_back(*i);

...
void insertToRootTree(
    HoleDiagonal *root,
    HoleDiagonal *tmpHole) // ply_vertex* tmpStPnt,ply_vertex* tmpEndPnt)
{
...
//
double findPolesForCutpoly(vector<ply_vertex *> &polygon, ply_vertex *&m1,
                           ply_vertex *&m2) {
...
double determineConcavityForConvexHole(c_ply &hole, ply_vertex *&m1,
                                       ply_vertex *&m2, ply_vertex *&fV) {
  ...
  //		pSearchEnd = m1;
  //	}
  //	Vector2d m12(m2->getPos()[0] - m1->getPos()[0], m2->getPos()[1] -
  //m1->getPos()[1]); 	Vector2d m12Norm = m12.normalize(); 	double MinDotProduct =
  //FLT_MAX; 	ply_vertex* extraVForFurthestV = NULL; 	ply_vertex* pSearchCur =
  //pSearchStart; 	while(pSearchCur != pSearchEnd)
  //	{
  //		assert(pSearchCur != furthestV);
  //		Vector2d furVec(pSearchCur->getPos()[0] -
  //furthestV->getPos()[0], pSearchCur->getPos()[1] - furthestV->getPos()[1]);
  ...
  //	}
  //
  //	maxRectDist = distTwoPnts(furthestV->getPos(),
  //extraVForFurthestV->getPos());
...
HoleDiagonal *getHoleDiag(ply_vertex *v1, ply_vertex *v2,
                          vector<HoleDiagonal *> &holeDiags) {
  for (vector<HoleDiagonal *>::iterator dit = holeDiags.begin();
       dit != holeDiags.end(); ++dit) {
    HoleDiagonal *tdig = *dit;
    if ((tdig->startPnt == v1 && tdig->endPnt == v2) ||
        (tdig->startPnt == v2 && tdig->endPnt)) {
      return tdig;
    }
  }
  return NULL;
}
void collectFeatPntBetweenCuts(c_ply &hole, vector<ply_vertex *> &cuts,
                               list<ply_vertex *> &tmpHolePMs,
                               list<ply_vertex *> &tmpHoleFeatPnts, double tau,
                               vector<HoleDiagonal *> &holeDiags,
                               bool reducePMs) {
  uint tflag = ply_vertex_extra::getFlagID();
  for (vector<ply_vertex *>::const_iterator dit = cuts.begin();
       dit != cuts.end(); ++dit) {
    ply_vertex *ct = *dit;
  ...
  do {
    ...
    for (list<ply_vertex *>::iterator vit = tmpApproxlist.begin();
         vit != tmpApproxlist.end(); ++vit) {
      ply_vertex *tmpv = *vit;
      if (tmpv->isReflex()) {
        double toDiagDist = distToSeg(tmpLine.front()->getPos(),
                                      tmpLine.back()->getPos(), tmpv->getPos());
        tmpv->getExtra().concavity =
            min(tmpLine.front()->getExtra().holeCutConcavity,
                tmpLine.back()->getExtra().holeCutConcavity) +
            toDiagDist;
...
bool isHoleDiagonalValid(const ply_vertex *v1, const ply_vertex *v2) {
  if (v1->getExtra().getConcavity_hole() == NULL &&
      v2->getExtra().getConcavity_hole() == NULL)
    return true;
  ...
  if (v1->getExtra().getConcavity_hole() != NULL &&
      v1->getExtra().other_v != NULL) {
    helpV = v1->getExtra().other_v;
    minimumV = v1;
    minOtherV = v2;
  } else if (v2->getExtra().getConcavity_hole() != NULL &&
             v2->getExtra().other_v != NULL) {

...
struct ply_vertex_extra {
  ...
  bool isPM() const {
    return (concavity_bpc != NULL && concavity > 0) ||
           (concavity_hole != NULL && concavity > 0);
  }
  ...
  ply_vertex *
  getDihedralPre(); // get previous vertex for estimating the dihedral angle
  ply_vertex *
  getDihedralNext(); // get next vertex for estimating the dihedral angle

  void setConcavity_hole(c_ply *thole) { concavity_hole = thole; }
  ...
  float accLen; // accumulated length from start vertex... Added by Zhonghua
                // 04/06/2013
  float holeCutConcavity; // if the vertex is an end point of a cut for its
                          // hole. add by guilin 04/09/2013
...
public:
  ///////////////////////////////////////////////////////////////////////////
  ply_vertex() { init(); }
  ply_vertex(const Point2d &p) {
    pos = p;
    init();
  }
  ~ply_vertex();
  void setNext(ply_vertex *n) {
    next = n;
    if (n != NULL)
      n->pre = this;
  }
  void setPre(ply_vertex *n) {
    pre = n;
    if (n != NULL)
      n->next = this;
  }
...
private:
  ...
  // triangulation
  vector<triangle>
      triangulation; // catched triangulation, calculated by triangulate
};
...
public:
  ...
  double *getBBox() {
    assert(is_buildboxandcenter_called);
    return box;
  }
  const Point2d &getCenter() {
    assert(is_buildboxandcenter_called);

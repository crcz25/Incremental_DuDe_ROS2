...
template <class T, int D = 3> class Vector {
public:
  Vector(const T V[D]);
  ...
  void set(const Vector &other) { memcpy(v, other.v, D * sizeof(T)); }

  void set(const T other[D]) { memcpy(v, other, D * sizeof(T)); }
...
// Implementation
template <class T, int D> Vector<T, D>::Vector(const T V[D]) { set(V); }

template <class T, int D>
Vector<T, D>::Vector(const T &x, const T &y, const T &z, const T &w) {
  set(x, y, z, w);
}

template <class T, int D> Vector<T, D>::Vector(const Vector<T, D> &V) {
...
template <class T, int D> Vector<T, D> Vector<T, D>::operator-() const {
...
template <class T, int D> Vector<T, D> operator*(T &s, const Vector<T, D> &v) {
  return v * s;
}
...
template <class T, int D> istream &operator>>(istream &in, Vector<T, D> &vec) {
  for (int d = 0; d < D; d++)
    in >> vec[d];

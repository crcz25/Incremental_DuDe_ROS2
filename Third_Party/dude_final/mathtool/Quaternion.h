...
public:
  ...
  Quaternion(const Matrix4x4 &m4) {
    Matrix3x3 m3(m4[0][0], m4[0][1], m4[0][2], m4[1][0], m4[1][1], m4[1][2],
                 m4[2][0], m4[2][1], m4[2][2]);
    set(m3);
  }

  /// Construct from a rotation matrix m3
  Quaternion(const Matrix3x3 &m3) { set(m3); }
  ...
  Matrix3x3 getMatrix() const {
    double x_2 = 2 * sqr(m_v[0]);
    double y_2 = 2 * sqr(m_v[1]);
    double z_2 = 2 * sqr(m_v[2]);
    double xy = 2 * m_v[0] * m_v[1];
    double yz = 2 * m_v[1] * m_v[2];
    double zx = 2 * m_v[2] * m_v[0];
    double sx = 2 * m_s * m_v[0];
    double sy = 2 * m_s * m_v[1];
    double sz = 2 * m_s * m_v[2];
    return Matrix3x3(1 - y_2 - z_2, xy - sz, zx + sy, xy + sz, 1 - x_2 - z_2,
                     yz - sx, zx - sy, yz + sx, 1 - x_2 - y_2);
  }
  ...
  void set(double q1, double q2, double q3, double q4) {
    m_s = q1;
    m_v.set(q2, q3, q4);
  }

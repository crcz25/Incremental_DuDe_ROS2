...
public:
  ...
  void process_files(std::string name) {
    ...
    colormap = save_image_original_color(saving_path + "_TAG" + gt_ending,
                                         GT_segmentation);
    ...
    std::cout << name;
    printf(" No_Furniture Precision: %.1f Recall: %.1f time: %.0f Labels %.0f "
           "size %.2f\n",
           No_Furn_Results_Regions.precision, No_Furn_Results_Regions.recall,
           No_Furn_Results_Regions.time, max, proper_size);

    std::cout << name;
    printf(" Furniture Precision: %.1f Recall: %.1f time: %.0f Labels %.0f "
           "size %.2f\n",
           Furn_Results_Regions.precision, Furn_Results_Regions.recall,
           Furn_Results_Regions.time, max, proper_size);
  ...
  void process_files_incrementally(std::string name) {
    ...
    ///////// Original
    cv::Mat GT_segmentation = segment_Ground_Truth(image_GT);
    colormap = save_image_original_color(saving_path + "_TAG_inc" + gt_ending,
                                         GT_segmentation);
  ...
  /////////////////////
  std::map<int, int> compare_images2(cv::Mat GT_segmentation_in,
                                     cv::Mat DuDe_segmentation_in) {
  ...
  void process_all_files() {
    ...
    for (std::set<std::string>::iterator file_iter = files_to_read.begin();
         file_iter != files_to_read.end(); file_iter++) {
      std::cout << "Reading file  " << *file_iter << std::endl
                << std::endl
                << std::endl
                << std::endl;
    ...
    double std_precision =
        sqrt(this_quad_precision - this_precision * this_precision);
    ...
    for (int i = 0; i < Precisions.size(); i++) {
      ...
      std::cout << "current precision "
                << cum_inside_precision / Precisions[i].size() << std::endl;
      cum_quad_precision += cum_inside_precision / Precisions[i].size() *
    ...
    for (int i = 0; i < Recalls.size(); i++) {
      ...
      cum_quad_recall += cum_inside_recall / Recalls[i].size() *
                         cum_inside_recall / Recalls[i].size();
      std::cout << "current recall " << cum_inside_recall / Recalls[i].size()
                << std::endl;
    }

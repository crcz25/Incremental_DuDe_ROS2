...
public:
  ...
  void twistCallback(const geometry_msgs::msg::Twist::SharedPtr msg) {
    ...
    if (current_file < files_to_read.size()) {
      std::cout << "Reading file  " << files_to_read[current_file] << std::endl
                << std::endl;
      read_files(files_to_read[current_file]);

      std::cout << "Processed file  " << files_to_read[current_file]
                << std::endl
                << std::endl;
      // Print results
      ...
      std::cout << "Average Precision: " << cum_precision / size_precision
                << ", Average Recall: " << cum_recall / size_recall
                << std::endl;
      current_file++;
      publish_Image();
    }
  ...
  void twistCallback1(const geometry_msgs::msg::Twist::SharedPtr msg) {
    ...
    if (direction > 0) {
      ...
      std::cout << "File to process:  " << file_list[current_file] << std::endl
                << std::endl;
    } else {

      std::cout << "Processing file  " << file_list[current_file] << std::endl
                << std::endl;
      read_files(file_list[current_file]);

      std::cout << "Processed file  " << file_list[current_file] << std::endl
                << std::endl;
      // Print results
      std::cout << "Results  " << std::endl;
      float cum_precision = 0;
  ...
  void twistCallback2(const geometry_msgs::msg::Twist::SharedPtr msg) {
    ...
    if (current_file < files_to_read.size()) {
      ...
      std::string full_path_original =
          base_path + "/" + files_to_read[current_file] + gt_ending;
  ...
  void read_files(std::string name) {
    ...
    //			save_images_gray(name, DuDe_segmentation,
    //GT_segmentation);
  ...
  std::map<int, int> compare_images(cv::Mat GT_segmentation_in,
    ...
    for (tag2tagMapper::iterator it = gt_tag2mapper.begin();
      ...
      for (tag2points::iterator it2 = inside.begin(); it2 != inside.end();
        ...
        //					std::cout << "      " <<
        //it2->first << " with "<< it2->second.size() <<" points" << std::endl;
      }
      segmented2GT_tags[gt_tag_max] = it->first;
      //				std::cout << "   max is " <<
      //max_intersection << " that represents " <<
      //100*max_intersection/total_points   << std::endl;
  ...
  cv::Mat segment_Ground_Truth(cv::Mat GroundTruth_BW) {
    ...
    cv::dilate(drawing, drawing, cv::Mat(), cv::Point(-1, -1), 1,
               cv::BORDER_CONSTANT,
               cv::morphologyDefaultBorderValue()); // erode dilate drawing
    return drawing;
  }
  ...
  cv::Mat incremental_segment(cv::Mat image_in, double &time) {
    ...
    while (!stop_criteria) {
      ...
      if ((x > pre_decompose_BW.size().width) ||
          (y > pre_decompose_BW.size().height)) {
        stop_criteria = true;
      }
      ...
      while (entry = readdir(pDIR)) {
        if (strcmp(entry->d_name, ".") != 0 &&
          ...
          if (fullString.length() >= gt_ending.length()) {
            if (0 ==
                fullString.compare(fullString.length() - gt_ending.length(),
                                   gt_ending.length(), gt_ending)) {
              std::string filename = fullString.substr(
                  0, fullString.length() - gt_ending.length());
  ...
  void init() {
    ...
    for (int i = 0; i < files_to_read.size(); i++) {
      std::cout << "Reading file  " << files_to_read[i] << std::endl
                << std::endl
                << std::endl
                << std::endl;
  ...
  void print_report(std::vector<std::string> files_to_read) {
    ...
    for (int file_num = 0; file_num < files_to_read.size(); file_num++) {
      // Print results
      std::cout << "   " << file_num + 1
                << ") File:  " << files_to_read[file_num] << std::endl;
      float cum_precision = 0;
      float cum_recall = 0;
      int size_precision = 0, size_recall = 0;
      ...
      printf("     Avg Precision: %.2f, Avg Recall: %.2f, time: %.3f \n",
             cum_precision / size_precision, cum_recall / size_recall,
             Times[file_num] / 1000);
      current_file++;
    ...
    float std_precision =
        sqrt(avg_quad_precision - avg_precision * avg_precision);
    ...
    //			std::cout << "Full Average Precision: "<<
    //cum_precision/size_precision  << ", Full Average Recall: "<<
    //cum_recall/size_recall << std::endl;
    std::cout << files_to_read.size();
    printf(" files processed. Avg time: %.0f+%.0f ms, Avg Precision: "
           "%.1f%%+%.1f%%, Avg Recall %.1f%%+%.1f%% \n",
           avg_time, std_time, avg_precision, std_precision, avg_recall,
           std_recall);
  ...
  //////////////////
  void save_images_gray(std::string name, cv::Mat DuDe_segmentation,
                        cv::Mat GT_segmentation) {
    std::string full_path_decomposed =
        base_path + "/Tagged_Images/" + name + "_DuDe_segmented.png";
    double min, max;
    ...
    full_path_decomposed =
        base_path + "/Tagged_Images/" + name + "_original_tags.png";
  ...
  /////////////////
  void save_images_color(std::string name, cv::Mat DuDe_segmentation,
                         cv::Mat GT_segmentation) {
    std::string full_path_decomposed =
        base_path + "/Tagged_Images/" + name + "_DuDe_segmented" + FuT_ending;
    double min, max;
    ...
    ///////////////////////////
    full_path_decomposed =
        base_path + "/Tagged_Images/" + name + "_tagged" + gt_ending;
    color_vector.clear();
    color_vector.push_back(black);
    cv::minMaxLoc(GT_segmentation, &min, &max);
    for (int i = 0; i <= max; i++) {
    ...
    cv::Mat GT_segmentation_float =
        cv::Mat::zeros(GT_segmentation.size(), CV_8UC3);
  ...
  /////////////////
  void save_images_color3(std::string name, cv::Mat DuDe_segmentation,
                          cv::Mat GT_segmentation,
                          cv::Mat inc_DuDe_segmentation) {
    std::string full_path_decomposed =
        base_path + "/Tagged_Images/" + name + "_DuDe_segmented.png";
    double min, max;
    ...
    //////////////////////////////
    full_path_decomposed =
        base_path + "/Tagged_Images/" + name + "_inc_dude_segmented.png";
    color_vector.clear();
    color_vector.push_back(black);
    ...
    cv::Mat inc_DuDe_segmentation_float =
        cv::Mat::zeros(inc_DuDe_segmentation.size(), CV_8UC3);
    for (int i = 0; i < inc_DuDe_segmentation.rows; i++) {
      for (int j = 0; j < inc_DuDe_segmentation.cols; j++) {
        ...
        inc_DuDe_segmentation_float.at<cv::Vec3b>(i, j) =

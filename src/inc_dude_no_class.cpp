...
public:
  ...
  void mapCallback(const nav_msgs::OccupancyGridConstPtr &map) {
    ...
    double elapsed_secs_process; // = double(end_process - begin_process) /
                                 // CLOCKS_PER_SEC;
    //			end_process=clock();   elapsed_secs_process =
    //double(end_process - begin_process) / CLOCKS_PER_SEC;
    //std::cerr<<"Time elapsed in process "<< elapsed_secs_process*1000 << "
    //ms"<<std::endl;

    {
      std::cout << "Map_Info_.resolution  " << Map_Info_.resolution
                << std::endl;
    ...
    if ((map->info.origin.position.x != current_origin_.position.x) ||
        (map->info.origin.position.y != current_origin_.position.y)) {
    ...
    cv::Mat Occ_image(map->info.height + 2 * gap, map->info.width + 2 * gap,
                      CV_8U, 255);
    ...
    end_process = clock();
    elapsed_secs_process = double(end_process - begin_process) / CLOCKS_PER_SEC;
    std::cerr << "Time elapsed in process transform "
              << elapsed_secs_process * 1000 << " ms" << std::endl
              << std::endl;
    ...
    cv::Mat stable_drawing = cv::Mat::zeros(Occ_image.size().height,
                                            Occ_image.size().width, CV_8UC1);
    ...
    for (int i = 0; i < Stable.Region_contour.size(); i++) {
      ...
      for (int j = 0; j < big_contours_vector.size(); j++) {
        int connected = 0;
        cv::Point centroid;
        are_contours_connected(Stable.Region_contour[i], big_contours_vector[j],
                               centroid, connected);
        if (connected > 0) {
          //							cout << "contour
          //"<< j << " in region " <<i<< " is connected to stable region "<<k <<
          //endl;
          vector<int> pair;
          pair.push_back(i);
          pair.push_back(j);
    ...
    /*
                            cout << "number of growing regions " <<
    conection_prev_new.size() << endl; cout << "connected_contours.size " <<
    connected_contours.size() << endl; cout << "unconnected_contours.size " <<
    unconnected_contours.size() << endl; cout << "Sum " <<
    connected_contours.size() + unconnected_contours.size() << endl; cout <<
    "Original " << Stable.Region_contour.size() << endl;
    //*/

    // Draw image with expanded contours matched
    cv::Mat expanded_drawing = cv::Mat::zeros(Occ_image.size().height,
                                              Occ_image.size().width, CV_8UC1);
    drawContours(expanded_drawing, connected_contours, -1, 2, -1, 8);

    for (int i = 0; i < conection_prev_new.size(); i++) {
      drawContours(expanded_drawing, big_contours_vector,
                   conection_prev_new[i][1], 2, -1, 8);
    }
    ...
    /*
                            for (std::vector<vector<cv::Point> >::iterator it =
       Stable.Region_contour.begin() ; it != Stable.Region_contour.end(); ++it){
                                    int a=2;
                            }
    */
    ...
    end_process = clock();
    elapsed_secs_process = double(end_process - begin_process) / CLOCKS_PER_SEC;
    std::cerr << "Time elapsed in Pre-Decomp " << elapsed_secs_process * 1000
              << " ms" << std::endl
              << std::endl;
    ...
    for (int i = 0; i < Expanded_contour.size(); i++) {
      cv::Mat Temporal_Image = cv::Mat::zeros(Occ_image.size().height,
                                              Occ_image.size().width, CV_8UC1);
      cv::Mat temporal_image_cut = cv::Mat::zeros(
          Occ_image.size().height, Occ_image.size().width, CV_8UC1);
    ...
    end_process = clock();
    elapsed_secs_process = double(end_process - begin_process) / CLOCKS_PER_SEC;
    std::cerr << "Time elapsed in process multiple Decomp "
              << elapsed_secs_process * 1000 << " ms" << std::endl
              << std::endl;
    ...
    end_process = clock();
    elapsed_secs_process = double(end_process - begin_process) / CLOCKS_PER_SEC;
    std::cerr << "Time elapsed in process Stable Graph "
              << elapsed_secs_process * 1000 << " ms" << std::endl
              << std::endl;
    ...
    cv::Mat Drawing = cv::Mat::zeros(Occ_image.size().height,
                                     Occ_image.size().width, CV_8UC1);
    ...
    for (int i = 0; i < joint_centroids.size(); i++) {
      stringstream mix;
      mix << i;
      std::string text = mix.str();
      putText(Drawing, text,
              cv::Point(joint_centroids[i].x,
                        Occ_image.size().height - joint_centroids[i].y),
              cv::FONT_HERSHEY_SCRIPT_SIMPLEX, 0.5, joint_centroids.size() + 1,
              1, 8);
    }

    /*
                            cv::Mat Drawing2 =
    cv::Mat::zeros(Occ_image.size().height, Occ_image.size().width, CV_8UC1);

                            for(int i = 0; i <unconnected_contours.size();i++){
                                    drawContours(Drawing2, unconnected_contours,
    i, i+1, -1, 8);
                            }
                            cv::flip(Drawing2,Drawing2,0);
                            for(int i = 0; i <unconnected_centroids.size();i++){
                                    stringstream mix;      mix<<i;
    std::string text = mix.str(); putText(Drawing2, text,
    cv::Point(unconnected_centroids[i].x, Occ_image.size().height -
    unconnected_centroids[i].y ), cv::FONT_HERSHEY_SCRIPT_SIMPLEX, 0.5,
    unconnected_centroids.size()+1, 1, 8);
                            }
    ...
    resize_rect.y =
        Occ_image.size().height -
        (resize_rect.y + resize_rect.height); // because of the flipping images
    ...
    end_process = clock();
    elapsed_secs_process = double(end_process - begin_process) / CLOCKS_PER_SEC;
    std::cerr << "Time elapsed in process Paint " << elapsed_secs_process * 1000
              << " ms" << std::endl
              << std::endl;
  ...
  void odomCallback(const nav_msgs::Odometry &msg) {
    ...
    if (path_.size() > 0)
      distance += cv::norm(temp_Point - position_cm_);
  ...
  void publish_Contour() {
    ...
    for (int i = 0; i < Convex_Marker_.size(); i++) {
      for (int j = 0; j < Convex_Marker_[i].size(); j++) {
        ...
        //					std::cout <<"Points X:  "<<
        //Voronoi_Marker_[i].at(j).x + myROI_.x<<"   Y:  "<<
        //Voronoi_Marker_[i].at(j).y + myROI_.y << std::endl; 					std::cout
  ...
  cv::Mat clean_image3(cv::Mat Occ_Image, cv::Mat &black_image) {
    ...
    double end_cleaning = getTime();
    cout << "done, it last " << (end_cleaning - start_cleaning) << " ms"
         << endl;
    return cut_image;
  }
  ...
  void are_contours_connected(vector<cv::Point> first_contour,
                              vector<cv::Point> second_contour,
                              cv::Point &centroid, int &number_of_ones) {
  ...
  void adjust_stable_contours() {
    ...
    correction.x = (current_origin_.position.x - Map_Info_.origin.position.x) /
                   Map_Info_.resolution;
    correction.y = (current_origin_.position.y - Map_Info_.origin.position.y) /
                   Map_Info_.resolution;
    ...
    for (int i = 0; i < Stable.Region_contour.size(); i++) {
      ...
      for (int j = 0; j < Stable.Region_contour[i].size(); j++) {
        ...
        //					Stable.Region_contour[i][j] =
        //cartesian_to_pixel(pixel_to_cartesian(Stable.Region_contour[i][j]));
      }
    }
  ...
  cv::Point pixel_to_cartesian(cv::Point point_in) {
    cv::Point point_out;
    point_out.x =
        point_in.x * Map_Info_.resolution + current_origin_.position.x;
    point_out.y =
        point_in.y * Map_Info_.resolution + current_origin_.position.y;
  ...
  cv::Point cartesian_to_pixel(cv::Point point_in) {
    cv::Point point_out;
    point_out.x =
        (point_in.x - Map_Info_.origin.position.x) / Map_Info_.resolution;
    point_out.y =
        (point_in.y - Map_Info_.origin.position.y) / Map_Info_.resolution;

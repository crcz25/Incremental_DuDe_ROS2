...
cv::Mat Stable_graph::draw_stable_contour() {
  cv::Mat Drawing =
      cv::Mat::zeros(image_size.height, image_size.width, CV_8UC1);
  for (int i = 0; i < Region_contour.size(); i++) {
    drawContours(Drawing, Region_contour, i, i + 1, -1, 8);
  }
  cv::flip(Drawing, Drawing,
           0); /*
for(int i = 0; i < Region_centroid.size();i++){
stringstream mix;      mix<<i;				std::string text =
mix.str(); putText(Drawing, text, cv::Point(Region_centroid[i].x,
image_size.height - Region_centroid[i].y ), cv::FONT_HERSHEY_SCRIPT_SIMPLEX,
0.5, Region_centroid.size()+1, 1, 8);
}	*/
  return Drawing;
}
...
/////////////////////////////////
///// MAIN FUNCTION
Stable_graph Incremental_Decomposer::decompose_image(cv::Mat image_cleaned,
                                                     float pixel_Tau,
                                                     cv::Point2f origin,
                                                     float resolution_in) {
  ...
  cv::Mat stable_drawing = cv::Mat::zeros(image_cleaned.size().height,
                                          image_cleaned.size().width, CV_8UC1);
  drawContours(stable_drawing, Stable.Region_contour, -1, 255, -1, 8);

  cv::Mat working_image;
  working_image = image_cleaned & ~stable_drawing;
  //		compare(image_cleaned, stable_drawing, working_image,
  //cv::CMP_NE);
  ...
  /*
          cout << "number of growing regions " << conection_prev_new.size() <<
  endl; cout << "connected_contours.size " << connected_contours.size() << endl;
          cout << "unconnected_contours.size " << unconnected_contours.size() <<
  endl; cout << "Sum " << connected_contours.size() +
  unconnected_contours.size() << endl; cout << "Original " <<
  Stable.Region_contour.size() << endl;
  //*/

  // Draw image with expanded contours matched
  cout << "Draw image with expanded contours matched " << endl;
  cv::Mat expanded_drawing = cv::Mat::zeros(
      image_cleaned.size().height, image_cleaned.size().width, CV_8UC1);
  ...
  //		cout << "Expanded_contour.size "<<Expanded_contour.size()<<
  //endl;

  //		end_process=clock();   elapsed_secs_process = double(end_process
  //- begin_process) / CLOCKS_PER_SEC;			std::cerr<<"Time elapsed
  //in paint expanded diference "<< elapsed_secs_process*1000 << "
  ...
  for (int i = 0; i < Expanded_contour.size(); i++) {
    cv::Mat Temporal_Image = cv::Mat::zeros(
        image_cleaned.size().height, image_cleaned.size().width, CV_8UC1);
  ...
  //		end_process=clock();   elapsed_secs_process = double(end_process
  //- begin_process) / CLOCKS_PER_SEC;			std::cerr<<"Time elapsed
  //in process multiple Decomp "<< elapsed_secs_process*1000 << "
  //ms"<<std::endl<<std::endl;
  ...
  //		end_process=clock();   elapsed_secs_process = double(end_process
  //- begin_process) / CLOCKS_PER_SEC;			std::cerr<<"Time elapsed
  //in process Stable Graph "<< elapsed_secs_process*1000 << "
  //ms"<<std::endl<<std::endl;
...
void Incremental_Decomposer::are_contours_connected(
    vector<cv::Point> first_contour, vector<cv::Point> second_contour,

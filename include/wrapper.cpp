...
///////////////////////////////////
void DuDe_OpenCV_wrapper::insert_contour_to_poly(
    std::vector<cv::Point> contour_in, c_ply &polygon) {
...
//Occupancy Image to Free Space
  std::cout << "Cleaning Image..... "; 		double start_cleaning =
getTime(); cv::Mat open_space = Occ_Image<10; cv::Mat black_image = Occ_Image>90
& Occ_Image<=100; cv::Mat Median_Image, Image_in, cut_image ;
  {
          cout << "Entering........ ";
          cv::dilate(black_image, black_image, cv::Mat(), cv::Point(-1,-1), 4,
cv::BORDER_CONSTANT, cv::morphologyDefaultBorderValue() ); cout <<
"dilated........ "; cv::medianBlur(open_space, Median_Image, 3); cout << "Median
Blur........ "; Image_in = Median_Image & ~black_image; cout << "And........ ";
          Image_in.copyTo(cut_image);
          cout << "copy........ ";
  }
  double end_cleaning = getTime();  cout << "done, it last
"<<(end_cleaning-start_cleaning)<< " ms"  << endl;
*/
  ///////////////////////////////
  ...
  cout << "done, it last " << (end_inserting - start_inserting) << " ms"
       << endl;
  ...
  cout << "done, decomposing " << finalPolygonPieces.size() << " polygons last "
       << (end_dude - start_dude) << " ms" << endl;
...
void DuDe_OpenCV_wrapper::Dual_decompose(c_polygon &polygons) {
  ...
  iterativeDecompose(*p1, dude.m_cuts, g_tau, finalPolygonPieces,
                     draw_decoration.allAccumulatedCuts, draw_decoration.se,
                     true);
  //	decomposeMoreTimes(polygons, dude.m_cuts, g_tau, finalPolygonPieces,
  //draw_decoration.allAccumulatedCuts, draw_decoration.se, true, 0); 	cout <<
  //"finalPolygonPieces.size() " << finalPolygonPieces.size() <<endl;
...
void DuDe_OpenCV_wrapper::export_all_svg_files() {
  ...
  svg::Dimensions svg_viewbox_dim(width + stroke_width * 2,
                                  height + stroke_width * 2);
  svg::Point svg_viewbox_org(bbox[0] - stroke_width,
                             height - bbox[3] + stroke_width);

  stringstream ss;
  ss << basename.c_str() << "_dude2d.svg";
  string sname = ss.str();
  svg::Document doc(sname, svg::Layout(dimensions, svg::Layout::BottomLeft,
                                       svg_viewbox_dim, svg_viewbox_org));

  for (int i = 0; i < psize; i++) {
    svg::Color randColor(svg::Color::Defaults(svg::Color::Aqua + (i % 15)));
    svg::Polygon p_i(svg::Fill(randColor),
                     svg::Stroke(stroke_width, svg::Color::Black));
...
void DuDe_OpenCV_wrapper::extract_contour_from_polygon() {
  ...
  for (int i = 0; i < finalPolygonPieces.size(); i++) {
    ...
    ply_vertex *ptr =
        temp_iter->getHead(); // There's only one polygon after decomposed
    do {
...
void DuDe_OpenCV_wrapper::extract_graph() {
  ...
  for (vector<c_diagonal>::iterator Diag_it = Diagonals.begin();
       Diag_it != Diagonals.end(); ++Diag_it) {
    ...
    cv::Point Diag_avg = cv::Point((pos_vertex1[0] + pos_vertex2[0]) / 2,
                                   (pos_vertex1[1] + pos_vertex2[1]) / 2);
    diagonal_centroid.push_back(Diag_avg);

    for (int contour_count = 0; contour_count < Decomposed_contours.size();
         contour_count++) {
      std::vector<cv::Point> current_contour =
          Decomposed_contours[contour_count];
    ...
    for (std::set<int>::iterator set_it = contours_connected.begin();
         set_it != contours_connected.end(); set_it++) {
      int connected_from = *set_it;
      for (std::set<int>::iterator set_it2 = contours_connected.begin();
           set_it2 != contours_connected.end(); set_it2++) {
...
void DuDe_OpenCV_wrapper::print_graph() {
  ...
  for (int i = 0; i < contours_centroid.size(); i++) {
    ...
    for (std::set<int>::iterator set_it = contours_connections[i].begin();
         set_it != contours_connections[i].end(); set_it++) {
      int connected_from = *set_it;
...
void DuDe_OpenCV_wrapper::measure_performance() {
  ...
  // Compactness second moment (s_i) =(1/(Mi*Ai))* \sum(((xi-x0)^2 + (yi-y0)^2
  // )/(Ai))
  //  q_i = c_i -s_i
  ...
  for (int i = 0; i < Decomposed_contours.size(); i++) {
    ...
    //		cout << "convexity "<< convexities.back() <<", area " <<
    //areas.back() << ", compactness "<< s_i<<endl;
    float q_i = c_i - s_i;
  ...
  // Simplicity exp(-(N-Ñ)/\Phi). N= current number of nodes; Ñ: expected number
  // of regions
  ...
  Overall_Quality = (Area_Coverage_Ratio * Validity_Ratio) /
                        (contours_centroid.size()) * Total_Quality +
                    lambda * Simplicity;

  cout << "C " << Area_Coverage_Ratio << " R " << Validity_Ratio << " N "
       << contours_centroid.size() << " S " << Simplicity << " Q_avg "
       << Total_Quality / contours_centroid.size() << endl;
